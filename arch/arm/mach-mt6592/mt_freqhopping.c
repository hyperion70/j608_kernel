...
#define MEMPLL367MHZ_DDS	0x00163a82	// 1466
...
static const struct freqhopping_ssc mt_ssc_mempll_setting[MT_SSC_NR_PREDEFINE_SETTING]= {
	...
	{367111 ,2 ,7 ,0, 1, MEMPLL367MHZ_DDS},//high freq. down SSC 0 ~ -1% =>for PCDDR3 use
	{367222 ,2 ,7 ,1, 0, MEMPLL266MHZ_DDS},//low freq. up SSC -1% ~ 0 =>for PCDDR3 use
	{0,0,0,0,0,0} //EOF
};
...
static noinline int __freq_to_index(enum FH_PLL_ID pll_id,int freq) 
{
	...
	switch(pll_id) {
		...
		if((get_ddr_type()== DDR3_16) || (get_ddr_type()== DDR3_32))	//PCDDR3 -1%
		{
			freq = 367111;	//down SSC	
			//printk("PCDDR3 Down SSC :0 ~ -1%, max freq = %d", freq);
		}
		while(i < MT_SSC_NR_PREDEFINE_SETTING){
...
static int mt_fh_hal_l2h_dvfs_mempll(void)  //mempll low to high (266->333MHz)
{
    ...
    if((get_ddr_type()== DDR3_16) || (get_ddr_type()== DDR3_32))	//PCDDR3 -1%
    {
    	//printk("PCDDR3 Down SSC :0 ~ -1%");
    	fh_set_field(REG_FHCTL2_CFG,FH_SFSTRX_DYS,mt_ssc_mempll_setting[5].df);
			fh_set_field(REG_FHCTL2_CFG,FH_SFSTRX_DTS,mt_ssc_mempll_setting[5].dt);
			//low to high, apply down hopping, config low bound.
			fh_write32(REG_FHCTL2_UPDNLMT, (PERCENT_TO_DDSLMT((fh_read32(REG_FHCTL2_DDS)&0x1FFFFF),mt_ssc_mempll_setting[5].lowbnd) << 16));    	
    }
    else	//LPDDR -8%
    {		
...
static int mt_fh_hal_h2l_dvfs_mempll(void)  //mempll high to low(333->266MHz)
{
    ...
    if((get_ddr_type()== DDR3_16) || (get_ddr_type()== DDR3_32))	//PCDDR3 1%
    {
    	//printk("PCDDR3 Up SSC :-1% ~ 0");
			fh_set_field(REG_FHCTL2_CFG,FH_SFSTRX_DYS,mt_ssc_mempll_setting[6].df);
			fh_set_field(REG_FHCTL2_CFG,FH_SFSTRX_DTS,mt_ssc_mempll_setting[6].dt);
			//high to low, apply up hopping, config upper bound.
			fh_write32(REG_FHCTL2_UPDNLMT, (PERCENT_TO_DDSLMT((fh_read32(REG_FHCTL2_DDS)&0x1FFFFF),mt_ssc_mempll_setting[6].upbnd)));		
			//fh_write32(REG_FHCTL2_UPDNLMT, (PERCENT_TO_DDSLMT((fh_read32(REG_FHCTL2_DDS)&0x1FFFFF),mt_ssc_mempll_setting[3].lowbnd) << 16));		   	
    }
    else //LPDDR 8%
    {    			
