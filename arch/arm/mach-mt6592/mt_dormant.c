...
int cpu_power_down(int mode,bool individual)
{
    ...
    mt_secure_call (MC_FC_GOTO_SLEEP, __pa(restore_ptr), cpu_id, individual==0);
...
void cpu_check_dormant_abort(void)
{
    ...
    else
    {
        if ((cpu_id == 0) && (cluster_id == 0) && (dormant_ret_flag[cluster_id][cpu_id] == 2))
        {
...
#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
            mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 1, 1);
#endif            
            spm_mtcmos_ctrl_cpu1(STA_POWER_ON, 1);

#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
            mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 2, 1);
#endif            
            spm_mtcmos_ctrl_cpu2(STA_POWER_ON, 1);

#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
            mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 3, 1);
#endif            
            ...
            {
#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
                mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 4, 1);
#endif            
                spm_mtcmos_ctrl_cpu4(STA_POWER_ON, 1);

#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
                mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 5, 1);
#endif 
                spm_mtcmos_ctrl_cpu5(STA_POWER_ON, 1);

#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
                mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 6, 1);
#endif 
                spm_mtcmos_ctrl_cpu6(STA_POWER_ON, 1);

#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
                mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 7, 1);
#endif 
...
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT)
        //printk("[Power/mcdi_dormant]: abort before SMC [cpu_id == 0x%x]\n", cpu_id);
        mt_secure_call(MC_FC_SLEEP_CANCELLED, 0, cpu_id, 0);            
#endif
