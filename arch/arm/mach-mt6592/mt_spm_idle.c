...
#endif
#if SPM_AEE_RR_REC
extern void aee_rr_rec_mcdi_wfi_val(u32 val);
extern u32 aee_rr_curr_mcdi_wfi_val(void);
...
#define SPM_MCDI_MAX_POLLING_COUNTER 10000

static void spm_read_timeout(u32 addr, u32 value, int mask)
{
    u64 wait_time = 0; 
		
    if( mask==0 )
    {		
        while(spm_read(addr)!=value)
        {
            wait_time++;
            udelay(1);
            if(wait_time==SPM_MCDI_MAX_POLLING_COUNTER)
                return;
        };
    }			
    return;
...
static void spm_mcdi_cpu_wake_up_event(bool wake_up_event, bool disable_dormant_power)
{
    ...
    spm_read_timeout(SPM_PCM_REG6_DATA, 0xdeaddead, 0);
    ...
    if(disable_dormant_power)
    {
        ...
        spm_read_timeout(SPM_CLK_CON, spm_read(SPM_CLK_CON)|CC_DISABLE_DORM_PWR, 0);
    }
    else
    {
        spm_write(SPM_CLK_CON,spm_read(SPM_CLK_CON)&~CC_DISABLE_DORM_PWR);
    ...
    spm_read_timeout(SPM_SLEEP_CPU_WAKEUP_EVENT, wake_up_event, 0);
    ...
    spm_read_timeout(SPM_PCM_REG6_DATA, 0x12345678, 0);
...
u32 spm_leave_MCDI(void)
{
    ...
    spm_read_timeout(SPM_PCM_REG11_DATA,0x55AA55AA,0);
...
static bool spm_is_cpu_irq_occur(int core_id)
{
	...
	{		
		...
		{
			case 4:
				if(spm_read(SPM_PCM_REG12_DATA)&(1<<7))	
					ret = 1;
				break;
			case 5:
				if(spm_read(SPM_PCM_REG12_DATA)&(1<<15))	
					ret = 1;				
				break;
			case 6:
				if(spm_read(SPM_PCM_REG12_DATA)&(1<<17))	
					ret = 1;				
				break;
			case 7:
				if(spm_read(SPM_PCM_REG12_DATA)&(1<<23))	
					ret = 1;				
				break;
...
{   
    ...
    //printk("spm_mcdi_wfi\n");
#if SPM_AEE_RR_REC
    aee_rr_rec_mcdi_wfi_val(aee_rr_curr_mcdi_wfi_val()|(1<<core_id));
#else
    g_is_mcdi_wfi=( g_is_mcdi_wfi | (1<<core_id) );
#endif    
    spin_unlock_irqrestore(&spm_lock, flags);
    
    if((SPM_MCDI_isKICK==0)||((spm_read(SPM_SLEEP_CPU_WAKEUP_EVENT)&0x1)==1)||((spm_read(SPM_APMCU_PWRCTL)&0x1)==1)||(atomic_read(&is_in_hotplug) >= 1))
    {
        spin_lock_irqsave(&spm_lock, flags);
#if SPM_AEE_RR_REC
        aee_rr_rec_mcdi_wfi_val(aee_rr_curr_mcdi_wfi_val()&~(1<<core_id));
...
void spm_mcdi_wakeup_all_cores(void)
{
    ...
    if((SPM_MCDI_isKICK==0)&&(spm_read(SPM_PCM_REG15_DATA)==0))
        return;
    /*disable MCDI Dormant*/
    //spm_write(SPM_CLK_CON,spm_read(SPM_CLK_CON)|CC_DISABLE_DORM_PWR);
    ...
    //spm_mcdi_cpu_wake_up_event(1,1);
#if SPM_AEE_RR_REC
    aee_rr_rec_mcdi_r15_val(spm_read(SPM_PCM_REG15_DATA));
#endif
    smp_call_function(empty_function, NULL, 1);
