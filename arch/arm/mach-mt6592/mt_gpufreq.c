...
{
    gpufreq_info("mt_gpufreq_check_freq_and_set_pll, g_cur_freq = %d\n", g_cur_freq);
...
static void mt_gpufreq_period_time_reset(void)
{
	...
	gpufreq_info("mt_gpufreq_period_time_reset(), mt_gpufreq_prev_wall_time = %d\n", mt_gpufreq_prev_wall_time);
}
...
unsigned int mt_gpufreq_gpu_clock_ratio(enum mt_gpufreq_clock_ratio act)
{
    ...
    else if(act == GPU_DVFS_CLOCK_RATIO_GET)
    {
        ...
        if(mt_gpufreq_clock_on == 1)
        {
			...
			gpufreq_dbg("GET, clock is still ON!\n");
        }
        ...
        gpufreq_dbg("GET 2, mt_gpu_clock_on_time_start = %d, mt_gpu_clock_total_on_time = %d\n", mt_gpu_clock_on_time_start, mt_gpu_clock_total_on_time);
        gpufreq_dbg("GET 3, mt_gpufreq_prev_wall_time = %d, mt_gpufreq_period_time = %d\n", mt_gpufreq_prev_wall_time, mt_gpufreq_period_time);
...
static int mt_setup_gpufreqs_table(struct mt_gpufreq_info *freqs, int num)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        gpufreq_info("freqs[%d].gpufreq_khz = %u\n", i, freqs[i].gpufreq_khz);
        gpufreq_info("freqs[%d].gpufreq_lower_bound = %u\n", i, freqs[i].gpufreq_lower_bound);
        gpufreq_info("freqs[%d].gpufreq_upper_bound = %u\n", i, freqs[i].gpufreq_upper_bound);
        gpufreq_info("freqs[%d].gpufreq_volt = %u\n", i, freqs[i].gpufreq_volt);
        gpufreq_info("freqs[%d].gpufreq_remap = %u\n", i, freqs[i].gpufreq_remap);
    }
    ...
    gpufreq_info("mt_setup_gpufreqs_table, g_cur_freq = %d, g_cur_volt = %d\n", g_cur_freq, g_cur_volt);
...
static void mt_gpu_clock_switch(unsigned int sel)
{
    ...
    switch (sel)
    {
        case GPU_MMPLL_D3: // 476Mhz
            ...
            gpufreq_dbg("mt_gpu_clock_switch: switch MFG clock to GPU_MMPLL_D3\n");

            clk_cfg_4 = (clk_cfg_4 & 0xFFFFFFF8) | (0x5);
            mt65xx_reg_sync_writel(clk_cfg_4, CLK_CFG_4);
            gpufreq_dbg("mt_gpu_clock_switch: switch HYD clock to GPU_MMPLL_D3\n");
        ...
        case GPU_SYSPLL_D2: // 403Mhz
            ...
            mt65xx_reg_sync_writel(clk_cfg_0, CLK_CFG_0);
            gpufreq_dbg("mt_gpu_clock_switch: switch MFG clock to GPU_SYSPLL_D2\n");

            clk_cfg_4 = (clk_cfg_4 & 0xFFFFFFF8) | (0x2);
            mt65xx_reg_sync_writel(clk_cfg_4, CLK_CFG_4);
            gpufreq_dbg("mt_gpu_clock_switch: switch HYD clock to GPU_SYSPLL_D2\n");
        ...
        case GPU_MMPLL_D4: // 357Mhz
            ...
            mt65xx_reg_sync_writel(clk_cfg_0, CLK_CFG_0);
            gpufreq_dbg("mt_gpu_clock_switch: switch MFG clock to GPU_MMPLL_D4\n");

            clk_cfg_4 = (clk_cfg_4 & 0xFFFFFFF8) | (0x6);
            mt65xx_reg_sync_writel(clk_cfg_4, CLK_CFG_4);
            gpufreq_dbg("mt_gpu_clock_switch: switch HYD clock to GPU_MMPLL_D4\n");
        ...
        case GPU_UNIVPLL1_D2: // 312Mhz
            ...
            mt65xx_reg_sync_writel(clk_cfg_0, CLK_CFG_0);
            gpufreq_dbg("mt_gpu_clock_switch: switch MFG clock to GPU_UNIVPLL1_D2\n");

            clk_cfg_4 = (clk_cfg_4 & 0xFFFFFFF8) | (0x4);
            mt65xx_reg_sync_writel(clk_cfg_4, CLK_CFG_4);
            gpufreq_dbg("mt_gpu_clock_switch: switch HYD clock to GPU_UNIVPLL1_D2\n");
        ...
        case GPU_MMPLL_D5: // 286Mhz
           if(mt_gpufreq_enable_mmpll == 0)
            ...
            mt65xx_reg_sync_writel(clk_cfg_0, CLK_CFG_0);
            gpufreq_dbg("mt_gpu_clock_switch: switch MFG clock to GPU_MMPLL_D5\n");

            clk_cfg_4 = (clk_cfg_4 & 0xFFFFFFF8) | (0x7);
            mt65xx_reg_sync_writel(clk_cfg_4, CLK_CFG_4);
            gpufreq_dbg("mt_gpu_clock_switch: switch HYD clock to GPU_MMPLL_D5\n");
        ...
        case GPU_SYSPLL_D3: // 268Mhz
            ...
            mt65xx_reg_sync_writel(clk_cfg_0, CLK_CFG_0);
            gpufreq_dbg("mt_gpu_clock_switch: switch MFG clock to GPU_SYSPLL_D3\n");
        ...
        case GPU_MMPLL_D6: // 238Mhz
            ...
            mt65xx_reg_sync_writel(clk_cfg_0, CLK_CFG_0);
            gpufreq_dbg("mt_gpu_clock_switch: switch MFG clock to GPU_MMPLL_D6\n");
        ...
        case GPU_UNIVPLL1_D4: // 156Mhz
            ...
            mt65xx_reg_sync_writel(clk_cfg_0, CLK_CFG_0);
            gpufreq_dbg("mt_gpu_clock_switch: switch MFG clock to GPU_UNIVPLL1_D4\n");
            ...
            break;
        default:
            gpufreq_warn("mt_gpu_clock_switch: do not support specified clock (%d)\n", sel);
            break;
    }

    gpufreq_dbg("CLK_CFG_0 = 0x%x, CLK_CFG_4 = 0x%x\n", DRV_Reg32(CLK_CFG_0), DRV_Reg32(CLK_CFG_4));
}
...
static void mt_gpu_volt_switch(unsigned int volt_old, unsigned int volt_new)
{
    ...
    {
        gpufreq_dbg("mt_gpu_volt_switch: switch MFG power to GPU_POWER_VRF18_1_15V\n");
    }
    else if (volt_new == GPU_POWER_VRF18_1_10V)
    {
        gpufreq_dbg("mt_gpu_volt_switch: switch MFG power to GPU_POWER_VRF18_1_10V\n");
    }
    else if (volt_new == GPU_POWER_VRF18_1_05V)
    {
        gpufreq_dbg("mt_gpu_volt_switch: switch MFG power to GPU_POWER_VRF18_1_05V\n");
    }
    else if (volt_new == GPU_POWER_VCORE_1_05V)
    {
        gpufreq_dbg("mt_gpu_volt_switch: use VCORE, not support voltage scaling\n");
        return;
    }
    else
    {
        gpufreq_warn("mt_gpu_volt_switch: do not support specified volt (%d)\n", volt_new);
        return;
    }
    ...
    if (volt_old < volt_new)
    {
        ...
        {
            gpufreq_dbg("mt_gpu_volt_switch: volt_old < volt_new, i=(%d)\n", i);
...
static void mt_gpu_volt_switch_initial(unsigned int volt_new)
{
    ...
    if (volt_new == GPU_POWER_VRF18_1_15V)
    {
        ...
        gpufreq_info("mt_gpu_volt_switch_initial: switch MFG power to GPU_POWER_VRF18_1_15V\n");
    }
    else if (volt_new == GPU_POWER_VRF18_1_10V)
    {
        upmu_set_vrf18_2_vosel(2);
        gpufreq_info("mt_gpu_volt_switch_initial: switch MFG power to GPU_POWER_VRF18_1_10V\n");
    }
    else if (volt_new == GPU_POWER_VRF18_1_05V)
    {
        upmu_set_vrf18_2_vosel(0);
        gpufreq_info("mt_gpu_volt_switch_initial: switch MFG power to GPU_POWER_VRF18_1_05V\n");
    }
    else if (volt_new == GPU_POWER_VCORE_1_05V)
    {
        gpufreq_info("mt_gpu_volt_switch_initial: use VCORE, not support voltage scaling\n");
        return;
    }
    else
    {
        gpufreq_warn("mt_gpu_volt_switch_initial: do not support specified volt (%d)\n");
        return;
    }
...
void mt_gpufreq_thermal_protect(unsigned int limited_power)
{
    ...
    gpufreq_info("limit frequency upper bound to id = %d, frequency = %d\n", g_limited_max_id, mt_gpufreqs[g_limited_max_id].gpufreq_khz);
...
static ssize_t mt_gpufreq_state_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    if((mt_gpufreq_registered == false) && (mt_gpufreq_registered_statewrite == false))
    {
        ...
        gpufreq_info("freqs[0].gpufreq_khz = %u\n", mt_gpufreqs[0].gpufreq_khz);
        gpufreq_info("freqs[0].gpufreq_lower_bound = %u\n", mt_gpufreqs[0].gpufreq_lower_bound);
        gpufreq_info("freqs[0].gpufreq_upper_bound = %u\n", mt_gpufreqs[0].gpufreq_upper_bound);
        gpufreq_info("freqs[0].gpufreq_volt = %u\n", mt_gpufreqs[0].gpufreq_volt);
        gpufreq_info("freqs[0].gpufreq_remap = %u\n", mt_gpufreqs[0].gpufreq_remap);
...
static ssize_t mt_gpufreq_clock_on_ratio_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    {
        gpufreq_err("bad argument!! please provide proper mdelay time\n");
    }
...
int mt_gpufreq_register(struct mt_gpufreq_info *freqs, int num)
{
    ...
    {
        gpufreq_info("mediatek gpufreq already registered !\n");
    }
    return 0;
}
...
static int __init mt_gpufreq_init(void)
{
    ...
    gpufreq_info("mt_gpufreq_init, clk_cfg_0 = %d\n", clk_cfg_0);
