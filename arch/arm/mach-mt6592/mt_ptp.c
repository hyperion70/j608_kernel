...
* show current PTP offset
****************************/
static int ptp_offset_read(struct seq_file *m, void *v)
{
...
static int proc_ptp_offset_open(struct inode *inode, struct file *file)
{
    return single_open(file, ptp_offset_read, NULL);
}
static const struct file_operations ptp_offset_proc_fops = {
    .owner = THIS_MODULE,
    .open  = proc_ptp_offset_open, 
    .read  = seq_read,
    .write = ptp_offset_write,
};
...
static int __init ptp_init(void)
{
    ...
    else
    {
        ...
        if (proc_create("ptp_offset", S_IRUGO | S_IWUSR | S_IWGRP, dir, &ptp_offset_proc_fops) == NULL) {
            ptp_notice("%s: create_proc_entry ptp_offset failed\n", __FUNCTION__);
            ASSERT(0);
            return -1;
        }
        #endif
