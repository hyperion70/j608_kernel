...
***************************************************/
#if defined(CONFIG_IS_VCORE_USE_6333VCORE)
...
* debug message
****************************/
#define TAG_CPUFREQ     "[Power/cpufreq] "

#define cpufreq_err(fmt, args...)       \
    pr_err(TAG_CPUFREQ KERN_CONT "[ERROR]"fmt, ##args)
#define cpufreq_warn(fmt, args...)      \
    pr_warn(TAG_CPUFREQ KERN_CONT "[WARNING]"fmt, ##args)
#define cpufreq_info(fmt, args...)      \
    pr_notice(TAG_CPUFREQ KERN_CONT fmt, ##args)
...
#define TAG_SOCFREQ     "[Power/socfreq] "

#define socfreq_err(fmt, args...)       \
    pr_err(TAG_SOCFREQ KERN_CONT "[ERROR]"fmt, ##args)
#define socfreq_warn(fmt, args...)      \
    pr_warn(TAG_SOCFREQ KERN_CONT "[WARNING]"fmt, ##args)
#define socfreq_info(fmt, args...)      \
    pr_notice(TAG_SOCFREQ KERN_CONT fmt, ##args)
#define socfreq_dbg(fmt, args...)       \
    do {                                \
        if (mt_socfreq_debug)               \
            pr_notice(TAG_SOCFREQ KERN_CONT fmt, ##args);    \
    } while (0)
#define socfreq_dbg2(fmt, args...)       \
    do {                                \
...
static unsigned int g_limited_max_freq_by_user = 0;
...
void DFS_phase_mt6333_config_interface(char dest_vol)
{
	...
	cpufreq_info("[Phase check]src_vol:0x%x\n", src_vol);
#endif
	...
	if (src_vol <= 0x10) {
		...
		src_debug_vol = 0;
		src_debug_vol_1 = 0;
		cpufreq_err("un-valid vcore:0x%x\n", src_vol);
		mt6333_read_interface_polling(0x6B, &src_debug_vol, 0x7F, 0);
		mt6333_read_interface_polling(0x6C,&src_debug_vol_1, 0x7F, 0);
		cpufreq_err("vcore by interrupt mode:0x%x\n, 0x%x\n",
				src_debug_vol, src_debug_vol_1);
		cpufreq_err(" vcore by polling mode:0x%x\n, 0x%x\n",
				src_vol, src_vol_1);
		BUG_ON(1);
	}

#if defined(DEBUG_DFS_phase_mt6333_config_interface)
	cpufreq_info("[Phase check]dst_vol:0x%x\n", dest_vol);
#endif
...
#if defined(DEBUG_DFS_phase_mt6333_config_interface)
	cpufreq_info("%d phase voltage adjust done\n", PHASE_NUMBER);
#endif
...
void mt_vcore_dvfs_disable_by_sdio(unsigned int type, bool disabled)
{
	...
	if(type == 0)
	{
	    ...
	    if(mt_socfreq_disabled_by_sdio_autoK == true)
	    {
		    ...
		    socfreq_info("mt_socfreq_disable_by_sdio, limited freq. at \n");
	    }
		socfreq_info("mt_socfreq_disable_by_sdio, mt_socfreq_disabled_by_sdio_autoK = %d\n", mt_socfreq_disabled_by_sdio_autoK);
	}
	else if(type == 1)
	{
		mt_socfreq_disabled_by_sdio_ot = disabled;
		socfreq_dbg("mt_socfreq_disable_by_sdio: mt_socfreq_disabled_by_sdio_ot = %d\n", mt_socfreq_disabled_by_sdio_ot);
	}
...
void mt_vcore_dvfs_volt_set_by_sdio(unsigned int volt)
{
	...
	socfreq_info("mt_socfreq_volt_set_by_sdio, reg = %d\n", reg);
	...
	{
		socfreq_info("mt_socfreq_volt_set_by_sdio, could not set volt now,  mt_socfreq_disabled_by_sdio_autoK = %d\n", mt_socfreq_disabled_by_sdio_autoK);
	}
#endif
}
...
static void mt_socfreq_return_default(unsigned int state)
{
	...
	if(state == 0)
	{
		...
		socfreq_dbg("mt_socfreq_return_default: cancel default \n");
	}
	else
	{
		g_cur_socfreq_keep_default_state = 1;
		socfreq_dbg("mt_socfreq_return_default: return default \n");
		...
		#endif
		{
			...
			socfreq_dbg("mt_socfreq_return_default:Set VCORE up\n");
			...
			socfreq_dbg("mt_socfreq_return_default: new_vcore_idx = %d, g_cur_cpufreq_OPPidx = %d, new_soc_idx = %d\n", new_vcore_idx, g_cur_cpufreq_OPPidx, new_soc_idx);
			...
			socfreq_dbg("mt_socfreq_return_default: Set MMPLL freq down\n");
			...
			socfreq_dbg("mt_soc_mt_socfreq_return_defaultdvfs: Set VENCPLL freq down\n");
			...
			socfreq_dbg("mt_socfreq_return_default: Set MEMPLL freq up\n");
		}
		else if(g_cur_socfreq_default_ddr_freq == DDR_DVFS_F1)
		{
			...
			socfreq_dbg("mt_socfreq_return_default: Set VENCPLL freq down\n");

			/* Set MEMPLL 266MHz */
			mt_fh_dram_overclock(266); /* MEMPLL 266MHz*/
			g_cur_socfreq_ddr_freq = DDR_DVFS_F1;
			g_soc_ddr_enable_state = 0;
			socfreq_dbg("mt_socfreq_return_default: Set MEMPLL freq down\n");
			...
			socfreq_dbg("mt_socfreq_return_default: Set VCORE down\n");
		...
		for (i = 0; i < SOC_DVFS_TYPE_NUM; i++)
        {
            ...
            socfreq_dbg("mt_socfreq_return_default: reset mt6592_soc_dvfs_config[%d] = %d\n", i, mt6592_soc_dvfs_config[i]);
...
static unsigned int mt_soc_volt_fixed_check(unsigned int enable)
{
	if(g_soc_fixed_volt_state == 1)
	{
		...
		socfreq_dbg("mt_soc_volt_fixed_check 1: ask enable = %d\n", enable);
	}
		...
		|| ((g_soc_fixed_mempll_state == 1) && (g_soc_fixed_mempll_enable == 1)))
	{
		...
		socfreq_dbg("mt_soc_volt_fixed_check 2: ask enable = %d\n", enable);
	}
...
static unsigned int mt_soc_mmpll_fixed_check(unsigned int mmpll_enable)
{
	if(g_soc_fixed_mmpll_state == 1)
	{
		...
		socfreq_dbg("mt_soc_mmpll_fixed_check: ask mmpll_enable = %d\n", mmpll_enable);
	}
...
static unsigned int mt_soc_vencpll_fixed_check(unsigned int vencpll_enable)
{
	if(g_soc_fixed_vencpll_state == 1)
	{
		...
		socfreq_dbg("mt_soc_vencpll_fixed_check: ask vencpll_enable = %d\n", vencpll_enable);
	}
...
static unsigned int mt_soc_mempll_fixed_check(unsigned int mempll_enable)
{
	if(g_soc_fixed_mempll_state == 1)
	{
		...
		socfreq_dbg("mt_soc_mempll_fixed_check: ask mempll_enable = %d\n", mempll_enable);
	}
	...
	#endif
    {
		...
		if(g_soc_DRAM_Type == LPDDR3) /* 3 */
		{
			...
			else
			{
				...
				g_cur_socfreq_OPPidx = 7;
				g_soc_volt_enable_state = 1;
				socfreq_info("mt_socfreq_ddr_detection: By soc-avs, VCORE highest 1.075V\n");
				#else

				new_soc_idx = 7;
				new_vcore_idx = new_soc_idx; // Set VCORE to 1.075V as default

				socfreq_dbg("mt_socfreq_ddr_detection: new_vcore_idx = %d, g_cur_cpufreq_OPPidx = %d, new_soc_idx = %d\n", new_vcore_idx, g_cur_cpufreq_OPPidx, new_soc_idx);
		...
		socfreq_info("DFS_Detection() < 0, g_soc_DRAM_Type = %d\n", g_soc_DRAM_Type);
	}
	else
	{
...
void mt_gpufreq_power_protect(void)
{
    ...
    for (i = 0; i < MT_CPUFREQ_POWER_LIMITED_MAX_NUM; i++)
    {
        socfreq_dbg("mt_gpufreq_power_protect: mt_cpufreq_limited_gpu_power_array[%d] = %d\n ", i, mt_cpufreq_limited_gpu_power_array[i]);
    }
    ...
    socfreq_info("limit gpu frequency upper bound to id = %d, frequency = %d, limited_power = %d\n", g_gpu_limited_max_id, mt_gpufreqs[g_gpu_limited_max_id].gpufreq_khz, limited_power);
...
{
    socfreq_info("mt_gpufreq_thermal_protect: limited_power = %d\n", limited_power);
...
{
    socfreq_info("mt_gpufreq_lbat_volt_protect: limited_power = %d\n", limited_power);
...
{
    socfreq_info("mt_gpufreq_lbat_volume_protect: limited_power = %d\n", limited_power);
...
static void mt_setup_gpufreqs_power_table(int num)
{
    ...
    {
        socfreq_err("GPU power table memory allocation fail\n");
        return;
    }
    ...
    for (i = 0; i < num; i++) {
        ...
        socfreq_info("mt_gpufreqs_power[%d].gpufreq_khz = %u\n", i, mt_gpufreqs_power[i].gpufreq_khz);
        socfreq_info("mt_gpufreqs_power[%d].gpufreq_power = %u\n", i, mt_gpufreqs_power[i].gpufreq_power);
    }
...
static unsigned int mt_get_max_vcore(unsigned int cpu_opp_idx, unsigned int soc_opp_idx)
{
	...
	if(is_ext_buck_exist() == 0)
	{
		...
		cpufreq_dbg("mt_get_max_vcore: CpuVolt =%d, SocVolt = %d\n", CpuVolt, SocVolt);
		...
		{
			cpufreq_dbg("mt_get_max_vcore: SOC could not support DVFS, VCORE could not be scaled lower than 1.125V \n");
		}
		...
		if(mt_socfreq_allowed_enable == false)
		{
			cpufreq_dbg("mt_get_max_vcore: mt_socfreq_allowed_enable = %d \n", mt_socfreq_allowed_enable);
			cpufreq_dbg("mt_get_max_vcore: SOC could not support DVFS, VCORE could not be scaled, the only fix VORE in 1.125V \n");
			return soc_opp_idx;
		}
		#endif
	    ...
	    if(CpuVolt > SocVolt)
	    {
	        cpufreq_dbg("mt_get_max_vcore: CPU VCORE > SOC VCORE \n");
			return cpu_opp_idx;
	    }
		...
		else
		{
	        ...
	        {
	            cpufreq_dbg("mt_get_max_vcore: CPU VCORE = SOC VCORE, SOC VCORE = %d \n", DVFS_VCORE_1p1);
				return soc_opp_idx;
	        }
			...
			{
	            cpufreq_err("mt_get_max_vcore: None defined case!!! soc_volt = %d\n", dvfs_vcore_config[soc_opp_idx].dvfs_vcore);
	            return soc_opp_idx;
			}
			#endif
		}
...
static unsigned int mt_soc_type_enable_check(SOC_DVFS_TYPE_ENUM type, unsigned int sochp_enable)
{
	...
	if(type == SOC_DVFS_TYPE_GPU_HP)
	{
		if((mt6592_soc_dvfs_config[type] == 0) && (sochp_enable == 0))
		{
			...
			{
				socfreq_dbg("mt_soc_type_enable_check: thermal limit gpu freq, g_gpu_limited_max_id = %d\n", g_gpu_limited_max_id);
			}
			else
			{
				if(g_gpu_limited_previous == 0)
				{
					socfreq_dbg("mt_soc_type_enable_check: type: %d, previous state: %d, sochp_enable: %d, dummy and return.\n", type, mt6592_soc_dvfs_config[type], sochp_enable);
					dummy_request = 1;
				}
				else
				{
					socfreq_dbg("mt_soc_type_enable_check: thermal unlimit gpu freq, g_gpu_limited_max_id = %d\n", g_gpu_limited_max_id);
				}
			}
		}
	}
...
static unsigned int mt_soc_opp_mask(SOC_DVFS_TYPE_ENUM type, unsigned int sochp_enable)
{
		...
		for (i = 0; i < SOC_DVFS_TYPE_NUM; i++)
        {
            socfreq_dbg2("mt_soc_opp_mask: mt6592_soc_dvfs_config[%d] = %d\n", i, mt6592_soc_dvfs_config[i]);
			Final_Mask |= mt6592_soc_dvfs_config[i];
		}

		g_soc_final_mask = Final_Mask;
		//socfreq_dbg("mt_soc_opp_mask: g_soc_final_mask = %d\n", g_soc_final_mask);
...
static unsigned int mt_soc_vencpll_opp_mask(SOC_DVFS_TYPE_ENUM type, unsigned int sochp_enable)
{
		...
		for (i = 0; i < SOC_DVFS_TYPE_NUM; i++)
        {
            //socfreq_dbg("mt_soc_vencpll_opp_mask: mt6592_soc_dvfs_config[%d] = %d\n", i, mt6592_soc_dvfs_config[i]);
			Final_Mask |= mt6592_soc_vencpll_dvfs_config[i];
		}

		g_soc_vencpll_final_mask = Final_Mask;
		//socfreq_dbg("mt_soc_vencpll_opp_mask: g_soc_vencpll_final_mask = %d\n", g_soc_vencpll_final_mask);
...
unsigned int mt_soc_vcore_freq_sub(SOC_DVFS_TYPE_ENUM type, unsigned int sochp_enable)
{
	...
	socfreq_dbg("mt_soc_dvfs: g_soc_final_mask = %d, g_soc_mmpll_final_mask = %d\n", g_soc_final_mask, g_soc_mmpll_final_mask);
	socfreq_dbg("mt_soc_dvfs: g_soc_vencpll_final_mask = %d, g_soc_mempll_final_mask = %d\n", g_soc_vencpll_final_mask, g_soc_mempll_final_mask);
	...
	if(enable == 1)
	{
		if(mt_socfreq_allowed_enable == false)
		{
			socfreq_dbg("mt_soc_dvfs:Not allow to DVS up, mt_socfreq_allowed_enable = %d\n", mt_socfreq_allowed_enable);
		}
		else
		{
			if(g_soc_volt_enable_state == 1)
			{
				socfreq_dbg("mt_soc_dvfs: volt already high, g_soc_volt_enable_state = %d\n", g_soc_volt_enable_state);
			}
			else
			{
				g_soc_volt_enable_state = 1;
				...
				socfreq_dbg("mt_soc_dvfs: new_vcore_idx = %d, g_cur_cpufreq_OPPidx = %d, new_soc_idx = %d\n", new_vcore_idx, g_cur_cpufreq_OPPidx, new_soc_idx);
				...
				if(g_soc_avs_type == 0)
				{
					...
					socfreq_dbg("mt_soc_dvfs:Set VCORE up, g_cur_vcore = %d\n", dvfs_vcore_config[new_vcore_idx].dvfs_vcore);
					//socfreq_dbg("mt_soc_dvfs: Set VCORE = %d\n", dvfs_vcore_config[new_vcore_idx].dvfs_vcore);
				}
				else
				{
					/* Scaling voltage and frequency up */
		...
		//socfreq_dbg("mt_soc_dvfs: g_cur_soc_volt = %d mv\n", g_cur_soc_volt);
	}
		...
		||(type == SOC_DVFS_TYPE_FIXED))
	{
		...
		if(g_gpu_limited_max_id != 0)
		{
			...
			else
			{
				...
				{
					socfreq_dbg("mt_soc_dvfs: No thermal limit GPU freq mach, mt_gpufreqs[%d].gpufreq_khz = %d\n", g_gpu_limited_max_id, mt_gpufreqs[g_gpu_limited_max_id].gpufreq_khz);
				}
			}
		...
		else
		{
			if(mt_socfreq_allowed_enable == false)
			{
				if(g_soc_gpu_enable_state == 2)
				{
					...
					socfreq_dbg("mt_soc_dvfs: LPDDR3 without DFS / DDR3\n");
					socfreq_dbg("mt_soc_dvfs: If thermal have limited GPU freq, when thermal unlimited, reset to high freq.\n");
				}

				socfreq_dbg("mt_soc_dvfs:Not allow to GPU DFS, mt_socfreq_allowed_enable = %d\n", mt_socfreq_allowed_enable);
			}
			else
			{
				if(mmpll_enable == 1)
				{
					...
					{
						socfreq_dbg("mt_soc_dvfs: gpu already high, g_soc_gpu_enable_state = %d\n", g_soc_gpu_enable_state);
					}
					else
					{
						if((g_soc_freq_spare0 == 0) || (g_soc_freq_spare0 == 1))
						...
						socfreq_dbg("mt_soc_dvfs: Set MMPLL freq up, g_cur_socfreq_gpu_freq = %d\n", g_cur_socfreq_gpu_freq);
					}
				}
				else
				{
					...
					{
						socfreq_dbg("mt_soc_dvfs: gpu already low, g_soc_gpu_enable_state = %d\n", g_soc_gpu_enable_state);
					}
					else
					{
						mt_dfs_mmpll(1796000); /* MMPLL 500MHz*/
						...
						socfreq_dbg("mt_soc_dvfs: Set MMPLL freq down, g_cur_socfreq_gpu_freq = %d\n", g_cur_socfreq_gpu_freq);
					}
				}
			}
		}

		//socfreq_dbg("mt_soc_dvfs: g_cur_socfreq_gpu_freq = %d\n", g_cur_socfreq_gpu_freq);
	}
		...
		||(type == SOC_DVFS_TYPE_FIXED))
	{
		if(mt_socfreq_allowed_enable == false)
		{
			socfreq_dbg("mt_soc_dvfs:Not allow to MM DFS, mt_socfreq_allowed_enable = %d\n", mt_socfreq_allowed_enable);
		}
		else
		{
			if(vencpll_enable == 1)
			{
				...
				{
					socfreq_dbg("mt_soc_dvfs: mm already high, g_soc_mm_enable_state = %d\n", g_soc_mm_enable_state);
				}
				else
				{
					mt_dfs_vencpll(1599000); /* VENCPLL 400MHz*/
					...
					socfreq_dbg("mt_soc_dvfs: Set VENCPLL freq up, g_cur_socfreq_mm_freq = %d\n", g_cur_socfreq_mm_freq);
				}
			}
			else
			{
				...
				{
					socfreq_dbg("mt_soc_dvfs: mm already low, g_soc_mm_enable_state = %d\n", g_soc_mm_enable_state);
				}
				else
				{
					mt_dfs_vencpll(1183000); /* VENCPLL 295MHz*/
					...
					socfreq_dbg("mt_soc_dvfs: Set VENCPLL freq down, g_cur_socfreq_mm_freq = %d\n", g_cur_socfreq_mm_freq);
				}
			}
		}

		//socfreq_dbg("mt_soc_dvfs: g_cur_socfreq_mm_freq = %d\n", g_cur_socfreq_mm_freq);
	}
	...
	#endif
	{
		...
		{
			socfreq_dbg("mt_soc_dvfs: This DDR not support DFS\n");
		}
		else
		{
			if(mempll_enable == 1)
			{
				...
				{
					socfreq_dbg("mt_soc_dvfs: ddr already high, g_soc_ddr_enable_state = %d\n", g_soc_ddr_enable_state);
				}
				else
				{
					#ifdef CPUFREQ_MEMPLL_OD
					...
					g_soc_ddr_enable_state = 1;
					socfreq_dbg("mt_soc_dvfs: Set MEMPLL freq up, g_cur_socfreq_ddr_freq = %d\n", g_cur_socfreq_ddr_freq);
				}
			}
			else
			{
				...
				{
					socfreq_dbg("mt_soc_dvfs: ddr already low, g_soc_ddr_enable_state = %d\n", g_soc_ddr_enable_state);
				}
				else
				{
					mt_fh_dram_overclock(266); /* MEMPLL 266MHz*/
					...
					g_cur_socfreq_ddr_freq = DDR_DVFS_F1;
					g_soc_ddr_enable_state = 0;
					socfreq_dbg("mt_soc_dvfs: Set MEMPLL freq down, g_cur_socfreq_ddr_freq = %d\n", g_cur_socfreq_ddr_freq);
				}
			}
		}

		//socfreq_dbg("mt_soc_dvfs: g_cur_socfreq_ddr_freq = %d\n", g_cur_socfreq_ddr_freq);
	}
	...
	if(enable == 0)
	{
		if(mt_socfreq_allowed_enable == false)
		{
			socfreq_dbg("mt_soc_dvfs:Not allow to DVS down, mt_socfreq_allowed_enable = %d\n", mt_socfreq_allowed_enable);
		}
		else
		{
			/* Set VCORE voltage */
			...
			{
				socfreq_dbg("mt_soc_dvfs: volt already low, g_soc_volt_enable_state = %d\n", g_soc_volt_enable_state);
			}
			else
			{
				g_soc_volt_enable_state = 0;
				...
				socfreq_dbg("mt_soc_dvfs: Set VCORE down, g_cur_vcore =  %d\n", dvfs_vcore_config[new_vcore_idx].dvfs_vcore);
				//socfreq_dbg("mt_soc_dvfs: Set VCORE down\n");
			}
		}
...
unsigned int mt_soc_dvfs(SOC_DVFS_TYPE_ENUM type, unsigned int sochp_enable)
{
	...
	socfreq_dbg("mt_soc_dvfs: type = %d, sochp_enable = %d\n", type, sochp_enable);

	if(g_cur_socfreq_keep_default_state == 1)
	{
		socfreq_dbg("mt_soc_dvfs: Return request because command of return DEFAULT setting\n");
		return 0;
	}
    ...
    *************************************************/
    ...
    {
    	socfreq_dbg("SDIO online tuning, disable soc dvfs, return!\n");
        return 0;
    }
	#endif
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					socfreq_dbg("mt_soc_dvfs:By soc-avs, VCORE 1.075V\n");
				}
				...
				socfreq_dbg("mt_soc_dvfs:Set mt6333 VCORE up, g_cur_soc_volt = %d mv\n", g_cur_soc_volt);
			}
    ...
    if(enable == 0)
    {
		...
		else
		{
			...
			else
			{
				...
				socfreq_dbg("mt_soc_dvfs: Set VCORE down, g_cur_soc_volt = %d\n", g_cur_soc_volt);
			}
		...
		//socfreq_dbg("mt_soc_dvfs: mt6333 g_cur_soc_volt = %d mv\n", g_cur_soc_volt);
	}
...
static unsigned int mt_cpufreq_turbo_mode_check_efuse(void)
{
	...
	cpufreq_info("mt_cpufreq_turbo_mode_check_efuse: on_off = %d, turbo_4_core = %d, turbo_2_core = %d\n", on_off, turbo_4_core, turbo_2_core);

	g_cpufreq_turbo_mode_efuse_on_off = on_off;
	cpufreq_info("g_cpufreq_turbo_mode_efuse_on_off = %d\n", g_cpufreq_turbo_mode_efuse_on_off);
	...
	cpufreq_info("g_cpufreq_turbo_mode_efuse_4_core = %d\n", g_cpufreq_turbo_mode_efuse_4_core);
	...
	cpufreq_info("g_cpufreq_turbo_mode_efuse_2_core = %d\n", g_cpufreq_turbo_mode_efuse_2_core);
...
unsigned int mt_cpufreq_cur_vproc(void)
{
	...
	else
	{
		...
		{
			//error
			cpufreq_warn("mt_cpufreq_cur_vproc: Not support\n");
		}
		...
		if(is_fan53555_exist() == 1)
		{
			...
			for (i = 0; i < array_size; i++)
			{
				if (vproc == mt6592_fan53555_vproc_map[i].cpufreq_vproc)
				{
					...
					found = 1;
					cpufreq_dbg("mt_cpufreq_ext_buck_vproc_mapping: vproc = %d, mt6592_fan53555_vproc_map[%d].cpufreq_ext_vproc = %d\n", vproc, i, mt6592_fan53555_vproc_map[i].cpufreq_ext_vproc);
					break;
				}
			}
			...
			{
				ext_vproc = vproc;
				cpufreq_dbg("mt_cpufreq_ext_buck_vproc_mapping: Not found correspond voltage, vproc = %d\n", vproc);
			}
		}
		else if(is_ncp6335_exist() == 1)
		{
			ext_vproc = vproc;
			cpufreq_dbg("mt_cpufreq_ext_buck_vproc_mapping: vproc = %d, ncp6335\n", vproc);
		}
		else
		{
			ext_vproc = vproc;
...
static unsigned int mt_cpufreq_limit_max_freq_by_early_suspend(void)
{
    ...
    mt_cpufreq_target(policy, DVFS_F4, CPUFREQ_RELATION_L);
...
void mt_cpufreq_downgrade_freq_check(void)
{
    ...
    //cpufreq_dbg("mt_cpufreq_downgrade_freq_check: ============================\n");
    ...
    {
        cpufreq_warn("mt_cpufreq_downgrade_freq_check: temp_0 < 0 || temp_0 > 125000\n");
        mutex_unlock(&mt_cpufreq_downgrade_freq_mutex);
        return;
    }
    ...
    if(temp_0 <= mt_cpufreq_ptpod_temperature_limit_1)
    {
        ...
        //cpufreq_dbg("mt_cpufreq_downgrade_freq_check: temp_0 <= 45, mt_cpufreq_ptpod_temperature_delta_counter = %d\n", mt_cpufreq_ptpod_temperature_delta_counter);
        //cpufreq_dbg("mt_cpufreq_downgrade_freq_check: temp_0 <= 45, mt_cpufreq_downgrade_freq_counter = %d\n", mt_cpufreq_downgrade_freq_counter);
        //cpufreq_dbg("mt_cpufreq_downgrade_freq_check: temp_0 <= 45, mt_cpufreq_downgrade_freq_counter_return = %d\n", mt_cpufreq_downgrade_freq_counter_return);

        //cpufreq_dbg("mt_cpufreq_downgrade_freq_check: temp_0 <= 45, mt_cpufreq_downgrade_freq_counter_limit = %d\n", mt_cpufreq_downgrade_freq_counter_limit);
        //cpufreq_dbg("mt_cpufreq_downgrade_freq_check: temp_0 <= 45, mt_cpufreq_downgrade_freq_counter_return_limit = %d\n", mt_cpufreq_downgrade_freq_counter_return_limit);
    ...
    if(mt_cpufreq_ptpod_temperature_delta_counter >= mt_cpufreq_ptpod_temperature_delta_counter_limit)
    {
        ...
        cpufreq_dbg("mt_cpufreq_downgrade_freq_check: temp_0 = %d, delta_temp_0 = %d\n", temp_0, delta_temp_0);
        ...
        cpufreq_dbg("mt_cpufreq_downgrade_freq_check: mt_cpufreq_downgrade_freq_counter_limit = %d\n", mt_cpufreq_downgrade_freq_counter_limit);
        cpufreq_dbg("mt_cpufreq_downgrade_freq_check: mt_cpufreq_downgrade_freq_counter_return_limit = %d\n", mt_cpufreq_downgrade_freq_counter_return_limit);
    }
    else
    {
        mt_cpufreq_ptpod_temperature_delta_counter++;
    ...
    if (num_online_cpus() >= mt_cpufreq_over_max_cpu)
    {
        if (mt_cpufreq_downgrade_freq_counter < mt_cpufreq_downgrade_freq_counter_limit)
        {
            mt_cpufreq_downgrade_freq_counter++;
            cpufreq_dbg("mt_cpufreq_downgrade_freq_check: mt_cpufreq_downgrade_freq_counter_limit = %d\n", mt_cpufreq_downgrade_freq_counter_limit);
            cpufreq_dbg("mt_cpufreq_downgrade_freq_check: mt_cpufreq_downgrade_freq_counter = %d\n", mt_cpufreq_downgrade_freq_counter);
            ...
            cpufreq_dbg("mt_cpufreq_downgrade_freq_check: freq limit, mt_cpufreq_downgrade_freq_for_ptpod = %d\n", mt_cpufreq_downgrade_freq_for_ptpod);
            ...
            if(g_cur_freq > mt_cpufreq_downgrade_freq)
            {
                ...
                mt_cpufreq_target(policy, mt_cpufreq_downgrade_freq, CPUFREQ_RELATION_L);
    ...
    cpufreq_dbg("mt_cpufreq_downgrade_freq_check: mt_cpufreq_downgrade_freq_counter = %d\n", mt_cpufreq_downgrade_freq_counter);
    cpufreq_dbg("mt_cpufreq_downgrade_freq_check: mt_cpufreq_downgrade_freq_counter_return = %d\n", mt_cpufreq_downgrade_freq_counter_return);
    cpufreq_dbg("mt_cpufreq_downgrade_freq_check: mt_cpufreq_ptpod_temperature_delta_counter = %d\n", mt_cpufreq_ptpod_temperature_delta_counter);
...
unsigned int mt_cpufreq_voltage_set_by_ptpod(unsigned int pmic_volt[], unsigned int array_size)
{
    ...
    {
        cpufreq_err("mt_cpufreq_voltage_set_by_ptpod: ERROR!array_size is invalide, array_size = %d\n", array_size);
    }
	...
	if(is_ext_buck_exist() == 0)
	{
	    ...
	    for (i = 0; i < array_size; i++)
	    {
			...
			cpufreq_dbg("mt_cpufreq_voltage_set_by_ptpod: pmic_volt[%d] = %d\n", i, pmic_volt[i]);
	        ...
	        {
	            pmic_volt[i] = 0x30;
	            cpufreq_dbg("mt_cpufreq_voltage_set_by_ptpod: PTPOD voltage < 1.0V, force pmic_volt[%d] = %d\n", i, pmic_volt[i]);
	        }
			else if(pmic_volt[i] > 0x48) /* DVFS_VCORE_1p1 = 1.15V */
	        {
	            pmic_volt[i] = 0x48; /* DVFS_VCORE_1p1 = 1.15V */
	            cpufreq_dbg("mt_cpufreq_voltage_set_by_ptpod: PTPOD voltage > 1.15V, force pmic_volt[%d] = %d\n", i, pmic_volt[i]);
	        }
			else
	        {
	            cpufreq_dbg("mt_cpufreq_voltage_set_by_ptpod: 1.0V =< PTPOD voltage =< 1.15V, pmic_volt[%d] = %d\n", i, pmic_volt[i]);
			}
	    ...
	    for (i = 0; i < (sizeof(mt_cpufreq_pmic_volt)/4); i++)
	    {
			...
			cpufreq_dbg("mt_cpufreq_spm_volt[%d] = %x\n", i, mt_cpufreq_spm_volt[i]);
	    }

		ret = mt_cpufreq_volt_set(g_cur_cpufreq_OPPidx);
		if(ret == 0)
		{
			cpufreq_err("mt_cpufreq_voltage_set_by_ptpod: I2C return fail, ret = %d\n", ret);
		}
	...
	else
	{
		...
		if(is_fan53555_exist() == 1)
		{
	    	for (i = 0; i < array_size; i++)
	    	{
				...
				cpufreq_dbg("mt_cpufreq_voltage_set_by_ptpod: fan53555 mt_cpufreq_pmic_volt[%d] = %d\n", i, mt_cpufreq_pmic_volt[i]);
	    	}
		}
		else if(is_ncp6335_exist() == 1)
		{
	    	for (i = 0; i < array_size; i++)
	    	{
				...
				cpufreq_dbg("mt_cpufreq_voltage_set_by_ptpod: ncp6335 mt_cpufreq_pmic_volt[%d] = %d\n", i, mt_cpufreq_pmic_volt[i]);
	    	}
		}
		else
		{
	    	for (i = 0; i < array_size; i++)
	    	{
				...
				cpufreq_dbg("mt_cpufreq_voltage_set_by_ptpod: not suppot, mt_cpufreq_pmic_volt[%d] = %d\n", i, mt_cpufreq_pmic_volt[i]);
	    	}
		}
...
unsigned int mt_cpufreq_max_frequency_by_DVS(unsigned int num)
{
    ...
    {
        cpufreq_info("PTPOD:num = %d, frequency= %d\n", num, mt_cpu_freqs[num].cpufreq_khz);
    ...
    cpufreq_info("PTPOD:num = %d, NOT found! return 0!\n", num);
    return 0;
}
...
int mt_socfreq_state_set(int enabled)
{
    if (enabled)
    {
        if (!mt_socfreq_pause)
        {
            socfreq_info("socfreq already enabled\n");
            return 0;
        }
        ...
        **************/
        ...
        socfreq_info("enable SOC DVFS: g_soc_dvfs_disable_count = %d\n", g_soc_dvfs_disable_count);
        ...
        ************************************************/
        ...
        {
            socfreq_info("someone still disable socfreq, cannot enable it\n");
        }
    }
    else
    {
        ...
        ***************/
        g_soc_dvfs_disable_count++;
        socfreq_info("disable SOC DVFS: g_dvfs_disable_count = %d\n", g_soc_dvfs_disable_count);

        if (mt_socfreq_pause)
        {
            socfreq_info("socfreq already disabled\n");
            return 0;
        }
	...
	if(g_cpufreq_get_ptp_level == 0)
	{
		switch (newOPPidx)
		{
			case 0:
	            if(is_ext_buck_exist()==0)
	            {
	                cpufreq_dbg("switch to DVFS_V0_ptp0: %d mV\n", DVFS_V0_ptp0);
	                spm_dvfs_ctrl_volt(0);
	            }
	            else
	            {
	                cpufreq_dbg("external buck %d : mt_cpufreq_pmic_volt[%d]: %d mV\n", pmic_external_buck_used, newOPPidx, mt_cpufreq_pmic_volt[newOPPidx]);
			...
			case 1:
	            if(is_ext_buck_exist()==0)
	            {
	            	cpufreq_dbg("switch to DVFS_V1_ptp1: %d mV\n", DVFS_V1_ptp1);
			...
			case 2:
	            if(is_ext_buck_exist()==0)
	            {
	            	cpufreq_dbg("switch to DVFS_V2_ptp2: %d mV\n", DVFS_V2_ptp2);
			...
			case 3:
	            if(is_ext_buck_exist()==0)
	            {
	            	cpufreq_dbg("switch to DVFS_V3_ptp3: %d mV\n", DVFS_V3_ptp3);
			...
			case 4:
	            if(is_ext_buck_exist()==0)
	            {
	            	cpufreq_dbg("switch to DVFS_V4_ptp4: %d mV\n", DVFS_V4_ptp4);
			...
			case 5:
	            if(is_ext_buck_exist()==0)
	            {
	            	cpufreq_dbg("switch to DVFS_V5_ptp5: %d mV\n", DVFS_V5_ptp5);
			...
			case 6:
	            if(is_ext_buck_exist()==0)
	            {
	            	cpufreq_dbg("switch to DVFS_V6_ptp6: %d mV\n", DVFS_V6_ptp6);
			...
			case 5:
	            if(is_ext_buck_exist()==0)
	            {
	            	cpufreq_dbg("switch to DVFS_V4_ptp5: %d mV\n", DVFS_V4_ptp5);
	                spm_dvfs_ctrl_volt(5);
	            }
	            else
	            {
			...
			case 6:
	            if(is_ext_buck_exist()==0)
	            {
	            	cpufreq_dbg("switch to DVFS_V4_ptp6: %d mV\n", DVFS_V4_ptp6);
	                spm_dvfs_ctrl_volt(6);
	            }
	            else
	            {
			...
			#else
			case 7:
				cpufreq_dbg("switch to DVFS_VCORE_1p1: %d mV\n", DVFS_VCORE_1p1);
	            spm_dvfs_ctrl_volt(7);
				break;
			case 8:
				if(is_ext_buck_exist() == 0)
				{
					cpufreq_dbg("switch to DVFS_VCORE_1p0: should not happened\n");
				}
				else
				{
					cpufreq_dbg("switch to DVFS_VCORE_1p0: %d\n", DVFS_V4_ptp4);
	...
	else if(g_cpufreq_get_ptp_level == 1)
	{
		switch (newOPPidx)
		{
			...
			case 1:
	            if(is_ext_buck_exist()==0)
	            {
	            	cpufreq_dbg("switch to DVFS_V1_0_ptp1: %d mV\n", DVFS_V1_0_ptp1);
	                spm_dvfs_ctrl_volt(1);
	            }
	            else
	            {
...
static int mt_cpufreq_set_initial(unsigned int freq_new, unsigned int new_cpu_idx)
{
	...
	{
		cpufreq_err("mt_cpufreq_set_initial: ERROR frequency can not < 793MHz, freq_new = %d !\n", freq_new);
		return 0;
	}
	...
	if(ret == 0)
	{
		cpufreq_err("mt_cpufreq_set_initial: I2C return fail, could not set freq up, ret = %d\n", ret);
		return 0;
	}
	udelay(PMIC_SETTLE_TIME);
...
************************************************************/
static void mt_cpufreq_set_locked(unsigned int freq_old, unsigned int freq_new, unsigned int target_volt, unsigned int new_cpu_idx)
{
	...
	cpufreq_dbg("mt_cpufreq_set_locked: freq_old = %d, freq_new = %d\n", freq_old, freq_new);
	...
	{
		cpufreq_warn("ext buck sw not ready\n");
		return;
	}
	...
	{
		cpufreq_info("Set max freq in first time!\n");
		...
		{
			cpufreq_warn("Set max freq not success!\n");
		}
		return;
	}
	...
	if (freq_new > freq_old)
	{
		...
		if(ret == 0)
		{
			cpufreq_err("mt_cpufreq_set_locked: I2C return fail, could not set volt up, ret = %d\n", ret);
			return;
		}
	    udelay(PMIC_SETTLE_TIME);
		...
		else
		{
			if(freq_new == mt_cpu_freqs[0].cpufreq_khz)
			{
				...
				cpufreq_dbg("mt_cpufreq_set_locked: [turbo mode] scaling up, freq_old = %d, freq_new = %d\n", freq_old, freq_new);
			}
		}
		#endif
	...
	else if(freq_new == freq_old)
	{
		...
		else
		{
			if(freq_new == mt_cpu_freqs[0].cpufreq_khz)
			{
				...
				{
					cpufreq_dbg("mt_cpufreq_set_locked: [turbo mode] equal, freq_new == g_cur_freq_highest = %d, return\n", g_cur_freq_highest);
					return;
				}
				...
				cpufreq_dbg("mt_cpufreq_set_locked: [turbo mode] equal, freq_old = %d, freq_new = %d\n", freq_old, freq_new);
			}
		}
	}
	#endif
	else
	{
		...
		else
		{
			if(freq_old == mt_cpu_freqs[0].cpufreq_khz)
			{
				...
				cpufreq_dbg("mt_cpufreq_set_locked: [turbo mode] scaling down, freq_old = %d, freq_new = %d\n", freq_old, freq_new);
			}
		}
		#endif
		...
		if(ret == 0)
		{
			...
			cpufreq_err("mt_cpufreq_set_locked: I2C return fail, could not set volt down, ret = %d\n", ret);
			return;
		}
	    #else
	    ret = mt_cpufreq_volt_set(new_vcore_idx);
...
static unsigned int mt_cpufreq_cpu_power_limited_verify(unsigned int target_freq)
{
	...
	cpufreq_dbg("mt_cpufreq_cpu_power_limited_verify: begin, target_freq = %d, online_cpu = %d\n", target_freq, online_cpu);
	...
	cpufreq_dbg("mt_cpufreq_cpu_power_limited_verify: index = %d, g_limited_max_ncpu = %d, g_limited_max_freq = %d\n", index, g_limited_max_ncpu, g_limited_max_freq);

    /* For MT6592 power table, ther is no 3,5,7 cpu case. */
	if((online_cpu == 3) || (online_cpu == 5) || (online_cpu == 7))
	{
		online_cpu = online_cpu + 1;
		cpufreq_dbg("mt_cpufreq_cpu_power_limited_verify: online cpu 3,5,7 not in power table, increase 1, online_cpu = %d\n", online_cpu);
	}
    ...
    for (index = i; index < (mt_cpu_freqs_num * MT_CPUFREQ_POSSIBLE_CPU); index++)
    {
        if (mt_cpu_power[index].cpufreq_ncpu == online_cpu)
        {
            ...
            {
                cpufreq_dbg("mt_cpufreq_cpu_power_limited_verify: Found freq => index = %d, target_freq = %d, online_cpu = %d\n", index, mt_cpu_power[index].cpufreq_khz, online_cpu);
	...
	if(found == 0)
	{
		...
		cpufreq_dbg("mt_cpufreq_cpu_power_limited_verify: Not found freq, set to g_limited_max_freq = %d\n", g_limited_max_freq);
	}

	cpufreq_dbg("mt_cpufreq_cpu_power_limited_verify: End, target_freq = %d, , online_cpu = %d\n", target_freq, online_cpu);
    return target_freq;
}

static int mt_cpufreq_set(struct cpufreq_policy *policy, unsigned int idx)
{
    int i, newOPPidx;
    unsigned int cpu;
    //unsigned long flags;
	...
	else
	{
		...
		else
		{
			...
			cpufreq_dbg("external buck not support\n");
		}
	}
	...
	*************************************************/
	if (mt_cpufreq_downgrade_freq_for_ptpod == true)
	{
		if(freqs.new > mt_cpufreq_downgrade_freq)
		{
			...
			cpufreq_dbg("Downgrade for PTPOD! limit freq <= %d, freqs.new = %d\n", mt_cpufreq_downgrade_freq, freqs.new);
		}
	}
	#endif
    ...
    *************************************************/
	...
	/**************************************
	* Search for max freq of user requested
	***************************************/
	if((g_limited_max_freq_by_user != 0) && (freqs.new > g_limited_max_freq_by_user)) {
		for (i = 0; i < mt_cpu_freqs_num; i++) {
			if (mt_cpu_freqs[i].cpufreq_khz <= g_limited_max_freq_by_user) {
				freqs.new = mt_cpu_freqs[i].cpufreq_khz;
				cpufreq_dbg("User limit freq! freqs.new = %d\n", freqs.new);
				break;
			}
		}
	}
    ...
    *************************************************/
    if (freqs.old == freqs.new)
    {
		...
		else
		{
			...
			{
				cpufreq_dbg("Highest CPU frequency and cpu number from %d to %d \n", g_prev_freq_target_online_cpu, g_cur_freq_target_online_cpu);
			}
			else
			{
				cpufreq_dbg("CPU frequency from %d MHz to %d MHz (skipped) due to same frequency\n", freqs.old / 1000, freqs.new / 1000);
    ...
    *******************************/
    mt_cpufreq_set_locked(freqs.old, freqs.new, next.cpufreq_volt, newOPPidx);
...
static int mt_cpufreq_target(struct cpufreq_policy *policy, unsigned int target_freq, unsigned int relation)
{
    ...
    *******************************/
    return mt_cpufreq_set(policy, idx);
}
...
void mt_cpufreq_early_suspend(struct early_suspend *h)
{
	...
	for (i = 0; i < SOC_DVFS_TYPE_NUM; i++)
	{
		socfreq_dbg2("mt_cpufreq_early_suspend: mt6592_soc_dvfs_config[%d] = %d\n", i, mt6592_soc_dvfs_config[i]);
	}
...
unsigned int mt_cpufreq_hotplug_notify(unsigned int ncpu)
{
	...
	if(g_max_freq_by_ptp < DVFS_F0)
	{
		cpufreq_dbg("mt_cpufreq_hotplug_notify: not support\n");
		return 0;
	}
	...
	cpufreq_dbg("mt_cpufreq_hotplug_notify: [turbo mode] cpu number = %d\n", ncpu);
	...
	cpufreq_dbg("mt_cpufreq_hotplug_notify: [turbo mode] prev cpu = %d, cur cpu = %d\n", g_prev_freq_target_online_cpu, g_cur_freq_target_online_cpu);
	...
	cpufreq_dbg("mt_cpufreq_hotplug_notify: [turbo mode] g_mt_cpufreq_hotplug_notify_change = %d\n", g_mt_cpufreq_hotplug_notify_change);
	cpufreq_dbg("mt_cpufreq_hotplug_notify: [turbo mode] g_cur_freq = %d\n", g_cur_freq);
	...
	else
	{
		if(g_mt_cpufreq_hotplug_notify_change == 1)
		{
			if(g_cur_freq == mt_cpu_freqs[0].cpufreq_khz)
			{
				...
				cpufreq_dbg("mt_cpufreq_hotplug_notify: [turbo mode] call mt_cpufreq_target()\n");

			    mt_cpufreq_target(policy, mt_cpu_freqs[0].cpufreq_khz, CPUFREQ_RELATION_L);
...
void mt_cpufreq_cpu_power_protect(void)
{
    ...
    for (i = 0; i < MT_CPUFREQ_POWER_LIMITED_MAX_NUM; i++)
    {
        cpufreq_dbg("mt_cpufreq_cpu_power_protect: mt_cpufreq_limited_cpu_power_array[%d] = %d\n ", i, mt_cpufreq_limited_cpu_power_array[i]);
    }
    ...
    if (limited_power == 0)
    {
        ...
        mt_cpufreq_target(policy, g_limited_max_freq, CPUFREQ_RELATION_L);
        #ifdef CONFIG_CPU_FREQ_GOV_HOTPLUG
        hp_limited_cpu_num(g_limited_max_ncpu);
        #endif

        cpufreq_info("thermal limit g_limited_max_freq = %d, g_limited_max_ncpu = %d, limited_power = %d\n", g_limited_max_freq, g_limited_max_ncpu, limited_power);
    }
    else
    {
        while (ncpu)
...
{
    cpufreq_info("mt_cpufreq_thermal_protect: limited_power = %d\n", limited_power);
...
{
    cpufreq_info("mt_cpufreq_lbat_volt_protect: low_battery_level = %d\n", low_battery_level);
...
int mt_cpufreq_lbat_volume_protect(void)
{
    ...
    //cpufreq_dbg("mt_cpufreq_lbat_volume_protect: bat_ui_percentage=%d, LOW_POWER_LIMIT_LEVEL_1 = %d\n", bat_ui_percentage, LOW_POWER_LIMIT_LEVEL_1);
...
* show SOC DVFS info
****************************/
static int mt_socfreq_dump_info_read(struct seq_file *m, void *v)
{
    int i = 0;

    for (i = 0; i < (SOC_DVFS_TYPE_NUM); i++)
    {
        seq_printf(m, "mt6592_soc_dvfs_config[%d] = %d\n", i, mt6592_soc_dvfs_config[i]);
    }

    seq_printf(m, "mt_socfreq_pause = %d\n", mt_socfreq_pause);
	seq_printf(m, "g_cur_socfreq_keep_default_state = %d\n", g_cur_socfreq_keep_default_state);
	seq_printf(m, "g_soc_api_call_test = %d\n", g_soc_api_call_test);
	seq_printf(m, "mt_num_possible_cpus = %d\n", mt_num_possible_cpus);
	seq_printf(m, "g_gpu_limited_max_id = %d\n", g_gpu_limited_max_id);
	seq_printf(m, "g_soc_final_mask = %d\n", g_soc_final_mask);
	seq_printf(m, "g_soc_mmpll_final_mask = %d\n", g_soc_mmpll_final_mask);
	seq_printf(m, "g_soc_vencpll_final_mask = %d\n", g_soc_vencpll_final_mask);
	seq_printf(m, "g_soc_mempll_final_mask = %d\n", g_soc_mempll_final_mask);
	seq_printf(m, "g_soc_DRAM_Type = %d\n", g_soc_DRAM_Type);
	seq_printf(m, "g_soc_avs_type = %d\n", g_soc_avs_type);
	seq_printf(m, "g_soc_freq_spare0 = %d\n", g_soc_freq_spare0);
	#ifdef CPUFREQ_MEMPLL_OD
	seq_printf(m, "g_ddr_frequency_type = %d\n", g_ddr_frequency_type);
	seq_printf(m, "g_default_soc_volt = %d\n", g_default_soc_volt);
	seq_printf(m, "g_cur_socfreq_default_ddr_freq = %d\n", g_cur_socfreq_default_ddr_freq);
	#endif
	seq_printf(m, "mt_socfreq_allowed_enable = %d\n", mt_socfreq_allowed_enable);
	seq_printf(m, "mt_socfreq_ddr_allowed_enable = %d\n", mt_socfreq_ddr_allowed_enable);
	seq_printf(m, "g_cur_soc_volt = %d\n", g_cur_soc_volt);
	seq_printf(m, "g_cur_socfreq_gpu_freq = %d\n", g_cur_socfreq_gpu_freq);
	seq_printf(m, "g_cur_socfreq_mm_freq = %d\n", g_cur_socfreq_mm_freq);
	seq_printf(m, "g_cur_socfreq_ddr_freq = %d\n", g_cur_socfreq_ddr_freq);
...
****************************/
static int mt_socfreq_state_read(struct seq_file *m, void *v)
{
    if (!mt_socfreq_pause)
        seq_printf(m, "SOC DVFS enabled\n");
    else
        seq_printf(m, "SOC DVFS disabled\n");
...
*************************************/
static ssize_t mt_socfreq_state_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int enabled = 0;
	char desc[32];
	int len = 0;
    ...
    if (sscanf(desc, "%d", &enabled) == 1)
    {
        ...
        {
            socfreq_err("bad argument!! argument should be \"1\" or \"0\"\n");
        }
    }
    else
    {
...
* show current SOC DVFS stauts
****************************/
static int mt_socfreq_enable_hp_read(struct seq_file *m, void *v)
{
    if (!g_soc_gpu_enable_state)
        seq_printf(m, "SOC DVFS high performance set\n");
    else
        seq_printf(m, "SOC DVFS high performance not set\n");
...
* set SOC DVFS stauts by sysfs interface
*************************************/
static ssize_t mt_socfreq_enable_hp_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int enabled = 0;
	char desc[32];
	int len = 0;
...
*************************************/
static int mt_socfreq_fixed_test_read(struct seq_file *m, void *v)
{
	seq_printf(m, "g_soc_fixed_volt_state = %d, g_soc_fixed_volt_enable = %d\n", g_soc_fixed_volt_state, g_soc_fixed_volt_enable);
	seq_printf(m, "g_soc_fixed_mmpll_state = %d, g_soc_fixed_mmpll_enable = %d\n", g_soc_fixed_mmpll_state, g_soc_fixed_mmpll_enable);
	seq_printf(m, "g_soc_fixed_vencpll_state = %d, g_soc_fixed_vencpll_enable = %d\n", g_soc_fixed_vencpll_state, g_soc_fixed_vencpll_enable);
	seq_printf(m, "g_soc_fixed_mempll_state = %d, g_soc_fixed_mempll_enable = %d\n", g_soc_fixed_mempll_state, g_soc_fixed_mempll_enable);

	return 0;
}

static ssize_t mt_socfreq_fixed_test_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d %d %d", &pll_volt, &state, &enable) == 3)
    {
...
*****************************/
static int mt_socfreq_limited_power_read(struct seq_file *m, void *v)
{
    seq_printf(m, "g_gpu_limited_max_id = %d\n", g_gpu_limited_max_id);
...
***********************************/
static ssize_t mt_socfreq_limited_power_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    {
        socfreq_err("bad argument!! please provide the maximum limited power\n");
    }
...
****************************/
static int mt_socfreq_debug_read(struct seq_file *m, void *v)
{
    if (mt_socfreq_debug)
        seq_printf(m, "socfreq debug enabled\n");
    else
        seq_printf(m, "socfreq debug disabled\n");
...
************************/
static ssize_t mt_socfreq_debug_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int debug = 0;
	char desc[32];
	int len = 0;
    ...
    if (sscanf(desc, "%d", &debug) == 1)
    {
        ...
        {
            socfreq_err("bad argument!! should be 0 or 1 [0: disable, 1: enable]\n");
        }
    }
    else
    {
...
* show gpu power info
****************************/
static int mt_socfreq_gpu_power_dump_read(struct seq_file *m, void *v)
{
    int i = 0;
...
****************************/
static int mt_socfreq_voltage_read(struct seq_file *m, void *v)
{
	seq_printf(m, "socfreq_voltage = 0x%x\n", mt6333_get_reg_value(0x6C));
...
*************************************/
static ssize_t mt_cpufreq_state_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int enabled = 0;
	char desc[32];
	int len = 0;
...
* limited power for thermal protect
***********************************/
static ssize_t mt_cpufreq_limited_power_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    unsigned int power = 0;
	char desc[32];
	int len = 0;
...
{
    seq_printf(m, "g_limited_load_for_cpu_power_test = %d\n", g_limited_load_for_cpu_power_test);
    seq_printf(m, "mt_cpufreq_thermal_limited_cpu_power = %d\n", mt_cpufreq_thermal_limited_cpu_power);
    seq_printf(m, "mt_cpufreq_lbat_volt_limited_cpu_power = %d\n", mt_cpufreq_lbat_volt_limited_cpu_power);
    seq_printf(m, "mt_cpufreq_lbat_volt_limited_gpu_power = %d\n", mt_cpufreq_lbat_volt_limited_gpu_power);
    seq_printf(m, "g_limited_max_cpu_power = %d\n", g_limited_max_cpu_power);
...
***********************************/
static ssize_t mt_cpufreq_limited_load_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    unsigned int load = 0;
	char desc[32];
	int len = 0;
...
static int mt_cpufreq_debug_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "g_cpufreq_get_ptp_level = %d\n", g_cpufreq_get_ptp_level);
	seq_printf(m, "pmic_external_buck_used = %d\n", pmic_external_buck_used);
...
************************/
static ssize_t mt_cpufreq_debug_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int debug = 0;
	char desc[32];
	int len = 0;
...
static int mt_cpufreq_power_dump_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "g_cpu_power_table_num = %d\n", g_cpu_power_table_num);
...
* PTPOD test
************************/
static int mt_cpufreq_ptpod_test_read(struct seq_file *m, void *v)
{
	return 0;
}

static ssize_t mt_cpufreq_ptpod_test_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int enable = 0;
	char desc[32];
	int len = 0;
...
************************/
static int mt_cpufreq_fftt_test_read(struct seq_file *m, void *v)
{
	return 0;
}

static ssize_t mt_cpufreq_fftt_test_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int enable = 0;
	char desc[32];
	int len = 0;
    ...
    if (sscanf(desc, "%d", &enable) == 1)
    {
        ...
        {
            cpufreq_err("bad argument!! should be 0 or 1 or 2\n");
        }
    }
    else
    {
...
***********************************/
static ssize_t mt_cpufreq_limited_by_hevc_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	struct cpufreq_policy *policy;
	unsigned int freq = 0;
	char desc[32];
	...
	if (sscanf(desc, "%u", &freq) == 1) {
		...
		if (policy) {
			if (policy->cur < g_limited_freq_by_hevc)
				mt_cpufreq_target(policy, g_limited_freq_by_hevc, CPUFREQ_RELATION_L);
			cpufreq_cpu_put(policy);
		}
...
/**********************************
* limited freq for user request
***********************************/
static ssize_t mt_cpufreq_limited_max_freq_by_user_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	struct cpufreq_policy *policy;
	unsigned int freq = 0;
	char desc[32];
	...
	if (sscanf(desc, "%u", &freq) == 1) {
		g_limited_max_freq_by_user = freq;
		cpufreq_info("limited_max_freq_by_user! g_limited_max_freq_by_user = %d\n", g_limited_max_freq_by_user);

		policy = cpufreq_cpu_get(0);
		if (policy) {
			if (g_limited_max_freq_by_user != 0 && policy->cur > g_limited_max_freq_by_user)
				mt_cpufreq_target(policy, g_limited_max_freq_by_user, CPUFREQ_RELATION_H);
			cpufreq_cpu_put(policy);
		}
		...
		cpufreq_err("bad argument!! please provide the max limit\n");

	return -EINVAL;
}

static int mt_cpufreq_limited_max_freq_by_user_read(struct seq_file *m, void *v)
{
	seq_printf(m, "g_limited_max_freq_by_user = %d\n", g_limited_max_freq_by_user);

	return 0;
}

static int mt_cpufreq_ptpod_freq_volt_read(struct seq_file *m, void *v)
{
	int i = 0;

	for (i = 0; i < mt_cpu_freqs_num; i++)
	{
		seq_printf(m, "default freq = %d, volt = %d\n", mt_cpu_freqs[i].cpufreq_khz, mt_cpu_freqs[i].cpufreq_volt);
	}
...
static int mt_cpufreq_downgrade_freq_info_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_downgrade_freq_step_idx_1 = %d\n", mt_cpufreq_downgrade_freq_step_idx_1);
	seq_printf(m, "mt_cpufreq_downgrade_freq_step_idx_2 = %d\n", mt_cpufreq_downgrade_freq_step_idx_2);
	seq_printf(m, "mt_cpufreq_downgrade_freq_step_idx_limit = %d\n", mt_cpufreq_downgrade_freq_step_idx_limit);

	seq_printf(m, "mt_cpufreq_ptpod_temperature_limit_1 = %d\n", mt_cpufreq_ptpod_temperature_limit_1);
	seq_printf(m, "mt_cpufreq_ptpod_temperature_limit_2 = %d\n", mt_cpufreq_ptpod_temperature_limit_2);
	seq_printf(m, "mt_cpufreq_ptpod_temperature_delta_1 = %d\n", mt_cpufreq_ptpod_temperature_delta_1);
	seq_printf(m, "mt_cpufreq_ptpod_temperature_delta_1_ratio_1 = %d\n", mt_cpufreq_ptpod_temperature_delta_1_ratio_1);
	seq_printf(m, "mt_cpufreq_ptpod_temperature_delta_2_ratio_1 = %d\n", mt_cpufreq_ptpod_temperature_delta_2_ratio_1);
	seq_printf(m, "mt_cpufreq_ptpod_temperature_delta_1_ratio_2 = %d\n", mt_cpufreq_ptpod_temperature_delta_1_ratio_2);
	seq_printf(m, "mt_cpufreq_ptpod_temperature_delta_2_ratio_2 = %d\n", mt_cpufreq_ptpod_temperature_delta_2_ratio_2);
	seq_printf(m, "mt_cpufreq_ptpod_temperature_delta_counter_limit = %d\n", mt_cpufreq_ptpod_temperature_delta_counter_limit);
	seq_printf(m, "mt_cpufreq_downgrade_freq_counter_limit = %d\n", mt_cpufreq_downgrade_freq_counter_limit);
	seq_printf(m, "mt_cpufreq_downgrade_freq_counter_return_limit = %d\n", mt_cpufreq_ptpod_temperature_delta_counter_limit - mt_cpufreq_downgrade_freq_counter_limit);
	seq_printf(m, "mt_cpufreq_over_max_cpu = %d\n", mt_cpufreq_over_max_cpu);

    return 0;
}

static int mt_cpufreq_downgrade_freq_step_idx_read(struct seq_file *m, void *v)
{
    return 0;
}

static ssize_t mt_cpufreq_downgrade_freq_step_idx_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d %d", &step_idx_1, &step_idx_2) == 2)
    {
    ...
    {
        cpufreq_err("bad argument!! argument should be oC\n");
    }

	return count;
}

static int mt_cpufreq_ptpod_temperature_limit_read(struct seq_file *m, void *v)
{
    return 0;
}

static ssize_t mt_cpufreq_ptpod_temperature_limit_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d %d", &dwn_temp_1, &dwn_temp_2) == 2)
    {
...
static int mt_cpufreq_ptpod_temperature_delta_read(struct seq_file *m, void *v)
{
    return 0;
}

static ssize_t mt_cpufreq_ptpod_temperature_delta_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int delta_1 = 0;
	char desc[32];
	int len = 0;
...
static int mt_cpufreq_ptpod_temperature_delta_ratio_read(struct seq_file *m, void *v)
{
    return 0;
}

static ssize_t mt_cpufreq_ptpod_temperature_delta_ratio_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d %d %d %d %d", &delta_count, &ratio_1, &ratio_2, &ratio_3, &ratio_4) == 5)
    {
...
static int mt_cpufreq_over_max_cpu_read(struct seq_file *m, void *v)
{
    return 0;
}

static ssize_t mt_cpufreq_over_max_cpu_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int max_cpu = 0;
	char desc[32];
	int len = 0;
...
static int mt_cpufreq_downgrade_freq_enable_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_downgrade_freq_for_ptpod_enable = %d\n", mt_cpufreq_downgrade_freq_for_ptpod_enable);

    return 0;
}

static ssize_t mt_cpufreq_downgrade_freq_enable_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int on_off = 0;
	char desc[32];
	int len = 0;
...
static int mt_cpufreq_lbat_volt_limited_cpu_power_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_lbat_volt_limited_cpu_power_0 = %d\n", mt_cpufreq_lbat_volt_limited_cpu_power_0);
	seq_printf(m, "mt_cpufreq_lbat_volt_limited_cpu_power_1 = %d\n", mt_cpufreq_lbat_volt_limited_cpu_power_1);
	seq_printf(m, "mt_cpufreq_lbat_volt_limited_cpu_power_2 = %d\n", mt_cpufreq_lbat_volt_limited_cpu_power_2);

    return 0;
}

static ssize_t mt_cpufreq_lbat_volt_limited_cpu_power_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d %d %d", &lbat_power_0, &lbat_power_1, &lbat_power_2) == 3)
    {
...
static int mt_cpufreq_lbat_volt_limited_gpu_power_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_lbat_volt_limited_gpu_power_0 = %d\n", mt_cpufreq_lbat_volt_limited_gpu_power_0);
	seq_printf(m, "mt_cpufreq_lbat_volt_limited_gpu_power_1 = %d\n", mt_cpufreq_lbat_volt_limited_gpu_power_1);
	seq_printf(m, "mt_cpufreq_lbat_volt_limited_gpu_power_2 = %d\n", mt_cpufreq_lbat_volt_limited_gpu_power_2);

    return 0;
}

static ssize_t mt_cpufreq_lbat_volt_limited_gpu_power_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int lbat_power_0 = 0, lbat_power_1 = 0, lbat_power_2 = 0;
	char desc[32];
	int len = 0;
...
static int mt_cpufreq_lbat_volume_limited_cpu_power_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_lbat_volume_limited_cpu_power_0 = %d\n", mt_cpufreq_lbat_volume_limited_cpu_power_0);
	seq_printf(m, "mt_cpufreq_lbat_volume_limited_cpu_power_1 = %d\n", mt_cpufreq_lbat_volume_limited_cpu_power_1);

    return 0;
}

static ssize_t mt_cpufreq_lbat_volume_limited_cpu_power_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d %d", &lbat_power_0, &lbat_power_1) == 2)
    {
...
static int mt_cpufreq_lbat_volume_limited_gpu_power_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_lbat_volume_limited_gpu_power_0 = %d\n", mt_cpufreq_lbat_volume_limited_gpu_power_0);
	seq_printf(m, "mt_cpufreq_lbat_volume_limited_gpu_power_1 = %d\n", mt_cpufreq_lbat_volume_limited_gpu_power_1);

    return 0;
}

static ssize_t mt_cpufreq_lbat_volume_limited_gpu_power_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int lbat_power_0 = 0, lbat_power_1 = 0;
	char desc[32];
	int len = 0;
...
static int mt_cpufreq_lbat_volt_limited_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_lbat_volt_limited_level = %d\n", mt_cpufreq_lbat_volt_limited_level);

    return 0;
}

static ssize_t mt_cpufreq_lbat_volt_limited_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d", &lbat_volt_level) == 1)
    {
...
static int mt_cpufreq_lbat_volt_drop_enable_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_lbat_volt_drop_enable = %d\n", mt_cpufreq_lbat_volt_drop_enable);

    return 0;
}

static ssize_t mt_cpufreq_lbat_volt_drop_enable_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int on_off = 0;
	char desc[32];
	int len = 0;
...
static int mt_cpufreq_lbat_volume_enable_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_lbat_volume_enable = %d\n", mt_cpufreq_lbat_volume_enable);

    return 0;
}

static ssize_t mt_cpufreq_lbat_volume_enable_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int enable = 0;
	char desc[32];
	int len = 0;
...
#ifdef CPUFREQ_HIGHEST_TURBO_MODE
static int mt_cpufreq_highest_turbo_mode_read(struct seq_file *m, void *v)
{
	seq_printf(m, "g_cpufreq_turbo_mode_efuse_on_off = %d\n", g_cpufreq_turbo_mode_efuse_on_off);
	seq_printf(m, "g_cpufreq_turbo_mode_efuse_4_core = %d\n", g_cpufreq_turbo_mode_efuse_4_core);
	seq_printf(m, "g_cpufreq_turbo_mode_efuse_2_core = %d\n", g_cpufreq_turbo_mode_efuse_2_core);

    return 0;
}

static ssize_t mt_cpufreq_highest_turbo_mode_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d %d %d", &on_off, &freq_4core, &freq_2core) == 3)
    {
        g_cpufreq_turbo_mode_efuse_on_off = on_off;
...
static int mt_cpufreq_total_limited_cpu_power_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_limited_cpu_power_array[0] = %d\n", mt_cpufreq_limited_cpu_power_array[0]);
	seq_printf(m, "mt_cpufreq_limited_cpu_power_array[1] = %d\n", mt_cpufreq_limited_cpu_power_array[1]);
	seq_printf(m, "mt_cpufreq_limited_cpu_power_array[2] = %d\n", mt_cpufreq_limited_cpu_power_array[2]);

    return 0;
}

static ssize_t mt_cpufreq_total_limited_cpu_power_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d %d %d", &limit_0, &limit_1, &limit_2) == 3)
    {
...
static int mt_cpufreq_total_limited_gpu_power_read(struct seq_file *m, void *v)
{
	seq_printf(m, "mt_cpufreq_limited_gpu_power_array[0] = %d\n", mt_cpufreq_limited_gpu_power_array[0]);
	seq_printf(m, "mt_cpufreq_limited_gpu_power_array[1] = %d\n", mt_cpufreq_limited_gpu_power_array[1]);
	seq_printf(m, "mt_cpufreq_limited_gpu_power_array[2] = %d\n", mt_cpufreq_limited_gpu_power_array[2]);

    return 0;
}

static ssize_t mt_cpufreq_total_limited_gpu_power_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    int limit_0 = 0, limit_1 = 0, limit_2 = 0;
	char desc[32];
	int len = 0;
...
static int mt_socfreq_state_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_socfreq_state_read, NULL);
}

static const struct file_operations mt_socfreq_state_fops = {
    .owner = THIS_MODULE,
    .open = mt_socfreq_state_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_socfreq_state_write,
    .release = single_release,
};

static int mt_socfreq_dump_info_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_socfreq_dump_info_read, NULL);
}

static const struct file_operations mt_socfreq_dump_info_fops = {
    .owner = THIS_MODULE,
    .open = mt_socfreq_dump_info_open,
    .read = seq_read,
};

#if defined(CONFIG_THERMAL_LIMIT_TEST)
static int mt_cpufreq_limited_load_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_limited_load_read, NULL);
}

static const struct file_operations mt_cpufreq_limited_load_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_limited_load_open,
    .read = seq_read,
    .llseek = seq_lseek,
...
#ifdef MT_DVFS_PTPOD_TEST
static int mt_cpufreq_ptpod_test_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_ptpod_test_read, NULL);
}

static const struct file_operations mt_cpufreq_ptpod_test_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_ptpod_test_open,
    .read = seq_read,
    .llseek = seq_lseek,
...
#ifdef MT_DVFS_FFTT_TEST
static int mt_cpufreq_fftt_test_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_fftt_test_read, NULL);
}

static const struct file_operations mt_cpufreq_fftt_test_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_fftt_test_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_fftt_test_write,
    .release = single_release,
};
#endif

static int mt_cpufreq_limited_by_hevc_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_limited_by_hevc_read, NULL);
}

static const struct file_operations mt_cpufreq_limited_by_hevc_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_limited_by_hevc_open,
    .read = seq_read,
    .llseek = seq_lseek,
...
static int mt_cpufreq_limited_max_freq_by_user_open(struct inode *inode, struct file *file)
{
	return single_open(file, mt_cpufreq_limited_max_freq_by_user_read, NULL);
}

static const struct file_operations mt_cpufreq_limited_max_freq_by_user_fops = {
	.owner = THIS_MODULE,
	.open = mt_cpufreq_limited_max_freq_by_user_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = mt_cpufreq_limited_max_freq_by_user_write,
	.release = single_release,
};

static int mt_cpufreq_ptpod_freq_volt_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_ptpod_freq_volt_read, NULL);
}
...
static int mt_socfreq_limited_power_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_socfreq_limited_power_read, NULL);
}

static const struct file_operations mt_socfreq_limited_power_fops = {
    .owner = THIS_MODULE,
    .open = mt_socfreq_limited_power_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_socfreq_limited_power_write,
    .release = single_release,
};

static int mt_socfreq_enable_hp_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_socfreq_enable_hp_read, NULL);
}

static const struct file_operations mt_socfreq_enable_hp_fops = {
    .owner = THIS_MODULE,
    .open = mt_socfreq_enable_hp_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_socfreq_enable_hp_write,
    .release = single_release,
};

static int mt_socfreq_gpu_power_dump_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_socfreq_gpu_power_dump_read, NULL);
}

static const struct file_operations mt_socfreq_gpu_power_dump_fops = {
    .owner = THIS_MODULE,
    .open = mt_socfreq_gpu_power_dump_open,
    .read = seq_read,
};

static int mt_socfreq_debug_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_socfreq_debug_read, NULL);
}

static const struct file_operations mt_socfreq_debug_fops = {
    .owner = THIS_MODULE,
    .open = mt_socfreq_debug_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_socfreq_debug_write,
    .release = single_release,
};

#ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
static int mt_socfreq_voltage_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_socfreq_voltage_read, NULL);
}

static const struct file_operations mt_socfreq_voltage_fops = {
    .owner = THIS_MODULE,
    .open = mt_socfreq_voltage_open,
    .read = seq_read,
};
#endif

static int mt_socfreq_fixed_test_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_socfreq_fixed_test_read, NULL);
}

static const struct file_operations mt_socfreq_fixed_test_fops = {
    .owner = THIS_MODULE,
    .open = mt_socfreq_fixed_test_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_socfreq_fixed_test_write,
    .release = single_release,
};

#ifdef PTPOD_DOWNGRADE_FREQ
static int mt_cpufreq_downgrade_freq_info_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_downgrade_freq_info_read, NULL);
}

static const struct file_operations mt_cpufreq_downgrade_freq_info_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_downgrade_freq_info_open,
    .read = seq_read,
};

static int mt_cpufreq_downgrade_freq_step_idx_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_downgrade_freq_step_idx_read, NULL);
}

static const struct file_operations mt_cpufreq_downgrade_freq_step_idx_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_downgrade_freq_step_idx_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_downgrade_freq_step_idx_write,
    .release = single_release,
};

static int mt_cpufreq_ptpod_temperature_limit_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_ptpod_temperature_limit_read, NULL);
}

static const struct file_operations mt_cpufreq_ptpod_temperature_limit_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_ptpod_temperature_limit_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_ptpod_temperature_limit_write,
    .release = single_release,
};

static int mt_cpufreq_ptpod_temperature_delta_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_ptpod_temperature_delta_read, NULL);
}

static const struct file_operations mt_cpufreq_ptpod_temperature_delta_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_ptpod_temperature_delta_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_ptpod_temperature_delta_write,
    .release = single_release,
};

static int mt_cpufreq_ptpod_temperature_delta_ratio_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_ptpod_temperature_delta_ratio_read, NULL);
}

static const struct file_operations mt_cpufreq_ptpod_temperature_delta_ratio_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_ptpod_temperature_delta_ratio_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_ptpod_temperature_delta_ratio_write,
    .release = single_release,
};

static int mt_cpufreq_over_max_cpu_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_over_max_cpu_read, NULL);
}

static const struct file_operations mt_cpufreq_over_max_cpu_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_over_max_cpu_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_over_max_cpu_write,
    .release = single_release,
};

static int mt_cpufreq_downgrade_freq_enable_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_downgrade_freq_enable_read, NULL);
}

static const struct file_operations mt_cpufreq_downgrade_freq_enable_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_downgrade_freq_enable_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_downgrade_freq_enable_write,
    .release = single_release,
};

#endif

static int mt_cpufreq_lbat_volt_limited_cpu_power_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_lbat_volt_limited_cpu_power_read, NULL);
}

static const struct file_operations mt_cpufreq_lbat_volt_limited_cpu_power_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_lbat_volt_limited_cpu_power_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_lbat_volt_limited_cpu_power_write,
    .release = single_release,
};

static int mt_cpufreq_lbat_volume_limited_cpu_power_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_lbat_volume_limited_cpu_power_read, NULL);
}

static const struct file_operations mt_cpufreq_lbat_volume_limited_cpu_power_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_lbat_volume_limited_cpu_power_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_lbat_volume_limited_cpu_power_write,
    .release = single_release,
};

static int mt_cpufreq_lbat_volt_limited_gpu_power_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_lbat_volt_limited_gpu_power_read, NULL);
}

static const struct file_operations mt_cpufreq_lbat_volt_limited_gpu_power_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_lbat_volt_limited_gpu_power_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_lbat_volt_limited_gpu_power_write,
    .release = single_release,
};

static int mt_cpufreq_lbat_volume_limited_gpu_power_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_lbat_volume_limited_gpu_power_read, NULL);
}

static const struct file_operations mt_cpufreq_lbat_volume_limited_gpu_power_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_lbat_volume_limited_gpu_power_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_lbat_volume_limited_gpu_power_write,
    .release = single_release,
};

static int mt_cpufreq_lbat_volt_limited_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_lbat_volt_limited_read, NULL);
}

static const struct file_operations mt_cpufreq_lbat_volt_limited_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_lbat_volt_limited_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_lbat_volt_limited_write,
    .release = single_release,
};

static int mt_cpufreq_lbat_volt_drop_enable_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_lbat_volt_drop_enable_read, NULL);
}

static const struct file_operations mt_cpufreq_lbat_volt_drop_enable_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_lbat_volt_drop_enable_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_lbat_volt_drop_enable_write,
    .release = single_release,
};

static int mt_cpufreq_lbat_volume_enable_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_lbat_volume_enable_read, NULL);
}

static const struct file_operations mt_cpufreq_lbat_volume_enable_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_lbat_volume_enable_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_lbat_volume_enable_write,
    .release = single_release,
};

#ifdef CPUFREQ_HIGHEST_TURBO_MODE
static int mt_cpufreq_highest_turbo_mode_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_highest_turbo_mode_read, NULL);
}

static const struct file_operations mt_cpufreq_highest_turbo_mode_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_highest_turbo_mode_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_highest_turbo_mode_write,
    .release = single_release,
};
#endif

static int mt_cpufreq_total_limited_cpu_power_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_total_limited_cpu_power_read, NULL);
}

static const struct file_operations mt_cpufreq_total_limited_cpu_power_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_total_limited_cpu_power_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_total_limited_cpu_power_write,
    .release = single_release,
};

static int mt_cpufreq_total_limited_gpu_power_open(struct inode *inode, struct file *file)
{
    return single_open(file, mt_cpufreq_total_limited_gpu_power_read, NULL);
}

static const struct file_operations mt_cpufreq_total_limited_gpu_power_fops = {
    .owner = THIS_MODULE,
    .open = mt_cpufreq_total_limited_gpu_power_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mt_cpufreq_total_limited_gpu_power_write,
    .release = single_release,
};
...
static int mt_cpufreq_pdrv_probe(struct platform_device *pdev)
{
    ...
    {
        cpufreq_info("ext buck sw not ready\n");
    }
    ...
    *************************************************/
	...
	else
	{
		...
		cpufreq_err("mt_cpufreq_pdrv_probe: mt_cpufreq_set_initial return not success\n");
		cpufreq_err("mt_cpufreq_pdrv_probe: keep to set to max freq in mt_cpufreq_set()\n");
	}
	#endif
	...
	*/
	...
	{
		cpufreq_err("mt_cpufreq_pdrv_probe: temp < 0 || temp > 125000\n");
...
static unsigned int mt_cpufreq_dvfs_get_cpu_freq(void)
{
    ...
    {
    	cpufreq_err("Frequency is out of range, return max freq.\n");
		return g_max_freq_by_ptp;
    }

	cpufreq_info("mt_cpufreq_dvfs_get_cpu_freq: ARMPLL_CON1 = 0x%x\n", DRV_Reg32(ARMPLL_CON1));
...
static int mt_cpufreq_pm_restore_early(struct device *dev)
{
	...
	for (i = 0; i < mt_cpu_freqs_num; i++)
    {
        if (g_cur_freq == mt_cpu_freqs[i].cpufreq_khz)
        {
			...
			cpufreq_info("match g_cur_cpufreq_OPPidx: %d\n", g_cur_cpufreq_OPPidx);
            break;
        }
    }
	...
	if (g_cur_freq > mt_cpu_freqs[0].cpufreq_khz)
	{
		...
		cpufreq_info("match, turob mode, g_cur_cpufreq_OPPidx: %d\n", g_cur_cpufreq_OPPidx);
	}
	#endif

	cpufreq_info( "CPU freq SW/HW: %d/%d\n", g_cur_freq, mt_cpufreq_dvfs_get_cpu_freq());
	cpufreq_info("g_cur_cpufreq_OPPidx: %d\n", g_cur_cpufreq_OPPidx);
...
static int __init mt_cpufreq_pdrv_init(void)
{
    ...
    else
    {
		...
		mt_entry = proc_create("cpufreq_debug", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_debug_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_limited_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_limited_power_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_state", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_state_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_power_dump", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_power_dump_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("socfreq_state", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_socfreq_state_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("socfreq_dump_info", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_socfreq_dump_info_fops);
		if (mt_entry)
		{
		}

		#if defined(CONFIG_THERMAL_LIMIT_TEST)
		mt_entry = proc_create("cpufreq_limited_load", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_limited_load_fops);
		if (mt_entry)
		{
		}
		#endif
		...
		#ifdef MT_DVFS_PTPOD_TEST
		mt_entry = proc_create("cpufreq_ptpod_test", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_ptpod_test_fops);
		if (mt_entry)
		{
		}
		#endif
		...
		#ifdef MT_DVFS_FFTT_TEST
		mt_entry = proc_create("cpufreq_fftt_test", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_fftt_test_fops);
		if (mt_entry)
		{
		}
		#endif

		mt_entry = proc_create("cpufreq_limited_by_hevc", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_limited_by_hevc_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_limited_max_freq_by_user", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_limited_max_freq_by_user_fops);
		if (mt_entry) {
		}

		mt_entry = proc_create("cpufreq_ptpod_freq_volt", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_ptpod_freq_volt_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("socfreq_limited_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_socfreq_limited_power_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("socfreq_enable_hp", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_socfreq_enable_hp_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("socfreq_gpu_power_dump", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_socfreq_gpu_power_dump_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("socfreq_debug", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_socfreq_debug_fops);
		if (mt_entry)
		{
		}

		#ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
		mt_entry = proc_create("socfreq_voltage", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_socfreq_voltage_fops);
		if (mt_entry)
		{
		}
		#endif

		mt_entry = proc_create("socfreq_fixed_test", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_socfreq_fixed_test_fops);
		if (mt_entry)
		{
		}

		#ifdef PTPOD_DOWNGRADE_FREQ
		mt_entry = proc_create("cpufreq_downgrade_freq_info", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_downgrade_freq_info_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_downgrade_freq_step_idx", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_downgrade_freq_step_idx_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_ptpod_temperature_limit", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_ptpod_temperature_limit_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_ptpod_temperature_delta", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_ptpod_temperature_delta_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_ptpod_temperature_delta_ratio", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_ptpod_temperature_delta_ratio_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_over_max_cpu", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_over_max_cpu_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_downgrade_freq_enable", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_downgrade_freq_enable_fops);
		if (mt_entry)
		{
		}
		#endif

		mt_entry = proc_create("cpufreq_lbat_volt_limited_cpu_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_lbat_volt_limited_cpu_power_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_lbat_volt_limited_gpu_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_lbat_volt_limited_gpu_power_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_lbat_volume_limited_cpu_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_lbat_volume_limited_cpu_power_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_lbat_volume_limited_gpu_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_lbat_volume_limited_gpu_power_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_lbat_volt_limited", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_lbat_volt_limited_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_lbat_volt_drop_enable", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_lbat_volt_drop_enable_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_lbat_volume_enable", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_lbat_volume_enable_fops);
		if (mt_entry)
		{
		}

		#ifdef CPUFREQ_HIGHEST_TURBO_MODE
		mt_entry = proc_create("cpufreq_highest_turbo_mode", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_highest_turbo_mode_fops);
		if (mt_entry)
		{
		}
		#endif

		mt_entry = proc_create("cpufreq_total_limited_cpu_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_total_limited_cpu_power_fops);
		if (mt_entry)
		{
		}

		mt_entry = proc_create("cpufreq_total_limited_gpu_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir, &mt_cpufreq_total_limited_gpu_power_fops);
		if (mt_entry)
		{
		}
