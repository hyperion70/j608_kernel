...
{
#if defined(CONFIG_MTK_MD5_SUPPORT)
    {"MTK_MD5_SUPPORT",	CONFIG_MTK_MD5_SUPPORT},
#else
	...
	if(!(md_active_setting & MD5_SETTING_ACTIVE)) {
  	...
  	switch(active_id) {
		...
		if (md_setting_flag == MD_2G_FLAG){
			meta_ext_md_support[active_index] = modem_2g;
		} else if (md_setting_flag == MD_WG_FLAG){
			meta_ext_md_support[active_index] = modem_wg;
		} else if (md_setting_flag == MD_TG_FLAG){
			meta_ext_md_support[active_index] = modem_tg;
...
void get_ext_md_post_fix(int md_id, char buf[], char buf_ex[])
{
	...
	switch(feature_val) {
	  	case modem_2g:
	  		...
	  		break;
		case modem_3g:
			snprintf(YYY, 16, "_3g_n");
			break;
		case modem_wg:
			snprintf(YYY, 16, "_wg_n");
			break;
		case modem_tg:
