...
#if !defined(CONFIG_TRUSTONIC_TEE_SUPPORT) && defined(SYSTEM_STABILITY_TOOL_ENABLE)
static int dram_clk;
...
static ssize_t DFS_HQA_Config_store(struct device_driver *driver, const char *buf, size_t count)
{
  ...
  if(sel==0x1){
    ...
    dfs_con.vcore_HF_HV=Vcore_HV_1066+0x14; //Emulated GPU High Voltage
    dfs_con.vcore_HF_NV=Vcore_NV_1066+0x14; //Emulated GPU High Voltage
    dfs_con.vcore_HF_LV=Vcore_LV_1066+0x14; //Emulated GPU High Voltage
...
int DFS_APDMA_early_init(void)
{
   ...
   memset((unsigned int *)src_array_v,0x6a6a6a6a,BUFF_LEN);
   #endif
...
{
  unsigned int high_memory_phys;
  unsigned int DFS_dummy_read_center_address;
...
void dump_DMA_Reserved_AREA(void)
{
  ...
  debug_p_src=(unsigned int *)src_array_v;
  debug_p_dst=(unsigned int *)dst_array_v;
  ...
  {
   for(j=0;j<8;j++)
     printk("&0x%x=0x%x ",(unsigned int)((debug_p_src+i*8)+j),*((debug_p_src+i*8)+j));
   printk("\n");
  }
  ...
  {
   for(j=0;j<8;j++)
     printk("&0x%x=0x%x ",(unsigned int)((debug_p_dst+i*8)+j),*((debug_p_dst+i*8)+j));
   printk("\n");
  }
