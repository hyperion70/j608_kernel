...
extern unsigned int mt_cpufreq_hotplug_notify(unsigned int ncpu);   //definition in mt_cpufreq.c
...
static inline void cpu_enter_lowpower(unsigned int cpu)
{
...
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT)    
    mt_secure_call (MC_FC_GOTO_SLEEP, 0, cpu, 2);
#endif
        ...
        ((cpu == 7) && (cpu_online(4) == 0) && (cpu_online(5) == 0) && (cpu_online(6) == 0)))
    {
        ...
        /* Disable CA7 snoop function */
        REG_WRITE(MP1_AXI_CONFIG, REG_READ(MP1_AXI_CONFIG) | ACINACTM);
    }
    else
    {
        /* Clear the SCTLR C bit to prevent further data cache allocation */
