...
/* do not allow core 0 to enter MCDI for TEE case */
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT)
    if (cpu == 0) {
        reason = BY_OTH;
        goto mcidle_out;
                ...
                struct kobj_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < nr_cpu_ids; i++) {
        p += sprintf(p, "soidle_cnt[%d]=%lu, dpidle_cnt[%d]=%lu, "
                ...
                i, mcidle_cnt[i], i, (long unsigned int)mcdi_pdn_cnt[i], i, slidle_cnt[i], i, rgidle_cnt[i]);
    }
#else
for (i = 0; i < nr_cpu_ids; i++) {
    p += sprintf(p, "soidle_cnt[%d]=%lu, dpidle_cnt[%d]=%lu, "
