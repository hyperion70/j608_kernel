...
static int deint_mapping[DEINT_MAX_CHANNEL];
#endif
...
void mt_eint_set_polarity(unsigned int eint_num, unsigned int pol)
{
    ...
    //Delay a while (more than 3T) to avoid eint trigger before hw debounce */
    udelay(200);
...
#ifdef DEINT_SUPPORT
static void mt_deint_enable (unsigned int deint_num)
{
  unsigned int base;
  unsigned int bit = 1 << (deint_num % 32);

  if (deint_num < MAX_DEINT_CNT)
  {
      base = (deint_num / 32) * 4 + DEINT_CON_BASE;
  }
  else
  {
      dbgmsg("[DEINT NUMBER ERROR] eint_num :%d \n",deint_num);
      return;
  }
  
  writel(bit, IOMEM(base));        
  dbgmsg ("[DEINT] control addr:%x = %x\n", base, bit);
}

static int deint_get_select(int deint_num)
{
    unsigned int base;
    unsigned int sel;
    unsigned int mask = 0xFF << ((deint_num % 4)*8);
    unsigned int shift = 8 * (deint_num % 4);

    if (deint_num < MAX_DEINT_CNT)
    {
        base = (deint_num / 4) * 4 + DEINT_SEL_BASE;
    }
    else
    {
        dbgmsg("[DEINT NUMBER ERROR] deint_num :%d \n",deint_num);
        return -1;
    }
  
    sel = (readl(IOMEM(base)) & mask) >> shift;
    dbgmsg ("[DEINT] %s :%x, select: %d\n", __func__, base, sel);
    
    return sel;
}

static int deint_clear_select(int deint_num)
{
    unsigned int base;
    unsigned int value = 0xFF << ((deint_num % 4)*8);

    if (deint_num < MAX_DEINT_CNT)
    {
        base = (deint_num / 4) * 4 + DEINT_SEL_CLR_BASE;
    }
    else
    {
        dbgmsg("[DEINT NUMBER ERROR] deint_num :%d \n",deint_num);
...
static void deint_set_select(int deint_num, int sel)
{
    unsigned int base;
    unsigned int value = sel << ((deint_num % 4)* 8);

    if ((deint_num < MAX_DEINT_CNT) && (sel < EINT_AP_MAXNUMBER))
    {
        base = (deint_num / 4) * 4 + DEINT_SEL_SET_BASE;
    }
    else
    {
        dbgmsg("[DEINT NUMBER OR SELECTION ERROR] deint_num :%d, sel :%d \n", deint_num, sel);
        return;
    }
  
    deint_clear_select(deint_num);
    
    writel(value, IOMEM(base));
    dbgmsg("[DEINT] %s :%x, value: %x\n", __func__, base, value);
 ...
 */
static void mt_deint_registration(unsigned int deint_num, unsigned int eint_num, unsigned int flag,
              void (DEINT_FUNC_PTR) (void))
{
    int num = eint_num;
    if (deint_num < DEINT_MAX_CHANNEL && num < EINT_MAX_CHANNEL) {

        // set deint <--> eint mapping
        deint_mapping[deint_num] = num;
        ...
        deint_set_select(deint_num, num);
        mt_deint_enable(deint_num);  
...
static int __init mt_eint_init(void)
{
    ...
    // sim-hot-plug unit test
    #if 0
    char info[120] = { 0 };
    char *p = "MD1_SIM1_HOT_PLUG_EINT";
    unsigned int len = strlen(p);
    unsigned int result;

    printk("============\n");
    printk("start test on SIM eint\n");
    printk("============\n");
 
    get_eint_attribute(p, len, SIM_HOT_PLUG_EINT_NUMBER, &info, &result); 
    get_eint_attribute(p, len, SIM_HOT_PLUG_EINT_DEBOUNCETIME, &info, &result); 
    get_eint_attribute(p, len, SIM_HOT_PLUG_EINT_POLARITY, &info, &result); 
    get_eint_attribute(p, len, SIM_HOT_PLUG_EINT_SENSITIVITY, &info, &result); 
    get_eint_attribute(p, len, SIM_HOT_PLUG_EINT_SOCKETTYPE, &info, &result); 
    #endif
