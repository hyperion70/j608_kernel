...
static int g_memory_debug = SPM_MP1_FC0_PWR_CON;
...
static int mt_hotplug_mechanism_resume(struct platform_device *pdev)
{
	unsigned int cpu, little_cpu_num_resume = 4;
	...
	for (cpu = 0; cpu <= little_cpu_num_resume; cpu++) {
		if (!cpu_online(cpu))
			cpu_up(cpu);
	}
...
extern int g_tlp_iowait_av;

extern int g_cpu_rush_count;

static int mt_hotplug_mechanism_read_test1(struct seq_file *m, void *v)
{
    seq_printf(m, "%d\n", g_test1);
    
    //HOTPLUG_INFO("hp_to_callback(%d)(%d)(%d)(%d)(%d)(%ld)(%d)(%ld)(%d)(%d)(%d)\n", 
    ...
    //    g_cpu_up_load_index, g_cpu_down_load_index);
