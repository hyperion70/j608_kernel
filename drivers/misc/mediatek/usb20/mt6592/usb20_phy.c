...
//ALPS00427972, implement the analog register formula
	...
	if (in_uart_mode) {
		USBPHY_SET8(0x68, 0x08);
		pr_debug("%s:%d - SWITCH to UART MODE after savecurrent!\n", __func__, __LINE__);
	}
#endif
}
...
void usb_phy_recover(void){
   ...
   //4 11. force_dp_pulldown = 1b'0
    ...
    USBPHY_SET8(0x6d, 0x3E);
		...
		printk("Kernel Driver USB HW reg: offset 0x0184=0x%x\n", get_devinfo_with_index(19));
    ...
    {
    	//printk("USB HW reg: read RG_USB20_INTR_EN 0x%x\n", USBPHY_READ8(0x00));
    ...
    {
    	//printk("USB HW reg: read RG_USB20_VRT_VREF_SEL 0x%x\n", USBPHY_READ8(0x05));
			USBPHY_WRITE8(0x05,((USBPHY_READ8(0x05)&0x8F)|(get_devinfo_with_index(19)&0x70)));
			printk("USB HW reg: write RG_USB20_VRT_VREF_SEL 0x%x\n", USBPHY_READ8(0x05));
		}
