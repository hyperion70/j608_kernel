...
static void mtkfb_fb_565_to_888(void* fb_va)
{
    ...
    unsigned char *d = (unsigned char*)(fb_va + DISP_GetFBRamSize() + DAL_GetLayerSize() + decouple_size / 3 * 2);
    unsigned short src_rgb565 = 0;
    int j = 0;
    int k = 0;
...
static void mtkfb_fb_565_to_uyvy(void* fb_va)
{
    unsigned int xres = DISP_GetScreenWidth();
    unsigned int yres = DISP_GetScreenHeight();
    unsigned int x_virtual = ALIGN_TO(xres,disphal_get_fb_alignment());
    ...
    MTKFB_INFO("565_to_uyvy, s = 0x%x, d=0x%x\n", s,d);
    memset((void *)(fb_va + DISP_GetFBRamSize() + DAL_GetLayerSize()), 0, decouple_size);
    for (j = 0; j < yres; ++ j)
    {
        for(k = 0; k < xres; ++ k)
        {
            src_rgb565 = *s++;
            b = (((src_rgb565 & 0x1F) << 3) | ((src_rgb565 & 0x1C) >> 2));
            g = (((src_rgb565 & 0x7E0) >> 3) | ((src_rgb565 & 0x600) >> 9));
            r = (((src_rgb565 & 0xF800) >> 8) | ((src_rgb565 & 0xE000) >> 13));
            // or swapped?
            //r = (((src_rgb565 & 0x1F) << 3) | ((src_rgb565 & 0x1C) >> 2));
            //g = (((src_rgb565 & 0x7E0) >> 3) | ((src_rgb565 & 0x600) >> 9));
            //b = (((src_rgb565 & 0xF800) >> 8) | ((src_rgb565 & 0xE000) >> 13));

            y = ((16842*r + 33030*g + 6422*b) >> 16) + 16;
            u = ((28770*b - 9699*r - 19070*g) >> 16) + 128;
            v = ((28770*r - 4653*b - 24117*g) >> 16) + 128;
    ...
    d = (unsigned char*)(fb_va + DISP_GetFBRamSize() + DAL_GetLayerSize());
    memcpy((void *)d + decouple_size / 3, (void *)d, decouple_size / 3);
    memcpy((void *)d + decouple_size / 3 * 2, (void *)d, decouple_size / 3);
}
...
static int mtkfb_probe(struct device *dev)
{
    ...
    fbdev->fb_size_in_byte = MTK_FB_SIZEV;
    {
        ...
        mtkfb_fb_565_to_uyvy((void*)fbdev->fb_va_base);
