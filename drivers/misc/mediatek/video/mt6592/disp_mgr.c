...
DCP_STATUS disp_create_session (struct disp_session_config_t *config) {
	...
	if (ret != DCP_STATUS_ALREADY_EXIST) {
		...
		/* do necessary clean up in case the session is not actually destoryed. */
		disp_sync_cleanup();	
...
// patch workaround: re-define of color format
// the 4 kinds of color format, which in DpDataType.h,
// is the same define as ARGB / ABGR / RGBA / BGRA
// thus we re-define it to make sure driver receives correct setting
#ifdef eXRGB8888
#define DP_COLOR_XRGB8888 (DP_COLORFMT_PACK(0,   1,  0, 0, 0, 32, 0,  1, 4))
#define eXRGB8888               DP_COLOR_XRGB8888
#endif
#ifdef eXBGR8888
#define DP_COLOR_XBGR8888 (DP_COLORFMT_PACK(0,   1,  0, 0, 0, 32, 0,  0, 4))
#define eXBGR8888               DP_COLOR_XBGR8888
#endif
#ifdef eRGBX8888
#define DP_COLOR_RGBX8888 (DP_COLORFMT_PACK(0,   1,  0, 0, 0, 32, 0,  1, 5))
#define eRGBX8888               DP_COLOR_RGBX8888
#endif
#ifdef eBGRX8888
#define DP_COLOR_BGRX8888 (DP_COLORFMT_PACK(0,   1,  0, 0, 0, 32, 0,  0, 5))
	...
	for (i = 0; i < MAX_INPUT_CONFIG; i++) {
		...
		if (config->layer_enable) {
			...
			case DISP_FORMAT_XRGB8888:
				job->input[i].format = eXRGB8888;
				layerpitch = 4;
				layerbpp = 32;
				break;
			case DISP_FORMAT_XBGR8888:
				job->input[i].format = eXBGR8888;
				layerpitch = 4;
				layerbpp = 32;
				break;
		...
		unsigned int offset = config->src_offset_y * config->src_pitch * layerpitch + config->src_offset_x * layerpitch;
		if(config->layer_enable && i == 0) 
			update_frm_seq_info(input->session_id, config->src_phy_addr, offset, config->frm_sequence, FRM_CONFIG);
		else if(config->layer_enable == 0 && i == 0)
			XLOG_ERR("layer 0 is not enabled\n");
