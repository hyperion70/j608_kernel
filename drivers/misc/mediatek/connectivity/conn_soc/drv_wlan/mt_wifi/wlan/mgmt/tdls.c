...
static TDLS_STATUS
TdlsCmdTestTxFmeSetupReqBufTranslate(UINT_8 *pCmdBuf, UINT_32 u4BufLen, PARAM_CUSTOM_TDLS_CMD_STRUC_T *prCmd);
...
static void TdlsCmdTestChSwReqRecv(P_GLUE_INFO_T prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	DBGLOG(TDLS, INFO, ("%s: [" MACSTR "] u4Chan=%u u4RegClass=%u"
			    ...
			    __func__, MAC2STR(rCmd.aucPeerMac),
...
static void TdlsCmdTestDiscoveryReqRecv(GLUE_INFO_T *prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	DBGLOG(TDLS, INFO,
	       ("<tdls_fme> %s: DialogToken=%d from " MACSTR "\n", __func__, ucDialogToken, MAC2STR(aucPeerMac)));
...
static void TdlsCmdTestPtiReqRecv(P_GLUE_INFO_T prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	DBGLOG(TDLS, INFO, ("%s: [" MACSTR "] u4DialogToken = %u\n",
			    __func__, MAC2STR(rCmd.aucPeerMac), (UINT32) rCmd.Content.rCmdPtiRspRcv.u4DialogToken));
...
/* return; */
	...
	kalIoctl(prGlueInfo,
		 TdlsTestFrameSend,
		 (PVOID) & rCmd, sizeof(PARAM_CUSTOM_TDLS_CMD_STRUC_T), FALSE, FALSE, TRUE, FALSE, &u4BufLen);
#endif
}
...
static void TdlsCmdTestSetupConfirmRecv(GLUE_INFO_T *prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	DBGLOG(TDLS, INFO,
		...
		__func__, ucDialogToken, ucStatusCode, MAC2STR(aucPeerMac)));
...
static void TdlsCmdTestTearDownRecv(GLUE_INFO_T *prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	DBGLOG(TDLS, INFO,
		...
		__func__, ucReasonCode, MAC2STR(aucPeerMac), fgIsFromWhich));
...
static void TdlsCmdTestTxTdlsFrame(P_GLUE_INFO_T prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	case TDLS_FRM_ACTION_CONFIRM:
		...
		DBGLOG(TDLS, INFO, ("<tdls_cmd> setup FmeType=%d Token=%d to "
				    "[" MACSTR "]\n", rCmd.ucFmeType, rCmd.ucToken, MAC2STR(rCmd.arRspAddr)));
		break;
...
static void TdlsCmdTestNullRecv(P_GLUE_INFO_T prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	DBGLOG(TDLS, INFO, ("%s: [" MACSTR "] u4PM = %u\n",
			    __func__, MAC2STR(rCmd.aucPeerMac), (UINT32) rCmd.Content.rCmdNullRcv.u4PM));
...
static VOID TdlsTimerTestDataContSend(ADAPTER_T *prAdapter, UINT_32 u4Param)
{
	...
	*(UINT_16 *) (prMsduInfo->data + 12) = 0x0800;
	...
	DBGLOG(TDLS, INFO, ("<tdls_cmd> %s try to send a data frame to " MACSTR "\n",
			    __func__, MAC2STR(aucTdlsTestDataSPeerMac)));
...
TdlsTestTdlsFrameSend(ADAPTER_T *prAdapter, VOID *pvSetBuffer, UINT_32 u4SetBufferLen, UINT_32 *pu4SetInfoLen)
{
	...
	TdlsexCfg80211TdlsMgmt(prWdev->wiphy, NULL, prCmd->arRspAddr, prCmd->ucFmeType, 1, 0, NULL,	/* open/none */
			       0);
...
*/
/*----------------------------------------------------------------------------*/
void cfg80211_tdls_oper_request(struct net_device *dev, const u8 *peer, u16 oper, u16 reason_code, gfp_t gfp)
{
	GLUE_INFO_T *prGlueInfo;
	ADAPTER_T *prAdapter;
	struct sk_buff *prMsduInfo;
	...
	DBGLOG(TDLS, INFO,
		...
		__func__, oper, reason_code, MAC2STR(peer)));
...
static void TdlsCmdInfoDisplay(GLUE_INFO_T *prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	DBGLOG(TDLS, INFO, ("<tdls_cmd> %s: Command PeerMac=" MACSTR " in BSS%u\n",
			    __func__, MAC2STR(rCmd.aucPeerMac), rCmd.ucNetTypeIndex));
		      ...
		      UINT8 *pucPeerMac, BOOLEAN fgIsFromUs, UINT16 u2ReasonCode, VOID *prOthers)
{
	...
	DBGLOG(TDLS, INFO,
		...
		__func__, MAC2STR(pucPeerMac), prGlueInfo->rTdlsLink.u4LinkIdx,
			    ...
			    UINT8 *pucPeerMac, TDLS_EVENT_HOST_SUBID_SPECIFIC_FRAME eFmeStatus, VOID *pInfo)
{
	...
	DBGLOG(TDLS, INFO,
		...
		__func__, MAC2STR(pucPeerMac), prGlueInfo->rTdlsLink.u4LinkIdx, eFmeStatus));
...
static void TdlsEventStatistics(GLUE_INFO_T *prGlueInfo, UINT8 *prInBuf, UINT32 u4InBufLen)
{
	...
	for (u4RateId = prStat->u4TxRateOkHisId; u4RateId < STAT_CNT_INFO_MAX_TX_RATE_OK_HIS_NUM; u4RateId++)
		printk("%d(%d) ", prStat->aucTxRateOkHis[u4RateId][0], prStat->aucTxRateOkHis[u4RateId][1]);
...
static void TdlsEventTearDown(GLUE_INFO_T *prGlueInfo, UINT8 *prInBuf, UINT32 u4InBufLen)
{
	...
	if (u4TearDownSubId == TDLS_HOST_EVENT_TD_PTI_TIMEOUT) {
		...
		DBGLOG(TDLS, WARN, ("<tdls_evt> %s: peer [" MACSTR "] Reason=%d\n",
				    __func__, MAC2STR(pMac), u4TearDownSubId));
	}
...
VOID TdlsexBssExtCapParse(STA_RECORD_T *prStaRec, UINT_8 *pucIE)
{
	...
	DBGLOG(TDLS, TRACE,
		...
		__func__,
		MAC2STR(prStaRec->aucMacAddr), prStaRec->fgTdlsIsProhibited, prStaRec->fgTdlsIsChSwProhibited));
}
...
/*----------------------------------------------------------------------------*/
int
TdlsexCfg80211TdlsMgmt(struct wiphy *wiphy,
		       ...
		       u8 *peer, u8 action_code, u8 dialog_token, u16 status_code, const u8 *buf, size_t len)
{
	...
	DBGLOG(TDLS, INFO, ("<tdls_cfg> %s: [" MACSTR "] %d %d %d 0x%p %u\n",
			    __func__, MAC2STR(peer), action_code, dialog_token, status_code, buf, (UINT32) len));
...
*/
/*----------------------------------------------------------------------------*/
int TdlsexCfg80211TdlsOper(struct wiphy *wiphy, struct net_device *dev, u8 *peer, enum nl80211_tdls_operation oper)
{
	ADAPTER_T *prAdapter;
	GLUE_INFO_T *prGlueInfo;
	WLAN_STATUS rStatus;
	...
	DBGLOG(TDLS, INFO, ("<tdls_cfg> %s: [" MACSTR "] %d %d\n",
			    __func__, MAC2STR(peer), oper, (wiphy->flags & WIPHY_FLAG_SUPPORTS_TDLS)));
			...
			BOOLEAN fgIsTearDown, UINT8 *pucPeerMac, BOOLEAN fgIsFromUs, UINT16 u2ReasonCode)
{
	...
	DBGLOG(TDLS, INFO,
		...
		__func__, MAC2STR(pucPeerMac), fgIsFromUs, u2ReasonCode));
...
TDLS_STATUS TdlsexKeyHandle(ADAPTER_T *prAdapter, PARAM_KEY_T *prNewKey)
{
	...
	if ((prStaRec != NULL) && IS_TDLS_STA(prStaRec)) {
		DBGLOG(TDLS, TRACE, ("<tdls> %s: [" MACSTR "] queue key (len=%d) "
				     ...
				     __func__, MAC2STR(prNewKey->arBSSID), (UINT32) prNewKey->u4KeyLength));

		if (prStaRec->ucStaState == STA_STATE_3) {
			DBGLOG(TDLS, TRACE, ("<tdls> %s: [" MACSTR "] tear down the link "
					     "due to STA_STATE_3\n", __func__, MAC2STR(prNewKey->arBSSID)));
	...
	*pu4SetInfoLen = sizeof(TDLS_CMD_LINK_T);
	...
	if (prStaRec == NULL) {
		DBGLOG(TDLS, ERROR, ("<tdls_cfg> %s: cannot find the peer! " MACSTR "\n",
				     __func__, MAC2STR(prCmd->aucPeerMac)));
		return TDLS_STATUS_FAILURE;
	}
	...
	*pu4SetInfoLen = sizeof(TDLS_MGMT_TX_INFO);
	...
	if (prStaRec != NULL) {
		DBGLOG(TDLS, INFO, ("<tdls_cfg> %s: [" MACSTR "] ps=%d status=%d\n",
				    __func__, MAC2STR(prStaRec->aucMacAddr),
...
/* ASSERT(prStaRec->ucPhyTypeSet & PHY_TYPE_SET_802_11N); */
	...
	DBGLOG(TDLS, INFO, ("<tdls_cmd> %s: [" MACSTR "] [" MACSTR "]\n",
			    __func__, MAC2STR(prGlueInfo->aucTdlsHtPeerMac), MAC2STR(prCmd->aucPeerMac)));
...
/*	prStaRec->u2HtExtendedCap */
	...
	if (prCmd->fgIsSupHt == FALSE) {
		...
		DBGLOG(TDLS, INFO, ("<tdls_cmd> %s: [" MACSTR "] update ht ie 0x%x\n",
				    __func__, MAC2STR(prStaRec->aucMacAddr), prHtCap->ucId));
...
VOID TdlsexRxFrameHandle(GLUE_INFO_T *prGlueInfo, UINT8 *pPkt, UINT16 u2PktLen)
{
	...
	DBGLOG(TDLS, TRACE,
		...
		__func__, ucActionId, MAC2STR(pucPeerMac)));
...
/* if (prStaRec->fgIsInPS == TRUE) */
	...
	DBGLOG(TDLS, INFO, ("<tdls> %s: (Status=%x) [" MACSTR "] ucStaRecIndex = %d!\n",
			    __func__, (INT32) Status, MAC2STR(prMsduInfo->aucEthDestAddr),
