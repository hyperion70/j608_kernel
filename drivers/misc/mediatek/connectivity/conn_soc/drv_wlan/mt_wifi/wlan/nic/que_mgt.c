					...
					if (IE_LEN(pucIE) != 24) {
						...
						*pucWmmParamSetCount =
						...
						for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
							...
							DBGLOG(QM, LOUD,
								("MQM: eAci = %d, ACM = %d, Aifsn = %d, "
								"CWmin = %d, CWmax = %d, TxopLimit = %d\n",
...
UINT_32 mqmGenerateWmmParamIEByParam(P_ADAPTER_T prAdapter, P_BSS_INFO_T prBssInfo, UINT_8 *pOutBuf)
{
	P_IE_WMM_PARAM_T prIeWmmParam;
	UINT_8 aucWfaOui[] = VENDOR_OUI_WFA;
	UINT_8 aucACI[] = {
...
/* 20120220 frog: update beacon content & change OP mode is a separate event for P2P network. */
	...
	prMsduInfo->u2FrameLength += mqmGenerateWmmParamIEByParam(prAdapter, prBssInfo, (UINT_8 *) prIeWmmParam);
#endif
}
