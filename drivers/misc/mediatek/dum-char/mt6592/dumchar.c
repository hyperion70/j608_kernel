	...
	while ((part = disk_part_iter_next(&piter))) {
		for (i = 0; i < PART_NUM; i++) {
			if (PartInfo[i].start_address == part->start_sect * 512) {
				...
				printk(KERN_ERR "%s:target=%s, name=%s\n", __func__, link_target, link_name);
...
static ssize_t dumchar_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		printk("[dumchar_read] It is forbidded to access %s with dumchar(/dev/%s)," 
            "the %s partition is managed by filesystem!\n", dev->dumname, dev->dumname, dev->dumname);
...
ssize_t dumchar_write (struct file *filp, const char __user *buf, size_t count,loff_t *f_pos)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		printk("[dumchar_write] It is forbidded to access %s with dumchar(/dev/%s),"
            "the %s partition is managed by filesystem!\n", dev->dumname, dev->dumname, dev->dumname);
...
static long dumchar_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		printk("[dumchar_ioctl] It is forbidded to access %s with dumchar(/dev/%s),"
            "the %s partition is managed by filesystem!\n", dev->dumname, dev->dumname, dev->dumname);
...
loff_t dumchar_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		printk("[dumchar_llseek] It is forbidded to access %s with dumchar(/dev/%s),"
            "the %s partition is managed by filesystem!\n", dev->dumname, dev-> dumname, dev->dumname);
...
int dumchar_open(struct inode *inode, struct file *filp)
{
	...
	if (!strcmp(dev->dumname,"usrdata") || !strcmp(dev->dumname,"cache") ||
		...
		printk("[dumchar_open] It is forbidded to access %s with dumchar(/dev/%s),"
            "the %s partition is managed by filesystem!\n", dev->dumname, dev->dumname, dev->dumname);
...
int dumchar_fsync(struct file *filp, loff_t start, loff_t end, int datasync)
{
	struct file_obj *fo = filp->private_data;
	struct dumchar_dev *dev = dumchar_devices + fo->index;
	ssize_t result = 0;
#ifdef CONFIG_MTK_EMMC_SUPPORT
	loff_t part_start, part_end;
#endif

	if (fo->act_filp == (struct file *)0xffffffff) {
		printk("[dumchar_fsync] It is forbidded to access %s with dumchar(/dev/%s)," 
            "the %s partition is managed by filesystem!\n", dev->dumname, dev->dumname, dev->dumname);
		printk("[dumchar_fsync] show_stack*************************************\n");
		show_stack(NULL,NULL);
		return -EINVAL;
	}
	...
	if (dev->type == EMMC) {
		...
		switch (dev->region) {
		case EMMC_PART_USER:
			part_start = dev->start_address;
			part_end = part_start + dev->size - 1;
			result = vfs_fsync_range(fo->act_filp, part_start, part_end, datasync);
			break;
		case EMMC_PART_BOOT1:
			printk("Dumchar: fsync not support on eMMC BOOT region\n");
			result = -EINVAL;
			break;				
		default:
			printk("DumChar: Wrong EMMC Region\n");
		...
		printk("Dumchar: fsync not support on Nand\n");
		result = -EINVAL;
	}
	return result;	
}
...
int dumchar_release(struct inode *inode, struct file *filp)
{
	...
	if (!strcmp(dev->dumname,"usrdata") || !strcmp(dev->dumname,"cache") || 
        !strcmp(dev->dumname,"android") || !strcmp(dev->dumname,"fat")) {
		printk("[dumchar_release] It is forbidded to access %s with dumchar(/dev/%s),"
            "the %s partition is managed by filesystem!\n", dev->dumname, dev->dumname, dev->dumname);
	} else {
		filp_close(fo->act_filp, NULL);
	}
...
int dumchar_probe(struct platform_device *dev)
{
	...
	dumchar_devices = kzalloc(PART_NUM * sizeof(struct dumchar_dev), GFP_KERNEL);
	if (!dumchar_devices) {
		result = -ENOMEM;
	...
	for (i = 0; i < PART_NUM; i++) {
		...
		if (dumchar_devices[i].type == EMMC) {
			...
			printk("[Dumchar] %s start address=%llx size=%llx\n", dumchar_devices[i].dumname, 
                dumchar_devices[i].start_address, dumchar_devices[i].size);
#endif
		} else {
#ifdef CONFIG_MTK_MTD_NAND
