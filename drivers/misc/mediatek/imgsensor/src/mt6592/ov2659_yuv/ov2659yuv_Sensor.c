...
#define OV2659YUV_DEBUG			1
#define OV2659YUV_FORCE_PRINTK	0

#define TAG		"[ov2659yuv]"

#if OV2659YUV_DEBUG
#if OV2659YUV_FORCE_PRINTK
#define OV2659SENSORDB(fmt, args...) printk(TAG fmt, ##args)
#define OV2659SENSORDB_FUNC_LINE(fmt, args...) printk(TAG "Func[%s],Line[%d] " fmt "\n", __FUNCTION__, __LINE__, ##args)
#else
#define OV2659SENSORDB(fmt, args...) pr_debug(TAG fmt, ##args)
#define OV2659SENSORDB_FUNC_LINE(fmt, args...) pr_debug(TAG "Func[%s],Line[%d] " fmt "\n", __FUNCTION__, __LINE__, ##args)
#endif
#else
#define OV2659SENSORDB(fmt, args...)
#define OV2659SENSORDB_FUNC_LINE(fmt, args...)
#endif
...
#define MDELAY_LOG(ms)	\
	{\
		OV2659SENSORDB_FUNC_LINE("delay %dms\n", (ms));\
		mdelay(ms);\
	}

#define OV2659_TEST_PATTERN_CHECKSUM (0x1c936910)
...
static void OV2659InitialSetting(void)
{
	...
	OV2659_write_cmos_sensor(0x3011, 0xC2/*0x82*/);		// increase io capability   
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    // Houston 20131023 color effect <-> Face buty
...
{
	OV2659SENSORDB("YUVSensorSetting +, icmd=%d, ipara=%d\n", iCmd, iPara);
	...
	OV2659SENSORDB("YUVSensorSetting -");
...
UINT32 OV2659YUVSetVideoMode(UINT16 u2FrameRate)
{
    ...
    {
        OV2659SENSORDB("Wrong frame rate setting \n");
    }   
	/* mDELAY(30); */
