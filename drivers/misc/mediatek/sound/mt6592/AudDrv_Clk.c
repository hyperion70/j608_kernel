...
int        Aud_LJ22M_Clk_cntr    = 0;
int        Aud_LJ24M_Clk_cntr    = 0;
...
void AudDrv_LowJitter24M_Clk_On(void)
{
    unsigned long flags;
    spin_lock_irqsave(&auddrv_Clk_lock, flags);
    if (Aud_LJ24M_Clk_cntr == 0)
    {
        PRINTK_AUD_CLK("+AudDrv_LowJitter24M_Clk_On, Aud_LJ24M_Clk_cntr:%d \n", Aud_LJ24M_Clk_cntr);
#ifdef PM_MANAGER_API
        enable_mux(MT_MUX_AUD_24M, "name");
        Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0, 0x00000200);  // bit9: lowjitter 24M power on
#else
        Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0, 0x00000200);  // bit9: lowjitter 24M power on
#endif
    }
...
void AudDrv_LowJitter24M_Clk_Off(void)
{
    unsigned long flags;
    spin_lock_irqsave(&auddrv_Clk_lock, flags);
    ...
    if (Aud_LJ24M_Clk_cntr == 0)
    {
        PRINTK_AUD_CLK("+ AudDrv_LowJitter24M_Clk_Off, Aud_LJ24M_Clk_cntr:%d \n", Aud_LJ24M_Clk_cntr);
        {            
#ifdef PM_MANAGER_API
            Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0x00000200, 0x00000200);    // bit9: lowjitter 24M power down
    ...
    else if (Aud_LJ24M_Clk_cntr < 0)
    {
        PRINTK_AUD_ERROR("!! AudDrv_LowJitter24M_Clk_Off, Aud_LJ24M_Clk_cntr<0 (%d) \n", Aud_LJ24M_Clk_cntr);
        //AUDIO_ASSERT(true);
...
void AudDrv_LowJitter22M_Clk_On(void)
{
    unsigned long flags;
    spin_lock_irqsave(&auddrv_Clk_lock, flags);
    if (Aud_LJ22M_Clk_cntr == 0)
    {
        PRINTK_AUD_CLK("+AudDrv_LowJitter22M_Clk_On, Aud_LJ22M_Clk_cntr:%d \n", Aud_LJ22M_Clk_cntr);
#ifdef PM_MANAGER_API
        enable_mux(MT_MUX_AUD_22M, "name");
        Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0, 0x00000100);  // bit8: lowjitter 22M power on
#else
        Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0, 0x00000100);  // bit8: lowjitter 22M power on
#endif
    }
...
void AudDrv_LowJitter22M_Clk_Off(void)
{
    unsigned long flags;
    spin_lock_irqsave(&auddrv_Clk_lock, flags);
    ...
    if (Aud_LJ22M_Clk_cntr == 0)
    {
        PRINTK_AUD_CLK("+ AudDrv_LowJitter22M_Clk_Off, Aud_LJ22M_Clk_cntr:%d \n", Aud_LJ22M_Clk_cntr);
        {            
#ifdef PM_MANAGER_API
            Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0x00000100, 0x00000100);    // bit8: lowjitter 22M power down
    ...
    else if (Aud_LJ22M_Clk_cntr < 0)
    {
        PRINTK_AUD_ERROR("!! AudDrv_LowJitter22M_Clk_Off, Aud_LJ22M_Clk_cntr<0 (%d) \n", Aud_LJ22M_Clk_cntr);
        //AUDIO_ASSERT(true);
...
void AudDrv_Suspend_Clk_On(void)
{
    ...
    if (Aud_AFE_Clk_cntr > 0)
    {
        ...
        if (Aud_I2S_Clk_cntr > 0)
        {
            ...
            {
                pr_err("AudDrv_Suspend_Clk_On() Aud enable_clock() MT_CG_AUDIO_I2S fail");
            }
        }
        if (Aud_LJ22M_Clk_cntr > 0)
        {
            enable_mux(MT_MUX_AUD_22M, "name");
            Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0x00000000, 0x00000100);  // bit8: lowjitter 22M power on
        }
        else
            Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0x00000100, 0x00000100);  // bit8: lowjitter 22M power down
        ...
        {
            enable_mux(MT_MUX_AUD_24M, "name");
            Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0x00000000, 0x00000200);  // bit9: lowjitter 24M power on
        }
        else
            Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0x00000200, 0x00000200);  // bit9: lowjitter 24M power down
...
void AudDrv_Suspend_Clk_Off(void)
{
    ...
    if (Aud_AFE_Clk_cntr > 0)
    {
        ...
        Afe_Set_Reg(AUDIOAFE_TOP_CON0, 0x00004344, 0x00004344); // bit2: afe power off, bit6: I2S power off

        if (disable_clock(MT_CG_AUDIO_AFE, "AUDIO"))
        {
            pr_err("AudDrv_Suspend_Clk_Off() disable_clock MT_CG_AUDIO_AFE fail");
        }
        ...
        if (Aud_I2S_Clk_cntr > 0)
        {
            ...
            {
                pr_err("AudDrv_Suspend_Clk_Off() disable_clock MT_CG_AUDIO_I2S fail");
            }
        }
