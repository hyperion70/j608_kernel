...
void mt_cirq_flush(void){
    ...
    mt_cirq_unmask_all();
    for (irq = 96; irq < (NR_MT_IRQ_LINE); irq+=32)
    {
        val = readl(IOMEM(((irq-96) / 32) * 4 + CIRQ_STA0));
    ...
    for (irq = 96; irq < (NR_MT_IRQ_LINE); irq+=32)
    {
        ...
        for (ix = 0; ix < 32; ix++)
        {
            if (value & (0x1)) //high trigger 
                mt_cirq_set_pol(irq+ix-96,MT_CIRQ_POL_NEG);
            else//low trigger
                mt_cirq_set_pol(irq+ix-96,MT_CIRQ_POL_POS);
...
static void mt_cirq_clone_sens(void)
{
    ...
    int ix;
    print_func();
    for (irq = 96; irq < (NR_MT_IRQ_LINE); irq+=16)
    {   
        ...
        for (ix = 0; ix < 16; ix++)
        {
            if (value & (0x2)) //edge trigger 
                mt_cirq_set_sens(irq-96+ix,MT_EDGE_SENSITIVE);
            else//level trigger
                mt_cirq_set_sens(irq-96+ix,MT_LEVEL_SENSITIVE);
            value >>= 2;
        }
...
static void mt_cirq_clone_mask(void)
{
    ...
    for (irq = 96; irq < (NR_MT_IRQ_LINE); irq+=32)
    {
        ...
        {
            if (value & (0x1)) //enable  
                mt_cirq_unmask(irq+ix-96);
            else//disable
                mt_cirq_mask(irq+ix-96);
    ...
    for (cirq_num = 0; cirq_num < MT_NR_CIRQ; cirq_num++)
    {
        ...
        if (0 == mask){
            ...
            while (__check_irq_type[irq_iter].num >= 0) {
                if (__check_irq_type[irq_iter].num == (cirq_num+96)){
