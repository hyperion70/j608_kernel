...
static unsigned int debug_settting_flag;
...
{
#ifdef ENABLE_CHIP_VER_CHECK
	sprintf(ver, "MT%04x_S%02x", get_chip_hw_ver_code(),
		(get_chip_hw_subcode() & 0xFF));
#else
	...
	case IPC_RPC_GET_SECRO_OP:
		{
			...
			if (masp_secro_en()) {
				...
				++cnt;
				img_len =
				    ((img_len + (blk_sz - 1)) >> cnt) << cnt;

				addr =
				    p_rpc_buf->buf + 4 * sizeof(unsigned int);
				tmp_data[0] =
				    masp_secro_md_get_data(md_image_post_fix
							   [md_id], addr, 0,
							   img_len);
...
static void enable_mem_access_protection(int md_id)
{
	...
	case MD_SYS1:
		...
		shr_mem_mpu_id = 3;
		rom_mem_mpu_attr =
		    SET_ACCESS_PERMISSON(FORBIDDEN, FORBIDDEN, SEC_R_NSEC_R,
					 SEC_R_NSEC_R);
		rw_mem_mpu_attr =
		    SET_ACCESS_PERMISSON(FORBIDDEN, FORBIDDEN, NO_PROTECTION,
					 FORBIDDEN);
		shr_mem_mpu_attr =
		    SET_ACCESS_PERMISSON(FORBIDDEN, FORBIDDEN, NO_PROTECTION,
					 NO_PROTECTION);
	...
	ap_mem_mpu_id = 6;
	ap_mem_mpu_attr =
	    SET_ACCESS_PERMISSON(NO_PROTECTION, FORBIDDEN, SEC_R_NSEC_R,
				 NO_PROTECTION);
	...
	emi_mpu_set_region_protection(kernel_base, (kernel_base + dram_size),
...
static void set_ap_region_protection(int md_id)
{
	...
	ap_mem_mpu_id = 6;
	ap_mem_mpu_attr =
	    SET_ACCESS_PERMISSON(NO_PROTECTION, FORBIDDEN, FORBIDDEN,
				 NO_PROTECTION);
	...
	if (masp_ccci_signfmt_verify_file
		...
		if (bypass_sec_header_offset > sec_total_len) {
			...
			CCCI_MSG_INF(md_id, "ctl",
				     ...
				     bypass_sec_header_offset, sec_total_len);
			*sec_tail_length =
			    sec_total_len - bypass_sec_header_offset;
			     ...
			     struct file *filp, struct image_info *img)
{
	...
	while (1) {
		...
		ret =
		    filp->f_op->read(filp, start, size_per_read, &filp->f_pos);
...
int cpy_check(int md_id, unsigned long start_addr, unsigned int size)
{
	...
	while (has_check < size) {
		start =
		    ioremap_nocache((start_addr + has_check), size_per_read);
		if (start <= 0) {
			CCCI_MSG_INF(md_id, "ctl",
...
static int md_dsp_irq_init(int md_id)
{
	...
	ret =
	    request_irq(irq_num, md_wdt_isr, IRQF_TRIGGER_FALLING, "MD-WDT",
			NULL);
	if (ret) {
...
static void map_md_side_register(int md_id)
{
	...
	case MD_SYS1:
		...
		md1_boot_slave_Vector =
		    (unsigned int)ioremap_nocache(0x20190000, 0x4);
		md1_boot_slave_Key =
		    (unsigned int)ioremap_nocache(0x2019379C, 0x4);
		md1_boot_slave_En =
		    (unsigned int)ioremap_nocache(0x20195488, 0x4);
		...
		if (!md1_ost_ptr)
			md1_ost_ptr =
			    ioremap_nocache(MD1_OS_TIMER_ADDR, 0x1000);
		if (!md1_mixedsys_ptr)
			md1_mixedsys_ptr =
			    ioremap_nocache(MD1_MIXEDSYS_ADDR, 0x1000);
		if (!md1_md_topsm_ptr)
			md1_md_topsm_ptr =
			    ioremap_nocache(MD1_MD_TOPSM_ADDR, 0x1000);
		if (!md1_modem_topsm_ptr)
			md1_modem_topsm_ptr =
			    ioremap_nocache(MD1_MODEM_TOPSM_ADDR, 0x1000);
#endif
		break;
