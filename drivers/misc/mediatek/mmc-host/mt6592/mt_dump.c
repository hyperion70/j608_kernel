...
static void simp_msdc_dma_stop(struct simp_msdc_host *host)
{
	...
	if(retry == 0){
		printk("############Failed to stop DMA,please check here");
	}
}
...
static unsigned int simp_init_emmc(void){
    ...
    for (i = 0; i < ARRAY_SIZE(g_freqs); i++) {
        ...
        if (g_freqs[i] <= HOST_MIN_MCLK){
            printk(KERN_EMERG "failed to init eMMC, line:%d\n", __LINE__); 
            ret = 1;
        }
    }
	...
	while(1){
        ...
        while (left) {
            ...
            if (l_count > 50000) {
                l_count = 0;
                if(print_count > 0){
                    printk(KERN_EMERG "size= %d, left= %d, done=%d.\r\n", size, left, done);
                    simp_msdc_dump_register(host);
                    print_count--;
                }
            }
...
static int simp_emmc_dump_write(unsigned char* buf, unsigned int len, unsigned int offset,unsigned int dev)
{
    ...
    if (PartInfo[l_dest_num].size < offset) {
        printk("write operation oversize!\n");
        return ret;
    }
...
#endif  
       	if (l_addr >= ((l_start_offset + PartInfo[l_dest_num].size) >> 9)) {
	...
	if (err) {
	    printk("write 512byte fail at 0x%x\n", l_addr);
	    return ret;
	}
	do{
