...
#endif
#if (5 == HOST_MAX_NUM)
	seq_printf(m, "-> MSDC[4] Zone: 0x%.8x\n", sd_debug_zone[4]);
#endif
...
#endif
#if (5 == HOST_MAX_NUM)
    seq_printf(m, "-> MSDC[4] mode<%d> size<%d>\n", drv_mode[4], dma_size[4]);
#endif
...
#endif
#if (5 == HOST_MAX_NUM)
	seq_printf(m, "-> Clokc SRC selection Host[4]<%d>\n", msdc_clock_src[4]); 
#endif
...
static int msdc_debug_proc_write(struct file *file, const char *buf, size_t count, loff_t *data)
{
	...
	if(cmd == SD_TOOL_ZONE) {
		...
		if (mode == 0) {
			...
			rw_thread = kthread_create(rwThread, (void*)(id | (mode << 4)), "msdc_rw_thread");
...
static const struct file_operations tune_flag_fops = {
    .read = msdc_tune_flag_proc_read_show,
};

static const struct file_operations msdc_tune_fops = {
    .read = msdc_tune_proc_read,
...
{   	
...
#endif
	if (prEntry) {
		pr_debug("[%s]: successfully create /proc/msdc_debug\n", __func__);
...
#endif
	if (prEntry) {
		pr_debug("[%s]: successfully create /proc/msdc_help\n", __func__);
    }else{
		pr_debug("[%s]: failed to create /proc/msdc_help\n", __func__);
    }

    prEntry = proc_create("msdc_FT", 0660, 0, &msdc_FT_fops);
	if(!prEntry)
    {
    ...
    tune = proc_create("msdc_tune", 0660, 0, &msdc_tune_fops);
    tune->gid=1001;
    tune_flag = proc_create("msdc_tune_flag", 0660, 0, &tune_flag_fops);
    tune_flag->gid=1001;
    return 0 ;
}
