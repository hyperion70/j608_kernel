       ...
       switch (apc_index){
            ...
            case 5:
                ...
                *DEVAPC0_D0_APC_5 |= permission_control << (2 * apc_bit_index);
                break;
            default:
                pr_info( "Set_Module_APC : The setting is error, please check if setting is correct or not !\n");
         }
    }
...
static int __init devapc_init(void)
{
    ...
    //devapc_device =platform_device_register_simple("devapc", 0, NULL, 0);
	//ret= IS_ERR(devapc_device);
     ...
     */
    ret = request_irq(MT_APARM_DOMAIN_IRQ_ID, (irq_handler_t)devapc_violation_irq, IRQF_TRIGGER_LOW | IRQF_SHARED, "mt_devapc", &g_devapc_ctrl);    
