...
******************************************************************************
*/
#define MT6592_AP_BT_CVSD_IRQ_LINE (252)
...
static long AudDrv_btcvsd_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case SET_BTCVSD_STATE:
        {
            ...
            {
                btsco.uTXState = arg;
                pr_debug("SET_BTCVSD_STATE set btsco.uTXState to 0x%lux \n", arg);
            }
            else //RX state
            {
                btsco.uRXState = arg;
            ...
            if (btsco.uTXState == BT_SCO_TXSTATE_IDLE && btsco.uRXState == BT_SCO_RXSTATE_IDLE)
            {
                ...
                if (disableBTirq == 0)
                {
                    disable_irq(MT6592_AP_BT_CVSD_IRQ_LINE);
                    Disable_CVSD_Wakeup();
                    disableBTirq = 1;
                }
            }
            else
            {
                if (disableBTirq == 1)
                {
                    ...
                    enable_irq(MT6592_AP_BT_CVSD_IRQ_LINE);
                    Enable_CVSD_Wakeup();
                    disableBTirq = 0;
                }
            }
...
static int AudDrv_BTCVSD_IRQ_handler(void)
{
    ...
    //pr_debug("AudDrv_BTCVSD_IRQ_handler uPacketLength=%d, uPacketNumber=%d, uBufferCount_TX=%d, uBufferCount_TX=%d \n", uPacketLength, uPacketNumber, uBufferCount_TX, uBufferCount_RX);
...
static int AudDrv_btcvsd_probe(struct platform_device *dev)
{
    ...
    ret = request_irq(MT6592_AP_BT_CVSD_IRQ_LINE, (irq_handler_t)AudDrv_BTCVSD_IRQ_handler, IRQF_TRIGGER_LOW/*IRQF_TRIGGER_FALLING*/, "BTCVSD_ISR_Handle", dev);
    if (ret < 0)
    {
        pr_debug("AudDrv_btcvsd_probe request_irq MT6592_AP_BT_CVSD_IRQ_LINE Fail \n");
    }
