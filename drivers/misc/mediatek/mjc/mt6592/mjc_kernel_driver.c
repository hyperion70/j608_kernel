...
static long mjc_ioctl(struct file *pfile, unsigned int u4cmd, unsigned long u4arg)
{
	...
	switch(u4cmd)
    {
		...
		case MJC_READ_REG:
		{
            ...
            rReadReg.val = (*(volatile unsigned int*)i4Add) & rReadReg.mask;

            MJCDBG("read 0x%lx (0x%x)= 0x%x (0x%x)\n", rReadReg.reg, i4Add, rReadReg.val, rReadReg.mask);
		...
		case MJC_WRITE_REG:
		{
            ...
            {
                MJCMSG("[ERROR] mjc_ioctl() MJC_WRITE_REG, addr invalid, addr min=0x%x, max=0x%x, addr=0x%lx \n",
                    MJC_REG_PA,
                    (MJC_REG_PA+MJC_REG_REGION),
                    rWriteReg.reg);
                return -1;
