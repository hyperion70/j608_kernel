...
#define M4U_ASSERT(x) if(!(x)){pr_err("M4U assert fail, file:%s, line:%d", __FILE__, __LINE__);}

//#define MTK_M4U_DBG
#ifdef MTK_M4U_DBG
#define M4UDBG(string, args...)	pr_info("M4U [pid=%d]"string,current->tgid,##args);
...
static int MTK_M4U_release(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    if(pNode->isM4uDrvConstruct==0 || pNode->isM4uDrvDeconstruct==0)
    {
        M4UMSG("warning on close: construct=%d, deconstruct=%d, open_pid=%d, cur_pid=%d\n",
            ...
            pNode->open_pid, current->pid);
        M4UMSG("open->tgid=%d, cur->tgid=%d, cur->mm=0x%p.\n",
								...
								unsigned long a_Param)
{
    ...
    unsigned int regVal = COM_ReadReg32(REG_SMI_SECUR_CON(1));
    ret = copy_from_user(&ModuleID, (void*)a_Param , sizeof(unsigned int));
    if (ModuleID == MDP_ROTVO && (regVal & 0xf == 0x6)) {
        m4u_aee_print("MTI_M4U_ioctl, ModuleID=%d, regVal=%s\n", ModuleID, regVal);
        return -EFAULT;
    }
    ...
    switch(a_Command)
    {
        ...
        case MTK_M4U_T_REGISTER_BUFFER:
        {
            ...
            pMvaInfo = (mva_info_t*)kmalloc(sizeof(mva_info_t), GFP_KERNEL);
            if(pMvaInfo==NULL)
            {
...
static void m4u_profile_init(void)
{
    ...
    MMProfileEnableEvent(M4U_MMP_Events[PROFILE_CACHE_FLUSH_ALL], 1);
}
...
int m4u_dump_debug_registers(void)
{
    int i;
    M4UMSG("============== %s ==============>\n", __FUNCTION__);
    ...
    else if(BufAddr<PAGE_OFFSET)  // from user space
    {
        ...
        else 
        {
            ...
            if((vma->vm_flags) & VM_PFNMAP)
            {
                ...
                {
                    M4UMSG("error: page_num=%d,module=%s, va=0x%x, size=0x%x, vm_flag=0x%lu.\n", 
                        page_num, m4u_get_module_name(eModuleID), BufAddr, BufSize, vma->vm_flags);                    	
                    M4UMSG("but vma is: vm_start=0x%lu, vm_end=0x%lu.\n", vma->vm_start, vma->vm_end);
                ...
                M4UINFO("alloc_mva VM_PFNMAP module=%s, va=0x%x, size=0x%x, vm_flag=0x%lu.\n", 
...
*/
    ...
    for(i=0; i<7; i++)
        COM_WriteReg32(REG_SMI_SECUR_CON(i), 0x77777777);
