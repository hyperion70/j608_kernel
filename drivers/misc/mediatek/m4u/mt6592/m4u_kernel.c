...
#ifdef MTK_M4U_DBG
#define M4UDBG(string, args...)	pr_debug("M4U_K"string, ##args);
#else
...
#define MTK_M4U_MSG
#ifdef MTK_M4U_MSG
#define M4UMSG(string, args...)	pr_info("M4U_K [pid=%d]"string,current->tgid,##args);
#define M4UWARN(string, args...) pr_warn("M4U_K [pid=%d]"string,current->tgid,##args);
#else
...
#define M4UERR(string, args...) do { \
	pr_err("M4U_K [pid=%d]error_assert_fail: "string,current->tgid,##args);  \
				...
				do {
					if (trylock_page(page)) {
                        ...
                        //make sure hw pte is not 0
                        {
                            ...
                            if(i==3000)
                                M4UMSG("error: cannot handle_mm_fault to get hw pte: va=0x%lu.\n", start);
                        }
