	...
	if (cl_dev_adp_cpu_state_active == 1)
	{
		...
		if (!triggered)
		{
			...
			else
			{
				...
				mtktscpu_dprintk("_adaptive_power Tp %ld, Tc %ld, Pt %d\n", prev_temp, curr_temp, total_power);
				...
				unsigned long state)
{
	...
	mtktscpu_dprintk("adp_cpu_set_cur_state[%d] =%lu, ttj=%d\n", (cdev->type[13] - '0'), state, ttj);
...
static int mtktscpu_read_opp(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "%d,%d,%d,%d\n",
                    ...
                    //(int)((NULL == mtk_thermal_get_gpu_loading_fp) ? 0 : mtk_thermal_get_gpu_loading_fp()),
                    (int)gpu_loading,
                    (int)mt_socfreq_get_cur_gpufreq());
...
static int mtktscpu_read_temperature_info(struct seq_file *m, void *v)
{
    seq_printf(m, "current temp:%d\n", read_curr_temp);
  	seq_printf(m, "Index17:0x%x\n", temp0_0xF020617C);
  	seq_printf(m, "Index16:0x%x\n", temp1_0xF0206178);
...
static int mtktscpu_talking_flag_read(struct seq_file *m, void *v)
{
...
static ssize_t mtktscpu_talking_flag_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char desc[32];
	int lv_talking_flag;
	int len = 0;
...
static int mtktscpu_set_temperature_read(struct seq_file *m, void *v)
{
...
static ssize_t mtktscpu_set_temperature_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char desc[32];
	int lv_tempe_switch;
	int len = 0;
...
static int mtktscpu_read_log(struct seq_file *m, void *v)
{
...
static int mtktscpu_read_cal(struct seq_file *m, void *v)
{
...
static int mtktscpu_read(struct seq_file *m, void *v)
{
	int i;
...
static ssize_t mtktscpu_write_log(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char desc[32];
	int log_switch;
	int len = 0;
...
#if CPT_ADAPTIVE_AP_COOLER
static int mtktscpu_read_dtm_setting(struct seq_file *m, void *v)
{
	int i;
...
static ssize_t mtktscpu_write_dtm_setting(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char desc[128];
	//char arg_name[32] = {0};
	//int arg_val = 0;
...
static int mtktscpu_read_gpu_threshold(struct seq_file *m, void *v)
{
//	int i;
...
static ssize_t mtktscpu_write_gpu_threshold(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char desc[128];
	int len = 0;
...
#if MTKTSCPU_FAST_POLLING
static int mtktscpu_read_fastpoll(struct seq_file *m, void *v)
{
//	int i;
...
static ssize_t mtktscpu_write_fastpoll(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char desc[128];
	int len = 0;
...
static ssize_t mtktscpu_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
...
{
    return single_open(file, mtktscpu_read_opp, NULL);
}
...
{
    return single_open(file, mtktscpu_read_log, NULL);
}
...
{
    return single_open(file, mtktscpu_read, NULL);
}
...
{
    return single_open(file, mtktscpu_read_cal, NULL);
}
...
{
    return single_open(file, mtktscpu_read_temperature_info, NULL);
}
...
{
    return single_open(file, mtktscpu_set_temperature_read, NULL);
}
static const struct file_operations mtktscpu_set_temperature_fops = {
    ...
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mtktscpu_set_temperature_write,
    .release = single_release,
};
...
{
    return single_open(file, mtktscpu_talking_flag_read, NULL);
}
static const struct file_operations mtktscpu_talking_flag_fops = {
    ...
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mtktscpu_talking_flag_write,
    .release = single_release,
};
...
{
    return single_open(file, mtktscpu_read_dtm_setting, NULL);
}
...
{
    return single_open(file, mtktscpu_read_gpu_threshold, NULL);
}
static const struct file_operations mtktscpu_gpu_threshold_fops = {
    ...
    .read = seq_read,
    .llseek = seq_lseek,
    .write = mtktscpu_write_gpu_threshold,
    .release = single_release,
};
#endif
...
{
    return single_open(file, mtktscpu_read_fastpoll, NULL);
}
...
static int __init mtktscpu_init(void)
{
	...
	if (!mtktscpu_dir)
	{
		mtktscpu_printk("[%s]: mkdir /proc/driver/thermal failed\n", __func__);
	}
	else
	{
