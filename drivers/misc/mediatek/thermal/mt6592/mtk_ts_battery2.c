...
do {                                    \
	if (mtktsbattery2_debug_log) {                \
		pr_notice("Power/battery2_Thermal" fmt, ##args); \
	}                                   \
} while(0)
...
//NTCG104EF104F(100K)
BATT_TEMPERATURE Batt_Temperature_Table6[] = {
    {-20,1135000},
    {-15,837800},
    {-10,624100},
    { -5,469100},
...
static INT16 BattThermistorConverTemp(INT32 Res)
{
    ...
    #if 0
    mtktsbattery2_dprintk("BattThermistorConverTemp() : TBatt_Value = %d\n",TBatt_Value);
    mtktsbattery2_dprintk("BattThermistorConverTemp() : Res = %d\n",Res);
    mtktsbattery2_dprintk("BattThermistorConverTemp() : RES1 = %d\n",RES1);
    mtktsbattery2_dprintk("BattThermistorConverTemp() : RES2 = %d\n",RES2);
    mtktsbattery2_dprintk("BattThermistorConverTemp() : TMP1 = %d\n",TMP1);
    mtktsbattery2_dprintk("BattThermistorConverTemp() : TMP2 = %d\n",TMP2);
    #endif
...
//	int i=0;
	switch(table_num)
    {
		...
		case 6://NTCG104EF104F
                mtktsbattery2_copy_table(Batt_Temperature_Table,Batt_Temperature_Table6);
			break;
        default://BAT_NTC_10
	            mtktsbattery2_copy_table(Batt_Temperature_Table,Batt_Temperature_Table4);
            break;
