...
do {									\
	if (mtktsbuck_debug_log) {				\
		pr_notice("Power/buck_Thermal" fmt, ##args); \
	}								   \
} while(0)
...
static int mtktsbuck_read(struct seq_file *m, void *v)
{
	seq_printf(m, "[ mtktsbuck_read] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,\n\
...
static ssize_t mtktsbuck_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
...
{
	return single_open(file, mtktsbuck_read, NULL);
}
...
static int __init mtktsbuck_init(void)
{
    ...
    mtktsbuck_dir = mtk_thermal_get_proc_drv_therm_dir_entry();
    if (!mtktsbuck_dir)
    {
        mtktsbuck_dprintk("[%s]: mkdir /proc/driver/thermal failed\n", __func__);
    }
    else
    {
        entry = proc_create("tzbuck",  S_IRUGO | S_IWUSR | S_IWGRP, mtktsbuck_dir, &tsbuck_fops);
        if (entry) {
