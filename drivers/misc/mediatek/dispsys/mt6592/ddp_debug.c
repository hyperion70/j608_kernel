...
//  Debug Options
// ---------------------------------------------------------------------------
static int slt_result2 =0;// 1 success,0 Error!
...
unsigned int gOvlRdmaFifoCtrl = 0x400000;
unsigned int gOvlReq = 0x40000000;
...
static char STR_HELP[] =
    ...
    "       mmp\n"
    "\n"
    "       debug, 6dump_layer, 9info, 10reg, 11~15rdma_ultra, 16ovl_fifo, 17~20greq, 21base\n"
    "\n"    
    "       dump_reg:moduleID\n"
    "\n"    
    "       dpfd_ut1:channel\n"
...
extern void disp_path_reset();
extern void disp_path_ovl_reset();
static void process_dbg_opt(const char *opt)
{
    ...
    else if (0 == strncmp(opt, "debug:", 6))
    {
        ...
        {
           //ddp_mem_test();
           if (DISP_REG_GET(0xf0000040)&0x0ff != 1) {
        ...
        else if(enable==16)
        {
            ...
            gOvlRdmaFifoCtrl= value; 
            sprintf(buf, "gOvlRdmaFifoCtrl = 0x%x, DISP_REG_OVL_RDMA0_FIFO_CTRL=0x%x\n",
                gOvlRdmaFifoCtrl, DISP_REG_GET(DISP_REG_OVL_RDMA0_FIFO_CTRL)); 
            DISP_MSG("ddp debug set gOvlRdmaFifoCtrl = 0x%x, DISP_REG_OVL_RDMA0_FIFO_CTRL=0x%x\n",
                gOvlRdmaFifoCtrl, DISP_REG_GET(DISP_REG_OVL_RDMA0_FIFO_CTRL));
        }
        else if((enable>=17)&&(enable<=20))
        {
            gOvlReq = ((enable-16)*2)<<28; 
            sprintf(buf, "gOvlReq = 0x%x, DISP_REG_OVL_DATAPATH_CON=0x%x\n",
                gOvlReq, DISP_REG_GET(DISP_REG_OVL_DATAPATH_CON)); 
            DISP_MSG("ddp debug set gOvlReq = 0x%x, DISP_REG_OVL_DATAPATH_CON=0x%x\n",
                gOvlReq, DISP_REG_GET(DISP_REG_OVL_DATAPATH_CON));
        }
        else if(enable==21)
        {
            ...
            disp_path_reset();
            sprintf(buf, "disp_path_reset done. \n");
        }
        else if(enable==23)
        {
            disp_path_ovl_reset();
            sprintf(buf, "disp_path_ovl_reset done. \n");
        }
    ...
    else if (0 == strncmp(opt, "ddp_suspend", 11))
	{
		...
		DISP_MSG("SLT DDP is_suspend %d\n",is_suspend);
	}
	else if (0 == strncmp(opt, "ddp_slt:", 8))
	{		
	    if (0 == strncmp(opt + 8, "1", 1))
        {
        	ddp_clock_on();
         	int result = slt_test_stream1();
			if(result ==0)
			{
				slt_result2 =1;
			}else
			...
			ddp_clock_off();
			DISP_MSG("SLT DDP TEST1 result %d\n",slt_result2);
        }
	}
    else
    {
...
static int Ddp_Read_State(char *buf, char **start, off_t offset, int count , int *eof, void *data)
{
	int len = 0;
    printk("Ddp SLT Result %d \n",slt_result2);
    len += sprintf(buf + len , "%d", slt_result2);
	return len;
}
static int Ddp_Suspend_State(char *buf, char **start, off_t offset, int count , int *eof, void *data)
{
	int len = 0;
    printk("Ddp SLT Result %d \n",is_suspend);
    len += sprintf(buf + len , "%d", is_suspend);
	return len;
}

static int Ddp_Write_State(struct file *file, const char *buffer,
                unsigned long count, void *data)
{
    printk("Ddp_Write_State\n");
	char desc[32]; 
	int len = 0;
	int num =0;
	...
	if (sscanf(desc, "%d", &num) == 1) 
	{
		slt_result2 = num;
	}
	printk("[%s]result is %d\n", __func__, slt_result2);
	return count;
}
...
char* ddp_get_fmt_name(DISP_MODULE_ENUM module, unsigned int fmt)
{
   ...
   else if(module==DISP_MODULE_RDMA || module==DISP_MODULE_RDMA1) // todo: confirm with designers
   {
       switch(fmt)
       {
           case 0:  return "yuyv";
