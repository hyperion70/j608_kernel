...
static unsigned int hdmi_query_buf_mva(unsigned int idx)
{
    ...
    list_for_each_entry(buf, &HDMI_Buffer_List, list)
    {
        if (buf->idx == idx)
        {
            HDMI_LOG("query buf list=%p, idx=%d, mva=0x%08x\n", buf, idx, buf->mva);
            return buf;
        }
    }
            ...
            if (!list_empty(&HDMI_Buffer_List))
            {
                        ...
                        || (hdmi_rdma_address_config(true, pBuffList->buffer_info) < 0))
                {
                    ...
                    if ((pBuffList != NULL) && (pBuffList->buf_state == insert_new))
                    {
                        ...
                        HDMI_LOG(" buffer config error to configed %p, state %d, idx %d\n", pBuffList, pBuffList->buf_state,  pBuffList->idx);
                    }
                }
                else
                {
...
//          int buf_sequence = 1;
        ...
        else
        {
            ...
            while (!list_empty(&HDMI_Buffer_List))
            {
                ...
                {
                    HDMI_LOG("delete list %p-->buffer %p \n", pBuffList, pBuffList->buffer_info.src_phy_addr);
                }
...
static void _rdma1_irq_handler(unsigned int param)
{
    ...
    if (param & 0x20) // taget line 0x20
    {
        ...
        atomic_set(&hdmi_rdma_config_event, 0x20);
...
int hdmi_rdma_address_config(bool enable, hdmi_video_buffer_info buffer_info)
{
    ...
    if (enable)
    {
        ...
        else if (buffer_info.src_fmt == MTK_FB_FORMAT_ABGR8888)
        {
            rdmaInputsize = 4;
            rdmaInputFormat = 36;
        }
...
static long hdmi_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        {
            HDMI_LOG("MTK_HDMI_USBOTG_STATUS, arg=%lu, enable %d\n", arg, p->is_enabled);
        ...
        {
            HDMI_LOG("video resolution configuration, arg=%lu, %ld\n", arg, hdmi_reschange);
        ...
        case MTK_HDMI_PREPARE_BUFFER:
        {
            ...
            if (p->is_clock_on)
            {
                ...
                if (!ion_client)
                {
                    ...
                    if (!ion_client)
                    {
                        HDMI_LOG(": get ion_client fail %p\n", ion_client);
                        r = -EFAULT;
                        up(&hdmi_update_mutex);
                        break;
                    }
        ...
        case MTK_HDMI_SCREEN_CAPTURE:
        {
            ...
            {
                HDMI_LOG("screen_capture done,is_enabled, wait_times(%d)\n", capture_wait_times);
            }
        ...
        {  
            ...
            pv_get_info.ui4_ntsc_resolution = 0;
            pv_get_info.ui4_pal_resolution =  SINK_720P60 | SINK_1080P30 | SINK_480P;
