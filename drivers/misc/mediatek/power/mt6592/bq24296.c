				  ...
				  kal_uint8 SHIFT)
{
	...
	pr_notice("[bq24296_read_interface] Reg[%x]=0x%x\n", RegNum, bq24296_reg);

	bq24296_reg &= (MASK << SHIFT);
	*val = (bq24296_reg >> SHIFT);
	pr_notice("[bq24296_read_interface] Val=0x%x\n", *val);
				    ...
				    kal_uint8 SHIFT)
{
	...
	ret = bq24296_read_byte(RegNum, &bq24296_reg);
	/* pr_notice("[bq24296_config_interface] Reg[%x]=0x%x\n", RegNum, bq24296_reg); */

	bq24296_reg &= ~(MASK << SHIFT);
	bq24296_reg |= (val << SHIFT);

	ret = bq24296_write_byte(RegNum, bq24296_reg);
	/* pr_notice("[bq24296_config_interface] Write Reg[%x]=0x%x\n", RegNum, bq24296_reg); */

	/* Check */
	/* bq24296_read_byte(RegNum, &bq24296_reg); */
	/* pr_notice("[bq24296_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24296_reg); */
...
void bq24296_dump_register(void)
{
	...
	for (i = 0; i < bq24296_REG_NUM; i++) {
		...
		pr_notice("[bq24296_dump_register] Reg[0x%X]=0x%X\n", i, bq24296_reg[i]);
	}
}
				    ...
				    const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		pr_notice("[store_bq24296_access] buf is %s and size is %d\n", buf, size);
