...
{
    pr_notice( "[register_low_battery_notify] start\n");

    lbcb_tb[prio_val].lbcb = low_battery_callback;

    pr_notice( "[register_low_battery_notify] prio_val=%d\n",prio_val);
}
    ...
    else
    {
        for(i=0 ; i<LBCB_NUM ; i++)
        {
            if(lbcb_tb[i].lbcb != NULL)
            {
                ...
                pr_notice( "[exec_low_battery_callback] prio_val=%d,low_battery=%d\n",i,low_battery_level);
            }
        }
    }
}
...
void low_battery_protect_init(void)
{
    ...
    pr_notice( "[low_battery_protect_init] Done\n");
}
...
void bat_l_int_handler(void)
{
    ...
    if(g_low_battery_level==1)
        ...
        pr_notice( "[bat_l_int_handler]err,g_low_battery_level=%d\n", g_low_battery_level);
    ...
    pr_notice( "Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
...
{
    pr_notice("no kpd_pwrkey_pmic_handler at FPGA\n");
}
#else
...
#ifdef CONFIG_MTK_FAN5402_SUPPORT
...
{
    pr_notice("no kpd_pmic_rstkey_handler at FPGA\n");
}
#else
...
{
    pr_notice( "[mt_pmic_eint_irq] receive interrupt\n");
...
{
    pr_notice( "[PMIC_EINT_SETTING] start: CUST_EINT_PMIC_NUM=%d\n",CUST_EINT_MT6322_PMIC_NUM);
    ...
    pr_notice( "[CUST_EINT] CUST_EINT_PMIC_NUM=%d\n", CUST_EINT_MT6322_PMIC_NUM);
...
{
    pr_notice( "[show_low_battery_protect_ut] g_low_battery_level=%d\n", g_low_battery_level);
    return sprintf(buf, "%u\n", g_low_battery_level);
}
static ssize_t store_low_battery_protect_ut(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_notice( "[store_low_battery_protect_ut] \n");

    if(buf != NULL && size != 0)
    {
        pr_notice( "[store_low_battery_protect_ut] buf is %s and size is %zu \n",buf,size);
        val = simple_strtoul(buf,&pvalue,16);
        if(val<=2)
        {
            pr_notice( "[store_low_battery_protect_ut] your input is %d\n", val);
            exec_low_battery_callback(val);
        }
        else
        {
            pr_notice( "[store_low_battery_protect_ut] wrong number (%d)\n", val);
        }
    }
    return size;
}
...
{
    pr_notice( "[show_low_battery_protect_stop] g_low_battery_stop=%d\n", g_low_battery_stop);
    return sprintf(buf, "%u\n", g_low_battery_stop);
}
static ssize_t store_low_battery_protect_stop(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_notice( "[store_low_battery_protect_stop] \n");

    if(buf != NULL && size != 0)
    {
        pr_notice( "[store_low_battery_protect_stop] buf is %s and size is %zu \n",buf,size);
        val = simple_strtoul(buf,&pvalue,16);
        if( (val!=0) && (val!=1) )
            val=0;
        g_low_battery_stop = val;
        pr_notice( "[store_low_battery_protect_stop] g_low_battery_stop=%d\n", g_low_battery_stop);
    }
    return size;
}
...
{
    pr_notice( "[show_low_battery_protect_level] g_low_battery_level=%d\n", g_low_battery_level);
    return sprintf(buf, "%u\n", g_low_battery_level);
}
static ssize_t store_low_battery_protect_level(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    pr_notice( "[store_low_battery_protect_level] g_low_battery_level=%d\n", g_low_battery_level);
...
void pmic_ldo_vol_sel(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt)
{
    ...
    else if(powerId == MT6322_POWER_LDO_VIO28)      { pr_notice( "[dct_pmic_VIO28_enable] No vlotage can setting!\n"); }
    else if(powerId == MT6322_POWER_LDO_VUSB)       { pr_notice( "[dct_pmic_VUSB_enable] No vlotage can setting!\n"); }
    ...
    else if(powerId == MT6322_POWER_LDO_VCN_1V8)    {pr_notice( "[dct_pmic_VCN_1V8_enable] No vlotage can setting!\n");}
    ...
    else if(powerId == MT6322_POWER_LDO_VRTC)       { pr_notice( "[dct_pmic_VRTC_enable] No vlotage can setting!\n");}
    ...
    else if(powerId == MT6322_POWER_LDO_VRF18)      { pr_notice( "[dct_pmic_VRF18_enable] No vlotage can setting!\n");}
    else if(powerId == MT6322_POWER_LDO_VIO18)      { pr_notice( "[dct_pmic_VIO18_enable] No vlotage can setting!\n");}
    else if(powerId == MT6322_POWER_LDO_VCAMD)      { dct_pmic_VCAMD_sel(powerVolt); }
    else if(powerId == MT6322_POWER_LDO_VCAM_IO)    { pr_notice( "[dct_pmic_VCAM_IO_enable] No vlotage can setting!\n");}
    else if(powerId == MT6322_POWER_LDO_VTCXO)      { pr_notice( "[dct_pmic_VTCXO_enable] No vlotage can setting!\n");}
    else if(powerId == MT6322_POWER_LDO_VA)         { pr_notice( "[dct_pmic_VA_enable] No vlotage can setting!\n"); }
    ...
    else if(powerId == MT6322_POWER_LDO_VCN28)      { pr_notice( "[dct_pmic_VCN28_enable] No vlotage can setting!\n"); }
...
int ext_buck_vosel(unsigned long val)
{
    ...
    if(is_ext_buck_sw_ready()==1)
    {
        ...
        {
            pr_notice( "[ext_buck_vosel] no ext buck ?!\n");
        }
    }
    else
    {
        pr_notice( "[ext_buck_vosel] ext buck sw not ready\n");
    }
...
void ext_buck_init_for_platform(void)
{
    ...
    if( (g_vproc_vsel_gpio_number!=0) && (g_vproc_en_gpio_number!=0) )
    {
        ...
        pr_notice( "[ext_buck_init_for_platform] turn off internal VPROC.\n");
    }
    else
    {
        #ifdef CONFIG_IS_VCORE_USE_6333VCORE
            pr_notice( "[ext_buck_init_for_platform] CONFIG_IS_VCORE_USE_6333VCORE=y, nothing for internal VPROC.\n");
            //add setting id needed
        #else
            pr_notice( "[ext_buck_init_for_platform] CONFIG_IS_VCORE_USE_6333VCORE=n, nothing for internal VPROC.\n");
            //add setting id needed
        #endif
    }
}
...
void ext_buck_init(void)
{
    ...
    else
    {
        ...
        pr_notice( "[ext_buck_init] no ext buck\n");
    }
    ...
    pr_notice( "[low bat protect] Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
        AUXADC_CON6, upmu_get_reg_value(AUXADC_CON6),
        AUXADC_CON5, upmu_get_reg_value(AUXADC_CON5),
        INT_CON0, upmu_get_reg_value(INT_CON0)
        );
...
void ext_buck_pre_init(void)
{
    ...
    if(g_vproc_vsel_gpio_number!=0)
    {
        if(g_vproc_en_gpio_number!=0)
        {
            ...
            pr_notice( "[ext_buck_pre_init] Reg[0x220]=0x%x\n", upmu_get_reg_value(0x220));

            ext_buck_vproc_en(1);
            pr_notice( "[ext_buck_pre_init] ext_buck_vproc_en(1).\n");
        }
        else
        {
            pr_notice( "[ext_buck_pre_init] g_vproc_en_gpio_number==0\n");
        }
    }
    else
    {
        pr_notice( "[ext_buck_pre_init] g_vproc_vsel_gpio_number==0\n");
    }
#else
    if(g_vproc_vsel_gpio_number!=0)
    {
        ext_buck_vproc_vsel(1);
        pr_notice( "[ext_buck_pre_init] ext_buck_vproc_vsel(1)\n");
    }
...
void PMIC_INIT_SETTING_V1_hpm(void)
{
    ...
    pr_notice( "[PMIC_INIT_SETTING_V1_hpm] PMIC Chip = %x\n",chip_version);
    pr_notice( "[PMIC_INIT_SETTING_V1_hpm] 20130902\n");
...
{
    pr_notice( "MT6323_POWER_LDO_VIO28     =%d\n", MT6323_POWER_LDO_VIO28     );
    pr_notice( "MT6323_POWER_LDO_VUSB      =%d\n", MT6323_POWER_LDO_VUSB      );
    pr_notice( "MT6323_POWER_LDO_VMC       =%d\n", MT6323_POWER_LDO_VMC       );
    pr_notice( "MT6323_POWER_LDO_VMCH      =%d\n", MT6323_POWER_LDO_VMCH      );
    pr_notice( "MT6323_POWER_LDO_VEMC_3V3  =%d\n", MT6323_POWER_LDO_VEMC_3V3  );
    pr_notice( "MT6323_POWER_LDO_VGP1      =%d\n", MT6323_POWER_LDO_VGP1      );
    pr_notice( "MT6323_POWER_LDO_VGP2      =%d\n", MT6323_POWER_LDO_VGP2      );
    pr_notice( "MT6323_POWER_LDO_VGP3      =%d\n", MT6323_POWER_LDO_VGP3      );
    pr_notice( "MT6323_POWER_LDO_VCN_1V8   =%d\n", MT6323_POWER_LDO_VCN_1V8   );
    pr_notice( "MT6323_POWER_LDO_VSIM1     =%d\n", MT6323_POWER_LDO_VSIM1     );
    pr_notice( "MT6323_POWER_LDO_VSIM2     =%d\n", MT6323_POWER_LDO_VSIM2     );
    pr_notice( "MT6323_POWER_LDO_VRTC      =%d\n", MT6323_POWER_LDO_VRTC      );
    pr_notice( "MT6323_POWER_LDO_VCAM_AF   =%d\n", MT6323_POWER_LDO_VCAM_AF   );
    pr_notice( "MT6323_POWER_LDO_VIBR      =%d\n", MT6323_POWER_LDO_VIBR      );
    pr_notice( "MT6323_POWER_LDO_VM        =%d\n", MT6323_POWER_LDO_VM        );
    pr_notice( "MT6323_POWER_LDO_VRF18     =%d\n", MT6323_POWER_LDO_VRF18     );
    pr_notice( "MT6323_POWER_LDO_VIO18     =%d\n", MT6323_POWER_LDO_VIO18     );
    pr_notice( "MT6323_POWER_LDO_VCAMD     =%d\n", MT6323_POWER_LDO_VCAMD     );
    pr_notice( "MT6323_POWER_LDO_VCAM_IO   =%d\n", MT6323_POWER_LDO_VCAM_IO   );
    pr_notice( "MT6323_POWER_LDO_VTCXO     =%d\n", MT6323_POWER_LDO_VTCXO     );
    pr_notice( "MT6323_POWER_LDO_VA        =%d\n", MT6323_POWER_LDO_VA        );
    pr_notice( "MT6323_POWER_LDO_VCAMA     =%d\n", MT6323_POWER_LDO_VCAMA     );
    pr_notice( "MT6323_POWER_LDO_VCN33_BT  =%d\n", MT6323_POWER_LDO_VCN33_BT  );
    pr_notice( "MT6323_POWER_LDO_VCN33_WIFI=%d\n", MT6323_POWER_LDO_VCN33_WIFI);
    pr_notice( "MT6323_POWER_LDO_VCN28     =%d\n", MT6323_POWER_LDO_VCN28     );

    pr_notice( "MT6322_POWER_LDO_VIO28     =%d\n", MT6322_POWER_LDO_VIO28     );
    pr_notice( "MT6322_POWER_LDO_VUSB      =%d\n", MT6322_POWER_LDO_VUSB      );
    pr_notice( "MT6322_POWER_LDO_VMC       =%d\n", MT6322_POWER_LDO_VMC       );
    pr_notice( "MT6322_POWER_LDO_VMCH      =%d\n", MT6322_POWER_LDO_VMCH      );
    pr_notice( "MT6322_POWER_LDO_VEMC_3V3  =%d\n", MT6322_POWER_LDO_VEMC_3V3  );
    pr_notice( "MT6322_POWER_LDO_VGP1      =%d\n", MT6322_POWER_LDO_VGP1      );
    pr_notice( "MT6322_POWER_LDO_VGP2      =%d\n", MT6322_POWER_LDO_VGP2      );
    pr_notice( "MT6322_POWER_LDO_VGP3      =%d\n", MT6322_POWER_LDO_VGP3      );
    pr_notice( "MT6322_POWER_LDO_VCN_1V8   =%d\n", MT6322_POWER_LDO_VCN_1V8   );
    pr_notice( "MT6322_POWER_LDO_VSIM1     =%d\n", MT6322_POWER_LDO_VSIM1     );
    pr_notice( "MT6322_POWER_LDO_VSIM2     =%d\n", MT6322_POWER_LDO_VSIM2     );
    pr_notice( "MT6322_POWER_LDO_VRTC      =%d\n", MT6322_POWER_LDO_VRTC      );
    pr_notice( "MT6322_POWER_LDO_VCAM_AF   =%d\n", MT6322_POWER_LDO_VCAM_AF   );
    pr_notice( "MT6322_POWER_LDO_VIBR      =%d\n", MT6322_POWER_LDO_VIBR      );
    pr_notice( "MT6322_POWER_LDO_VM        =%d\n", MT6322_POWER_LDO_VM        );
    pr_notice( "MT6322_POWER_LDO_VRF18     =%d\n", MT6322_POWER_LDO_VRF18     );
    pr_notice( "MT6322_POWER_LDO_VIO18     =%d\n", MT6322_POWER_LDO_VIO18     );
    pr_notice( "MT6322_POWER_LDO_VCAMD     =%d\n", MT6322_POWER_LDO_VCAMD     );
    pr_notice( "MT6322_POWER_LDO_VCAM_IO   =%d\n", MT6322_POWER_LDO_VCAM_IO   );
    pr_notice( "MT6322_POWER_LDO_VTCXO     =%d\n", MT6322_POWER_LDO_VTCXO     );
    pr_notice( "MT6322_POWER_LDO_VA        =%d\n", MT6322_POWER_LDO_VA        );
    pr_notice( "MT6322_POWER_LDO_VCAMA     =%d\n", MT6322_POWER_LDO_VCAMA     );
    pr_notice( "MT6322_POWER_LDO_VCN33_BT  =%d\n", MT6322_POWER_LDO_VCN33_BT  );
    pr_notice( "MT6322_POWER_LDO_VCN33_WIFI=%d\n", MT6322_POWER_LDO_VCN33_WIFI);
    pr_notice( "MT6322_POWER_LDO_VCN28     =%d\n", MT6322_POWER_LDO_VCN28     );

    pr_notice( "MT6323_POWER_LDO_DEFAULT   =%d\n", MT6323_POWER_LDO_DEFAULT   );
    pr_notice( "MT6322_POWER_LDO_DEFAULT   =%d\n", MT6322_POWER_LDO_DEFAULT   );
    pr_notice( "MT65XX_POWER_COUNT_END     =%d\n", MT65XX_POWER_COUNT_END     );
}
...
static int pmic_mt_probe(struct platform_device *dev)
{
    ...
    pr_notice( "******** MT pmic driver probe!! ********\n" );

    //get PMIC CID
    ret_val=upmu_get_cid();
    pr_notice( "MT PMIC CID=0x%x\n", ret_val );
    ...
    pr_notice( "[PMIC_INIT_SETTING_V1_hpm] Done\n");
    ...
    if (IS_ERR(pmic_thread_handle))
    {
        ...
        pr_notice( "[pmic_thread_kthread] creation fails\n");
    }
    else
    {
        wake_up_process(pmic_thread_handle);
        pr_notice( "[pmic_thread_kthread] kthread_create Done\n");
    }

#if defined(CONFIG_MTK_FPGA)
    pr_notice( "[PMIC_EINT_SETTING] disable when CONFIG_MTK_FPGA\n");
#else
    //PMIC Interrupt Service
    PMIC_EINT_SETTING();
    ...
    pr_notice( "[PMIC] VTCXO control by SRCLKEM due to MTK_ENABLE_MD5, Reg[0x402]=0x%x\n",
...
{
    pr_notice( "******** MT pmic driver remove!! ********\n" );
...
{
    pr_notice( "******** MT pmic driver resume!! ********\n" );
    ...
    pr_notice( "[low bat protect] Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, g_low_battery_level=%d\n",
        AUXADC_CON6, upmu_get_reg_value(AUXADC_CON6),
        AUXADC_CON5, upmu_get_reg_value(AUXADC_CON5),
        INT_CON0, upmu_get_reg_value(INT_CON0),
        g_low_battery_level
...
{
	pr_notice( "******** MT pmic driver early suspend!! ********\n" );
	upmu_set_rg_vref18_enb(0);
	//upmu_set_rg_adc_deci_gdly(0);
	upmu_set_rg_clksq_en_aux(1);
	upmu_set_rg_aud26m_div4_ck_pdn(0);
...
{
	pr_notice( "******** MT pmic driver early resume!! ********\n" );
	upmu_set_rg_vref18_enb(0);
	//upmu_set_rg_adc_deci_gdly(0);
	upmu_set_rg_clksq_en_aux(1);
	upmu_set_rg_aud26m_div4_ck_pdn(0);
...
static int __init pmic_mt_init(void)
{
    ...
    if (ret) {
        pr_notice( "****[pmic_mt_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&pmic_mt_driver);
    if (ret) {
        pr_notice( "****[pmic_mt_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
    ...
    pr_notice( "****[pmic_mt_init] Initialization : DONE !!\n");
