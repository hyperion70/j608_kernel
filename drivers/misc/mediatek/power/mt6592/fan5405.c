...
kal_uint32 fan5405_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    //pr_notice("[fan5405_config_interface] Check Reg[%x]=0x%x\n", RegNum, fan5405_reg);
...
void fan5405_dump_register(void)
{
    ...
    for (i=0;i<fan5405_REG_NUM;i++)
    {
        ...
        pr_notice("[0x%x]=0x%x ", i, fan5405_reg[i]);
    }
    pr_notice("\n");
}
...
void fan5405_hw_init(void)
{
    if(g_enable_high_vbat_spec == 1)
    {
        ...
        {
            pr_notice("[fan5405_hw_init] (0x06,0x70) because 0x1020\n");
            fan5405_reg_config_interface(0x06,0x70); // set ISAFE
        }
        else
        {
            pr_notice("[fan5405_hw_init] (0x06,0x77)\n");
            fan5405_reg_config_interface(0x06,0x77); // set ISAFE and HW CV point (4.34)
        }
    }
    else
    {
        pr_notice("[fan5405_hw_init] (0x06,0x70) \n");
        fan5405_reg_config_interface(0x06,0x70); // set ISAFE
    }
}
#endif
...
static int fan5405_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //---------------------
  //  fan5405_hw_init();
        // 0x6 reg only write vaild at first time
