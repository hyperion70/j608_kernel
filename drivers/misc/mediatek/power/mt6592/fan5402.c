...
kal_uint32 fan5402_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
   ...
   battery_log(BAT_LOG_FULL,"--------------------------------------------------\n");
	...
	battery_log(BAT_LOG_FULL,"[fan5402_read_interface] Reg[%x]=0x%x\n", RegNum, fan5402_reg);

    fan5402_reg &= (MASK << SHIFT);
    *val = (fan5402_reg >> SHIFT);

	battery_log(BAT_LOG_FULL,"[fan5402_read_interface] val=0x%x\n", *val);
...
kal_uint32 fan5402_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = fan5402_read_byte(RegNum, &fan5402_reg);
    battery_log(BAT_LOG_FULL,"[fan5402_config_interface] Reg[%x]=0x%x\n", RegNum, fan5402_reg);
    ...
    battery_log(BAT_LOG_FULL,"[fan5402_config_interface] write Reg[%x]=0x%x\n", RegNum, fan5402_reg);

    // Check
    //fan5402_read_byte(RegNum, &fan5402_reg);
    //pr_notice("[fan5402_config_interface] Check Reg[%x]=0x%x\n", RegNum, fan5402_reg);
...
void fan5402_dump_register(void)
{
    ...
    for (i=0;i<fan5402_REG_NUM;i++)
    {
        ...
        pr_notice("[0x%x]=0x%x ", i, fan5402_reg[i]);
    }
    pr_notice("\n");
}
...
static int fan5402_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    battery_log(BAT_LOG_CRTI,"[fan5402_driver_probe] \n");
...
{
    battery_log(BAT_LOG_FULL,"[show_fan5402_access] 0x%x\n", g_reg_value_fan5402);
...
static ssize_t store_fan5402_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    battery_log(BAT_LOG_FULL,"[store_fan5402_access] \n");

    if(buf != NULL && size != 0)
    {
        battery_log(BAT_LOG_FULL,"[store_fan5402_access] buf is %s and size is %d \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 3)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
            battery_log(BAT_LOG_FULL,"[store_fan5402_access] write fan5402 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        else
        {
            ...
            battery_log(BAT_LOG_FULL,"[store_fan5402_access] read fan5402 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_fan5402);
            battery_log(BAT_LOG_FULL,"[store_fan5402_access] Please use \"cat fan5402_access\" to get value\r\n");
        }
    }
    return size;
}
...
static int fan5402_user_space_probe(struct platform_device *dev)
{
    ...
    battery_log(BAT_LOG_CRTI,"******** fan5402_user_space_probe!! ********\n" );
...
static int __init fan5402_init(void)
{
    ...
    battery_log(BAT_LOG_CRTI,"[fan5402_init] init start\n");

    i2c_register_board_info(FAN5402_BUSNUM, &i2c_fan5402, 1);

    if(i2c_add_driver(&fan5402_driver)!=0)
    {
        battery_log(BAT_LOG_CRTI,"[fan5402_init] failed to register fan5402 i2c driver.\n");
    }
    else
    {
        battery_log(BAT_LOG_CRTI,"[fan5402_init] Success to register fan5402 i2c driver.\n");
    }

    // fan5402 user space access interface
    ret = platform_device_register(&fan5402_user_space_device);
    if (ret) {
        battery_log(BAT_LOG_CRTI,"****[fan5402_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&fan5402_user_space_driver);
    if (ret) {
        battery_log(BAT_LOG_CRTI,"****[fan5402_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
