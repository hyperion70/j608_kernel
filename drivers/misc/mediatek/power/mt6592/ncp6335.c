...
int ncp6335_read_byte(kal_uint8 cmd, kal_uint8 *returnData)
{
    ...
    if (ret < 0)
    {
        pr_notice("[ncp6335_read_byte] ret=%d\n", ret);
...
int ncp6335_write_byte(kal_uint8 cmd, kal_uint8 writeData)
{
    ...
    if (ret < 0)
    {
        pr_notice("[ncp6335_write_byte] ret=%d\n", ret);
...
kal_uint32 ncp6335_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    pr_notice("[ncp6335_read_interface] Reg[%x]=0x%x\n", RegNum, ncp6335_reg);

    ncp6335_reg &= (MASK << SHIFT);
    *val = (ncp6335_reg >> SHIFT);

    pr_notice("[ncp6335_read_interface] val=0x%x\n", *val);
...
kal_uint32 ncp6335_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = ncp6335_read_byte(RegNum, &ncp6335_reg);
    pr_notice("[ncp6335_config_interface] Reg[%x]=0x%x\n", RegNum, ncp6335_reg);

    ncp6335_reg &= ~(MASK << SHIFT);
    ncp6335_reg |= (val << SHIFT);

    ret = ncp6335_write_byte(RegNum, ncp6335_reg);
    pr_notice("[ncp6335_config_interface] write Reg[%x]=0x%x\n", RegNum, ncp6335_reg);

    // Check
    //ncp6335_read_byte(RegNum, &ncp6335_reg);
    //pr_notice("[ncp6335_config_interface] Check Reg[%x]=0x%x\n", RegNum, ncp6335_reg);
...
void ncp6335_dump_register(void)
{
    ...
    for (i=0;i<ncp6335_REG_NUM;i++)
    {
        ...
        pr_notice("[0x%x]=0x%x ", i, ncp6335_reg[i]);
    }
    pr_notice("\n");
}
...
void ncp6335_hw_init(void)
{
    ...
    ret=ncp6335_read_interface(0x12,&val,0xFF,0);

    ret = ncp6335_config_interface(0x13,0x19,0xFF,0);
    ret=ncp6335_read_interface(0x13,&val,0xFF,0);

    ret = ncp6335_config_interface(0x15,0x80,0xFF,0);
    ret=ncp6335_read_interface(0x15,&val,0xFF,0);

    ret = ncp6335_config_interface(0x10,0xD0,0xFF,0); // VSEL=high, 1.1V
    ret=ncp6335_read_interface(0x10,&val,0xFF,0);

    if(g_vproc_vsel_gpio_number!=0)
    {
        ext_buck_vproc_vsel(1);
        pr_notice( "[ncp6335_hw_init] ext_buck_vproc_vsel(1)\n");
    }

    ret = ncp6335_config_interface(0x11,0x90,0xFF,0); // VSEL=low, 0.7V
    ret=ncp6335_read_interface(0x11,&val,0xFF,0);

    ret = ncp6335_config_interface(0x14,0x01,0xFF,0);
    ret=ncp6335_read_interface(0x14,&val,0xFF,0);

    ret = ncp6335_config_interface(0x16,0xE3,0xFF,0);
    ret=ncp6335_read_interface(0x16,&val,0xFF,0);

    pr_notice("[ncp6335_hw_init] Done\n");
}
...
void ncp6335_hw_component_detect(void)
{
    ...
    pr_notice("[ncp6335_hw_component_detect] exist=%d, Reg[0x03]=0x%x\n",
        g_ncp6335_hw_exist, val);
}

int is_ncp6335_sw_ready(void)
{
    //pr_notice("g_ncp6335_driver_ready=%d\n", g_ncp6335_driver_ready);
...
{
    //pr_notice("g_ncp6335_hw_exist=%d\n", g_ncp6335_hw_exist);
...
int ncp6335_vosel(unsigned long val)
{
    ...
    pr_notice("[ncp6335_vosel] val=%ld, reg_val=%ld\n", val, reg_val);
...
static int ncp6335_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    pr_notice("[ncp6335_driver_probe] \n");
    ...
    pr_notice("[ncp6335_driver_probe] g_ncp6335_hw_exist=%d, g_ncp6335_driver_ready=%d\n",
        g_ncp6335_hw_exist, g_ncp6335_driver_ready);

    if(g_ncp6335_hw_exist==0)
    {
        pr_notice("[ncp6335_driver_probe] return err\n");
        return err;
    }
...
static int ncp6335_driver_suspend(struct i2c_client *client, pm_message_t mesg)
{
    ...
    if(g_ncp6335_hw_exist==1)
    {
        ...
        pr_notice("[ncp6335_driver_suspend] Reg[0x13]=0x%x\n",val);
    }
    else
    {
        pr_notice("[ncp6335_driver_suspend] no HW\n");
    }
    return 0;
}
...
{
    pr_notice("[show_ncp6335_access] 0x%x\n", g_reg_value_ncp6335);
...
static ssize_t store_ncp6335_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_notice("[store_ncp6335_access] \n");

    if(buf != NULL && size != 0)
    {
        pr_notice("[store_ncp6335_access] buf is %s and size is %zu \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 3)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
            pr_notice("[store_ncp6335_access] write ncp6335 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
            ret=ncp6335_config_interface(reg_address, reg_value, 0xFF, 0x0);
            ret=ncp6335_read_interface(reg_address,&reg_value,0xFF,0);
        }
        else
        {
            ret=ncp6335_read_interface(reg_address, &g_reg_value_ncp6335, 0xFF, 0x0);
            pr_notice("[store_ncp6335_access] read ncp6335 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_ncp6335);
            pr_notice("[store_ncp6335_access] Please use \"cat ncp6335_access\" to get value\r\n");
        }
    }
    return size;
}
...
static int ncp6335_user_space_probe(struct platform_device *dev)
{
    ...
    pr_notice("******** ncp6335_user_space_probe!! ********\n" );
...
static int __init ncp6335_init(void)
{
    ...
    pr_notice("[ncp6335_init] NCP6335_AUTO_DETECT_DISABLE\n");
    g_ncp6335_hw_exist=0;
    g_ncp6335_driver_ready=1;
    ...
    if(g_vproc_vsel_gpio_number != 0)
    {
        pr_notice("[ncp6335_init] init start.\n");

        i2c_register_board_info(ncp6335_BUSNUM, &i2c_ncp6335, 1);

        if(i2c_add_driver(&ncp6335_driver)!=0)
        {
            pr_notice("[ncp6335_init] failed to register ncp6335 i2c driver.\n");
        }
        else
        {
            pr_notice("[ncp6335_init] Success to register ncp6335 i2c driver.\n");
        }

        // ncp6335 user space access interface
        ret = platform_device_register(&ncp6335_user_space_device);
        if (ret) {
            pr_notice("****[ncp6335_init] Unable to device register(%d)\n", ret);
            return ret;
        }
        ret = platform_driver_register(&ncp6335_user_space_driver);
        if (ret) {
            pr_notice("****[ncp6335_init] Unable to register driver (%d)\n", ret);
            return ret;
        }
    }
    else
    {
        pr_notice("[ncp6335_init] DCT no define EXT BUCK\n");
        g_ncp6335_hw_exist=0;
        g_ncp6335_driver_ready=1;
    }
