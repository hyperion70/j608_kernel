...
int fan53555_read_byte(kal_uint8 cmd, kal_uint8 *returnData)
{
    ...
    if (ret < 0)
    {
        pr_notice("[fan53555_read_byte] ret=%d\n", ret);
...
int fan53555_write_byte(kal_uint8 cmd, kal_uint8 writeData)
{
    ...
    if (ret < 0)
    {
        pr_notice("[fan53555_write_byte] ret=%d\n", ret);
...
kal_uint32 fan53555_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    pr_notice("[fan53555_read_interface] Reg[%x]=0x%x\n", RegNum, fan53555_reg);

    fan53555_reg &= (MASK << SHIFT);
    *val = (fan53555_reg >> SHIFT);

    pr_notice("[fan53555_read_interface] val=0x%x\n", *val);
...
kal_uint32 fan53555_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = fan53555_read_byte(RegNum, &fan53555_reg);
    pr_notice("[fan53555_config_interface] Reg[%x]=0x%x\n", RegNum, fan53555_reg);

    fan53555_reg &= ~(MASK << SHIFT);
    fan53555_reg |= (val << SHIFT);

    ret = fan53555_write_byte(RegNum, fan53555_reg);
    pr_notice("[fan53555_config_interface] write Reg[%x]=0x%x\n", RegNum, fan53555_reg);

    // Check
    //fan53555_read_byte(RegNum, &fan53555_reg);
    //pr_notice("[fan53555_config_interface] Check Reg[%x]=0x%x\n", RegNum, fan53555_reg);
...
void fan53555_dump_register(void)
{
    ...
    for (i=0;i<fan53555_REG_NUM;i++)
    {
        ...
        pr_notice("[0x%x]=0x%x ", i, fan53555_reg[i]);
    }
    pr_notice("\n");
}
...
void fan53555_hw_component_detect(void)
{
    ...
    pr_notice("[fan53555_hw_component_detect] exist=%d, Reg[0x03][7:5]=0x%x\n",
        g_fan53555_hw_exist, val);
}

int is_fan53555_sw_ready(void)
{
    //pr_notice("g_fan53555_driver_ready=%d\n", g_fan53555_driver_ready);
...
{
    //pr_notice("g_fan53555_hw_exist=%d\n", g_fan53555_hw_exist);
...
static int fan53555_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    pr_notice("[fan53555_driver_probe] \n");
    ...
    pr_notice("[fan53555_driver_probe] g_fan53555_hw_exist=%d, g_fan53555_driver_ready=%d\n",
        g_fan53555_hw_exist, g_fan53555_driver_ready);

    if(g_fan53555_hw_exist==0)
    {
        pr_notice("[fan53555_driver_probe] return err\n");
        return err;
    }
...
{
    pr_notice("[show_fan53555_access] 0x%x\n", g_reg_value_fan53555);
...
static ssize_t store_fan53555_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_notice("[store_fan53555_access] \n");

    if(buf != NULL && size != 0)
    {
        pr_notice("[store_fan53555_access] buf is %s and size is %zu \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 3)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
            pr_notice("[store_fan53555_access] write fan53555 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        else
        {
            ...
            pr_notice("[store_fan53555_access] read fan53555 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_fan53555);
            pr_notice("[store_fan53555_access] Please use \"cat fan53555_access\" to get value\r\n");
        }
    }
    return size;
}
...
static int fan53555_user_space_probe(struct platform_device *dev)
{
    ...
    pr_notice("******** fan53555_user_space_probe!! ********\n" );
...
static int __init fan53555_init(void)
{
    ...
    pr_notice("[fan53555_init] FAN53555_AUTO_DETECT_DISABLE\n");
    g_fan53555_hw_exist=0;
    g_fan53555_driver_ready=1;
    ...
    if(g_vproc_vsel_gpio_number != 0)
    {
        pr_notice("[fan53555_init] init start.\n");

        i2c_register_board_info(fan53555_BUSNUM, &i2c_fan53555, 1);

        if(i2c_add_driver(&fan53555_driver)!=0)
        {
            pr_notice("[fan53555_init] failed to register fan53555 i2c driver.\n");
        }
        else
        {
            pr_notice("[fan53555_init] Success to register fan53555 i2c driver.\n");
        }

        // fan53555 user space access interface
        ret = platform_device_register(&fan53555_user_space_device);
        if (ret) {
            pr_notice("****[fan53555_init] Unable to device register(%d)\n", ret);
            return ret;
        }
        ret = platform_driver_register(&fan53555_user_space_driver);
        if (ret) {
            pr_notice("****[fan53555_init] Unable to register driver (%d)\n", ret);
            return ret;
        }
    }
    else
