				  ...
				  kal_uint8 SHIFT)
{
	...
	pr_notice("[bq24196_read_interface] Reg[%x]=0x%x\n", RegNum, bq24196_reg);

	bq24196_reg &= (MASK << SHIFT);
	*val = (bq24196_reg >> SHIFT);
	pr_notice("[bq24196_read_interface] Val=0x%x\n", *val);
				    ...
				    kal_uint8 SHIFT)
{
	...
	ret = bq24196_read_byte(RegNum, &bq24196_reg);
	/*pr_notice("[bq24196_config_interface] Reg[%x]=0x%x\n", RegNum, bq24196_reg); */

	bq24196_reg &= ~(MASK << SHIFT);
	bq24196_reg |= (val << SHIFT);

	ret = bq24196_write_byte(RegNum, bq24196_reg);
	/*pr_notice("[bq24196_config_interface] Write Reg[%x]=0x%x\n", RegNum, bq24196_reg); */

	/*Check */
	/*bq24196_read_byte(RegNum, &bq24196_reg); */
	/*pr_notice("[bq24196_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24196_reg); */
...
void bq24196_dump_register(void)
{
	...
	for (i = 0; i < bq24196_REG_NUM; i++) {
		...
		pr_notice("[bq24196_dump_register] Reg[0x%X]=0x%X\n", i, bq24196_reg[i]);
	}
}
				    ...
				    const char *buf, size_t size)
{
	...
	pr_notice("[store_bq24196_access]\n");

	if (buf != NULL && size != 0) {
		pr_notice("[store_bq24196_access] buf is %s and size is %d\n", buf, size);
