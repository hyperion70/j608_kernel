...
kal_uint32 mt6333_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    //pr_notice("[mt6333_read_interface] Reg[%x]=0x%x\n", RegNum, mt6333_reg);

    mt6333_reg &= (MASK << SHIFT);
    *val = (mt6333_reg >> SHIFT);
    //pr_notice("[mt6333_read_interface] val=0x%x\n", *val);
...
kal_uint32 mt6333_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = mt6333_read_byte(RegNum, &mt6333_reg);
    //pr_notice("[mt6333_config_interface] Reg[%x]=0x%x\n", RegNum, mt6333_reg);

    mt6333_reg &= ~(MASK << SHIFT);
    mt6333_reg |= (val << SHIFT);

    ret = mt6333_write_byte(RegNum, mt6333_reg);
    //pr_notice("[mt6333_config_interface] write Reg[%x]=0x%x\n", RegNum, mt6333_reg);

    // Check
    //mt6333_read_byte(RegNum, &mt6333_reg);
    //pr_notice("[mt6333_config_interface] Check Reg[%x]=0x%x\n", RegNum, mt6333_reg);
...
void mt6333_dump_register(void)
{
    ...
    for (i=0;i<0xFF;i++)
    {
        ...
        pr_notice("[0x%x]=0x%x ", i, mt_swchr_60393_reg);
...
{
    kal_uint32 ret=0;
    pr_notice( "[chr_complete_int_handler]....\n");

    pr_notice("RGS_CHARGE_COMPLETE_HW = %d\n", mt6333_get_rgs_charge_complete_hw());
...
{
    kal_uint32 ret=0;
    pr_notice( "[thermal_sd_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[thermal_reg_in_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[thermal_reg_out_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[otg_thermal_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[otg_chrin_short_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[otg_drvcdt_short_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[chrwdt_flag_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[buck_vcore_oc_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[buck_vmem_oc_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[buck_vrf18_oc_int_handler]....\n");
...
{
	pr_notice( "set_thermal_mt6333_buck_int_status\n");
	thermal_mt6333_buck_int = val;
}

kal_uint32 get_thermal_mt6333_buck_int_status(void)
{
	//pr_notice( "get_thermal_mt6333_buck_int_status\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[buck_thermal_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[flash_en_timeout_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[flash_vled_short_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[flash_vled_open_int_handler]....\n");
...
{
    kal_uint32 ret=0;
    pr_notice( "[chr_plug_in_flash_int_handler]....\n");
...
static int mt6333_thread_kthread(void *x)
{
    ...
    pr_notice( "[mt6333_thread_kthread] enter\n");

    /* Run on a process content */
    while (1) {
        mutex_lock(&mt6333_mutex);

        pr_notice( "[mt6333_thread_kthread] running\n");

        //--------------------------------------------------------------------------------
        int_status_val_0 = mt6333_get_reg_value(MT6333_INT_STATUS0);
        pr_notice( "[6333 INT] int_status_val_0=0x%x\n", int_status_val_0);
        ...
        pr_notice( "[6333 INT] int_status_val_1=0x%x\n", int_status_val_1);
        ...
        pr_notice( "[6333 INT] int_status_val_2=0x%x\n", int_status_val_2);
...
{
    pr_notice( "[mt6333_pmic_eint_irq] receive interrupt\n");
...
{
    pr_notice( "[MT6333_EINT_SETTING] start: CUST_EINT_MT6333_PMIC_NUM=%d\n",CUST_EINT_MT6333_PMIC_NUM);
    ...
    pr_notice( "[CUST_EINT] CUST_EINT_MT6333_PMIC_NUM=%d\n",           CUST_EINT_MT6333_PMIC_NUM);
    pr_notice( "[CUST_EINT] CUST_EINT_MT6333_PMIC_DEBOUNCE_CN=%d\n",   CUST_EINT_MT6333_PMIC_DEBOUNCE_CN);
    pr_notice( "[CUST_EINT] CUST_EINT_MT6333_PMIC_TYPE=%d\n",          CUST_EINT_MT6333_PMIC_TYPE);
    pr_notice( "[CUST_EINT] CUST_EINT_MT6333_PMIC_DEBOUNCE_EN=%d\n",   CUST_EINT_MT6333_PMIC_DEBOUNCE_EN);
}
...
void tc_1101(void)
{
    ...
    if(reg_val != 0x93) pr_notice("Fail : i2c_read(0x00, 0x93), 0x%x\n", reg_val);

    ret=mt6333_read_interface(0xA9, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x00) pr_notice("Fail : i2c_read(0xA9, 0x00), 0x%x\n", reg_val);

    ret=mt6333_read_interface(0xFD, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x10) pr_notice("Fail : i2c_read(0xFD, 0x10), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x00, 0x00, 0xFF, 0x0);
    ret=mt6333_read_interface(0x00, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x93) pr_notice("Fail : i2c_write(0x00, 0x00), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0xA9, 0xFF, 0xFF, 0x0);
    ret=mt6333_read_interface(0xA9, &reg_val, 0xFF, 0x0);
    if(reg_val != 0xFF) pr_notice("Fail : i2c_write(0xA9, 0xFF), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0xA9, 0x38, 0xFF, 0x0);
    ret=mt6333_read_interface(0xA9, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x38) pr_notice("Fail : i2c_write(0xA9, 0x38), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0xFD, 0xFF, 0xFF, 0x0);
    ret=mt6333_read_interface(0xFD, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x10) pr_notice("Fail : i2c_write(0xFD, 0xFF), 0x%x\n", reg_val);

    pr_notice("[tc_1101] Done\n");
}
...
void tc_1102(void)
{
    ...
    if(reg_val != 0x00) pr_notice("Fail : i2c_write(0x04, 0x01), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x04, 0x11, 0xFF, 0x0);
    ret=mt6333_read_interface(0x04, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x00) pr_notice("Fail : i2c_write(0x04, 0x11), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x04, 0x95, 0xFF, 0x0);
    ret=mt6333_read_interface(0x04, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x01) pr_notice("Fail : i2c_write(0x04, 0x95), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x04, 0x00, 0xFF, 0x0);
    ret=mt6333_read_interface(0x04, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x01) pr_notice("Fail : i2c_write(0x04, 0x00), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x04, 0x20, 0xFF, 0x0);
    ret=mt6333_read_interface(0x04, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x01) pr_notice("Fail : i2c_write(0x04, 0x20), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x04, 0x94, 0xFF, 0x0);
    ret=mt6333_read_interface(0x04, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x00) pr_notice("Fail : i2c_write(0x04, 0x94), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x30, 0x01, 0xFF, 0x0);
    ret=mt6333_read_interface(0x30, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x00) pr_notice("Fail : i2c_write(0x30, 0x01), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x30, 0x11, 0xFF, 0x0);
    ret=mt6333_read_interface(0x30, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x00) pr_notice("Fail : i2c_write(0x30, 0x11), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x30, 0x95, 0xFF, 0x0);
    ret=mt6333_read_interface(0x30, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x01) pr_notice("Fail : i2c_write(0x30, 0x95), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x30, 0x00, 0xFF, 0x0);
    ret=mt6333_read_interface(0x30, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x01) pr_notice("Fail : i2c_write(0x30, 0x00), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x30, 0x20, 0xFF, 0x0);
    ret=mt6333_read_interface(0x30, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x01) pr_notice("Fail : i2c_write(0x30, 0x20), 0x%x\n", reg_val);

    ret=mt6333_config_interface(0x30, 0x94, 0xFF, 0x0);
    ret=mt6333_read_interface(0x30, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x00) pr_notice("Fail : i2c_write(0x30, 0x94), 0x%x\n", reg_val);

    pr_notice("[tc_1102] Done\n");
}
...
void tc_1103(void)
{
    ...
    ret=mt6333_read_interface(0xFD, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x20) pr_notice("Fail : i2c_read(0xFD, 0x20), 0x%x\n", reg_val);

    ret=mt6333_read_interface(0x07, &reg_val, 0xFF, 0x0);
    if(reg_val != 0x02) pr_notice("Fail : i2c_read(0x07, 0x02), 0x%x\n", reg_val);

    pr_notice("[tc_1103] Done\n");
}
...
void read_adc_value(int index)
{
    ...
    if(index == 0) //vcore
    {
        pr_notice("Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
    ...
    {
        pr_notice("Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
    ...
    pr_notice("[read_auxadc_value] ret = %d\n\n", ret);
}
...
void set_thr_sdn_en(int thr_sdn_en)
{
    ...
    pr_notice("[set_thr_sdn_en] Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
...
void exec_vcore_en_test(void)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[mt6333_set_vcore_en_ctrl] %d\n", i);
        mt6333_set_vcore_en_ctrl(i);

        switch(i){
            case 0:
                pr_notice("[mt6333_set_vcore_en(0)]\n");
                mt6333_set_vcore_en(0);
                read_adc_value(VCORE_INDEX);

                pr_notice("[mt6333_set_vcore_en(1)]\n");
            ...
            default:
                pr_notice("Invalid channel value(%d)\n", i);
                break;
        }
    }
}
...
void exec_vmem_en_test(void)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[mt6333_set_vmem_en_ctrl] %d\n", i);
        mt6333_set_vmem_en_ctrl(i);

        switch(i){
            case 0:
                pr_notice("[mt6333_set_rg_vmem_en_thr_sdn_sel(0)]\n");
                mt6333_set_rg_vmem_en_thr_sdn_sel(0);
                    //------------------------------------------
                    pr_notice("vmem_en=0, thr_sdn_en=0\n");
                    mt6333_set_vmem_en(0);
                    set_thr_sdn_en(0);
                    read_adc_value(VMEM_INDEX);
                    //------------------------------------------
                    pr_notice("vmem_en=0, thr_sdn_en=1\n");
                    mt6333_set_vmem_en(0);
                    set_thr_sdn_en(1);
                    read_adc_value(VMEM_INDEX);
                    //------------------------------------------
                    pr_notice("vmem_en=1, thr_sdn_en=0\n");
                    mt6333_set_vmem_en(1);
                    set_thr_sdn_en(0);
                    read_adc_value(VMEM_INDEX);
                    //------------------------------------------
                    pr_notice("vmem_en=1, thr_sdn_en=1\n");
                    mt6333_set_vmem_en(1);
                    set_thr_sdn_en(1);
                    read_adc_value(VMEM_INDEX);
                    //------------------------------------------
                pr_notice("[mt6333_set_rg_vmem_en_thr_sdn_sel(1)]\n");
                mt6333_set_rg_vmem_en_thr_sdn_sel(1);
                    //------------------------------------------
                    pr_notice("[mt6333_set_vmem_en(0)]\n");
                    mt6333_set_vmem_en(0);
                    read_adc_value(VMEM_INDEX);
                    //------------------------------------------
                    pr_notice("[mt6333_set_vmem_en(1)]\n");
                    mt6333_set_vmem_en(1);
                    read_adc_value(VMEM_INDEX);
                    //------------------------------------------
                break;
...
void exec_vrf18_en_test(void)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[mt6333_set_vrf18_en_ctrl] %d\n", i);
        mt6333_set_vrf18_en_ctrl(i);

        switch(i){
            case 0:
                pr_notice("vrf18_en=0, vrf18_en_spm=0]\n");
                mt6333_set_vrf18_en(0);
                mt6333_set_vrf18_en_spm(0);
                read_adc_value(VRF18_INDEX);
                //------------------------------------------
                pr_notice("vrf18_en=0, vrf18_en_spm=1]\n");
                mt6333_set_vrf18_en(0);
                mt6333_set_vrf18_en_spm(1);
                read_adc_value(VRF18_INDEX);
                //------------------------------------------
                pr_notice("vrf18_en=1, vrf18_en_spm=0]\n");
                mt6333_set_vrf18_en(1);
                mt6333_set_vrf18_en_spm(0);
                read_adc_value(VRF18_INDEX);
                //------------------------------------------
                pr_notice("vrf18_en=1, vrf18_en_spm=1]\n");
                mt6333_set_vrf18_en(1);
                mt6333_set_vrf18_en_spm(1);
                read_adc_value(VRF18_INDEX);
                break;
...
{
    pr_notice("[PMIC_BUCK_ON_OFF] start....\n");
    ...
    switch(index_val){
	  ...
	  default:
        pr_notice("[PMIC_BUCK_ON_OFF] Invalid channel value(%d)\n", index_val);
        break;

    }

    pr_notice("[PMIC_BUCK_ON_OFF] end....\n");
}
...
void vcore_vosel_sub_1_setting(int vsleep_en, int thr_sdn_en)
{
    ...
    pr_notice("[SRCLKEN=%d] Reg[0x%x]=0x%x\n", vsleep_en, MT6333_TESTO_CON2, mt6333_get_reg_value(MT6333_TESTO_CON2));

    set_thr_sdn_en(thr_sdn_en);

    pr_notice("[vcore_vosel_sub_1_setting] Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
...
void vcore_vosel_sub_1(void)
{
    ...
    for(j=0;j<=MT6333_PMIC_VCORE_VOSEL_SLEEP_MASK;j++)
    {
        ...
        pr_notice("[mt6333_set_vcore_vosel_sleep] j=%d, ",j);
        if(j==0)
            mdelay(500);
        read_adc_value(VCORE_INDEX);
    }
...
void vcore_vosel_sub_2(void)
{
    ...
    pr_notice("[mt6333_set_vcore_vosel] mt6333_set_rg_vcore_vosel_set_spm(0); Reg[%x]=0x%x \n",
    ...
    for(j=0;j<=MT6333_PMIC_VCORE_VOSEL_ON_MASK;j++)
    {
        ...
        pr_notice("[mt6333_set_vcore_vosel_on] j=%d, ",j);
        if(j==0)
            mdelay(500);
        read_adc_value(VCORE_INDEX);
    }
    ...
    pr_notice("[mt6333_set_vcore_vosel] mt6333_set_rg_vcore_vosel_set_spm(1); Reg[%x]=0x%x \n",
        MT6333_VRF18_CON22, mt6333_get_reg_value(MT6333_VRF18_CON22));

    for(j=0;j<=MT6333_PMIC_VCORE_VOSEL_ON_SPM_MASK;j++)
    {
        mt6333_set_vcore_vosel_on_spm(j);
        pr_notice("[mt6333_set_vcore_vosel_on_spm] j=%d, ",j);
        if(j==0)
            mdelay(500);
        read_adc_value(VCORE_INDEX);
    }
...
void exec_vcore_vosel_test(int index_val)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[mt6333_set_vcore_vosel_ctrl] %d\n", i);
        ...
        switch(i){
            case 0:
                for(j=0;j<=MT6333_PMIC_VCORE_VOSEL_MASK;j++)
                {
                    ...
                    pr_notice("[mt6333_set_vcore_vosel] j=%d, ",j);
                    if(j==0)
                        mdelay(500);
                    read_adc_value(VCORE_INDEX);
                }
            ...
            case 1:
                pr_notice("[mt6333_set_vcore_vosel] set VSLEEP_EN=0, THR_SDN=0\n"); //0
                vcore_vosel_sub_1_setting(0,0);
                vcore_vosel_sub_2();

                mt6333_set_vcore_vosel_sleep(0x37);

                pr_notice("[mt6333_set_vcore_vosel] set VSLEEP_EN=0, THR_SDN=1\n"); //1
                vcore_vosel_sub_1_setting(0,1);
                vcore_vosel_sub_1();

                pr_notice("[mt6333_set_vcore_vosel] set VSLEEP_EN=1, THR_SDN=0\n"); //1
                vcore_vosel_sub_1_setting(1,0);
                vcore_vosel_sub_1();

                pr_notice("[mt6333_set_vcore_vosel] set VSLEEP_EN=1, THR_SDN=1\n"); //1
                vcore_vosel_sub_1_setting(1,1);
                vcore_vosel_sub_1();
                break;

            default:
                pr_notice("At %d, Invalid channel value(%d)\n", index_val, i);
                break;
        }
    }
}
...
void exec_vmem_vosel_test(int index_val)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[mt6333_set_vmem_vosel_ctrl] %d\n", i);
        ...
        switch(i){
            case 0:
                for(j=0;j<=MT6333_PMIC_VMEM_VOSEL_MASK;j++)
                {
                    ...
                    pr_notice("[mt6333_set_vmem_vosel] j=%d, ",j);
                    if(j==0)
                        mdelay(500);
                    read_adc_value(VMEM_INDEX);
                }
            ...
            case 1:
                ...
                for(j=0;j<=MT6333_PMIC_VMEM_VOSEL_SLEEP_MASK;j++)
                {
                    ...
                    pr_notice("[mt6333_set_vmem_vosel_sleep] j=%d, ",j);
                    if(j==0)
                        mdelay(500);
                    read_adc_value(VMEM_INDEX);
                }
                ...
                for(j=0;j<=MT6333_PMIC_VMEM_VOSEL_ON_MASK;j++)
                {
                    ...
                    pr_notice("[mt6333_set_vmem_vosel_on] j=%d, ",j);
                    if(j==0)
                        mdelay(500);
                    read_adc_value(VMEM_INDEX);
                }
...
void exec_vrf18_vosel_sub_test(void)
{
    ...
    for(j=MT6333_PMIC_VRF18_VOSEL_SLEEP_MASK;j>=0;j--)
    {
        ...
        pr_notice("[mt6333_set_vrf18_vosel_sleep] j=%d, ",j);
        if(j==0)
            mdelay(500);
        read_adc_value(VRF18_INDEX);
    }
    ...
    for(j=0;j<=MT6333_PMIC_VRF18_VOSEL_ON_MASK;j++)
    {
        ...
        pr_notice("[mt6333_set_vrf18_vosel_on] j=%d, ",j);
        if(j==0)
            mdelay(500);
        read_adc_value(VRF18_INDEX);
    }
...
void exec_vrf18_vosel_test(int index_val)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[mt6333_set_vrf18_vosel_ctrl] %d\n", i);
        ...
        switch(i){
            case 0:
                for(j=0;j<=MT6333_PMIC_VRF18_VOSEL_MASK;j++)
                {
                    ...
                    pr_notice("[mt6333_set_vrf18_vosel] j=%d, ",j);
                    if(j==0)
                        mdelay(500);
                    read_adc_value(VRF18_INDEX);
                }
...
{
    pr_notice("[PMIC_BUCK_VOSEL] start....\n");
    ...
    switch(index_val){
	  ...
	  default:
        pr_notice("[PMIC_BUCK_VOSEL] Invalid channel value(%d)\n", index_val);
        break;

    }

    pr_notice("[PMIC_BUCK_VOSEL] end....\n");
}
...
void exec_vcore_dlc_subtest(void)
{
    ...
    pr_notice("[exec_vcore_dlc_subtest]\n");
    ...
    for(i=0;i<=MT6333_PMIC_VCORE_DLC_SLEEP_MASK;i++)
    {
        ...
        pr_notice("[exec_vcore_dlc_subtest] mt6333_set_vcore_dlc_sleep=%d, mt6333_set_vcore_dlc_n_sleep=%d\n", i, i);

        pr_notice("[exec_vcore_dlc_subtest] mt6333_get_qi_vcore_dlc=%d, mt6333_get_qi_vcore_dlc_n=%d\n",
    ...
    for(i=0;i<=MT6333_PMIC_VCORE_DLC_ON_MASK;i++)
    {
        ...
        pr_notice("[exec_vcore_dlc_subtest] mt6333_set_vcore_dlc_on=%d, mt6333_set_vcore_dlc_n_on=%d\n", i, i);
...
void exec_vmem_dlc_subtest(void)
{
    ...
    pr_notice("[exec_vmem_dlc_subtest]\n");
    ...
    for(i=0;i<=MT6333_PMIC_VMEM_DLC_SLEEP_MASK;i++)
    {
        ...
        pr_notice("[exec_vmem_dlc_subtest] mt6333_set_vmem_dlc_sleep=%d, mt6333_set_vmem_dlc_n_sleep=%d\n", i, i);

        pr_notice("[exec_vmem_dlc_subtest] mt6333_get_qi_vmem_dlc=%d, mt6333_get_qi_vmem_dlc_n=%d\n",
    ...
    for(i=0;i<=MT6333_PMIC_VMEM_DLC_ON_MASK;i++)
    {
        ...
        pr_notice("[exec_vmem_dlc_subtest] mt6333_set_vmem_dlc_on=%d, mt6333_set_vmem_dlc_n_on=%d\n", i, i);
...
void exec_vrf18_dlc_subtest(void)
{
    ...
    pr_notice("[exec_vrf18_dlc_subtest]\n");
    ...
    for(i=0;i<=MT6333_PMIC_VRF18_DLC_SLEEP_MASK;i++)
    {
        ...
        pr_notice("[exec_vrf18_dlc_subtest] mt6333_set_vrf18_dlc_sleep=%d, mt6333_set_vrf18_dlc_n_sleep=%d\n", i, i);

        pr_notice("[exec_vrf18_dlc_subtest] mt6333_get_qi_vrf18_dlc=%d, mt6333_get_qi_vrf18_dlc_n=%d\n",
    ...
    for(i=0;i<=MT6333_PMIC_VRF18_DLC_ON_MASK;i++)
    {
        ...
        pr_notice("[exec_vrf18_dlc_subtest] mt6333_set_vrf18_dlc_on=%d, mt6333_set_vrf18_dlc_n_on=%d\n", i, i);
...
void exec_dlc_subtest(int index_val)
{
    switch(index_val){
        ...
        case 2:
            ...
            break;
        default:
            pr_notice("[exec_dlc_subtest] Invalid channel value(%d)\n", index_val);
            break;
    }
}
...
void exec_vcore_dlc_test(int index_val)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[exec_vcore_dlc_test] %d\n", i);
        ...
        switch(i){
            case 0:
                for(j=0;j<=MT6333_PMIC_VCORE_DLC_MASK;j++)
                {
                    ...
                    {
                        pr_notice("[exec_vcore_dlc_test] fail at mt6333_get_qi_vcore_dlc=%d, mt6333_get_qi_vcore_dlc_n=%d\n",
                            mt6333_get_qi_vcore_dlc(), mt6333_get_qi_vcore_dlc_n());
                    }

                    pr_notice("[exec_vcore_dlc_test] mt6333_set_vcore_dlc=%d, mt6333_set_vcore_dlc_n=%d, mt6333_get_qi_vcore_dlc=%d, mt6333_get_qi_vcore_dlc_n=%d\n",
            ...
            default:
                pr_notice("BUCK=%d, Invalid channel value(%d)\n", index_val, i);
                break;
        }
    }
}
...
void exec_vmem_dlc_test(int index_val)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[exec_vmem_dlc_test] %d\n", i);
        ...
        switch(i){
            case 0:
                for(j=0;j<=MT6333_PMIC_VMEM_DLC_MASK;j++)
                {
                    ...
                    {
                        pr_notice("[exec_vmem_dlc_test] fail at mt6333_get_qi_vmem_dlc=%d, mt6333_get_qi_vmem_dlc_n=%d\n",
                            mt6333_get_qi_vmem_dlc(), mt6333_get_qi_vmem_dlc_n());
                    }

                    pr_notice("[exec_vmem_dlc_test] mt6333_set_vmem_dlc=%d, mt6333_set_vmem_dlc_n=%d, mt6333_get_qi_vmem_dlc=%d, mt6333_get_qi_vmem_dlc_n=%d\n",
...
void exec_vrf18_dlc_test(int index_val)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[exec_vrf18_dlc_test] %d\n", i);
        ...
        switch(i){
            case 0:
                for(j=0;j<=MT6333_PMIC_VRF18_DLC_MASK;j++)
                {
                    ...
                    {
                        pr_notice("[exec_vrf18_dlc_test] fail at mt6333_get_qi_vrf18_dlc=%d, mt6333_get_qi_vrf18_dlc_n=%d\n",
                            mt6333_get_qi_vrf18_dlc(), mt6333_get_qi_vrf18_dlc_n());
                    }

                    pr_notice("[exec_vrf18_dlc_test] mt6333_set_vrf18_dlc=%d, mt6333_set_vrf18_dlc_n=%d, mt6333_get_qi_vrf18_dlc=%d, mt6333_get_qi_vrf18_dlc_n=%d\n",
...
void PMIC_BUCK_DLC(int index_val)
{
    ...
    switch(index_val){
	  ...
	  default:
        pr_notice("[PMIC_BUCK_DLC] Invalid channel value(%d)\n", index_val);
        break;

    }

    pr_notice("[PMIC_BUCK_DLC] end....\n");
}
...
void exec_vcore_burst_subtest(void)
{
    ...
    pr_notice("[exec_vcore_burst_subtest]\n");

    for(i=0;i<=MT6333_PMIC_VCORE_BURST_SLEEP_MASK;i++)
    {
        mt6333_set_vcore_burst_sleep(i);

        pr_notice("[exec_vcore_burst_subtest] mt6333_set_vcore_bursth_sleep=%d\n", i);

        pr_notice("[exec_vcore_burst_subtest] mt6333_get_qi_vcore_burst=%d\n",
    ...
    for(i=0;i<=MT6333_PMIC_VCORE_BURST_ON_MASK;i++)
    {
        ...
        pr_notice("[exec_vcore_burst_subtest] mt6333_set_vcore_burst_on=%d\n", i);
...
void exec_vmem_burst_subtest(void)
{
    ...
    pr_notice("[exec_vmem_burst_subtest]\n");

    for(i=0;i<=MT6333_PMIC_VMEM_BURST_SLEEP_MASK;i++)
    {
        mt6333_set_vmem_burst_sleep(i);

        pr_notice("[exec_vmem_burst_subtest] mt6333_set_vmem_bursth_sleep=%d\n", i);

        pr_notice("[exec_vmem_burst_subtest] mt6333_get_qi_vmem_burst=%d\n",
    ...
    for(i=0;i<=MT6333_PMIC_VMEM_BURST_ON_MASK;i++)
    {
        ...
        pr_notice("[exec_vmem_burst_subtest] mt6333_set_vmem_burst_on=%d\n", i);
...
void exec_burst_subtest(int index_val)
{
    switch(index_val){
        ...
        case 2:
            ...
            break;
        default:
            pr_notice("[exec_burst_subtest] Invalid channel value(%d)\n", index_val);
            break;
    }
}
...
void exec_vcore_burst_test(int index_val)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[exec_vcore_burst_test] %d\n", i);
        ...
        switch(i){
            case 0:
                for(j=0;j<=MT6333_PMIC_VCORE_BURST_MASK;j++)
                {
                    ...
                    {
                        pr_notice("[exec_vcore_burst_test] fail at mt6333_get_qi_vcore_burst=%d\n",
                            mt6333_get_qi_vcore_burst());
                    }

                    pr_notice("[exec_vcore_burst_test] mt6333_set_vcore_burst=%d, mt6333_get_qi_vcore_burst=%d\n",
...
void exec_vmem_burst_test(int index_val)
{
    ...
    for(i=0;i<=1;i++)
    {
        pr_notice("[exec_vmem_burst_test] %d\n", i);
        ...
        switch(i){
            case 0:
                for(j=0;j<=MT6333_PMIC_VMEM_BURST_MASK;j++)
                {
                    ...
                    {
                        pr_notice("[exec_vmem_burst_test] fail at mt6333_get_qi_vmem_burst=%d\n",
                            mt6333_get_qi_vmem_burst());
                    }

                    pr_notice("[exec_vmem_burst_test] mt6333_set_vmem_burst=%d, mt6333_get_qi_vmem_burst=%d\n",
...
{
    pr_notice("[PMIC_BUCK_BURST] start....\n");
    ...
    switch(index_val){
	  ...
	  default:
        pr_notice("[PMIC_BUCK_BURST] Invalid channel value(%d)\n", index_val);
        break;

    }

    pr_notice("[PMIC_BUCK_BURST] end....\n");
}
...
void tc_2207(void)
{
    ...
    pr_notice("[tc_2207] Done\n");
}

void tc_2208(void)
{
    pr_notice("[tc_2208] integrate into tc_2207\n");
    PMIC_BUCK_VOSEL(VCORE_INDEX);
    pr_notice("[tc_2208] Done\n");
}

void tc_2209(void)
{
    pr_notice("[tc_2209] integrate into tc_2207\n");
    read_adc_value(3);
    pr_notice("[tc_2209] Done\n");
}
...
int is_charger_exist(void)
{
    ...
    pr_notice("[is_charger_exist] RGS_CHRDET=%d\n", mt6333_get_rgs_chrdet());
    pr_notice("[is_charger_exist] RGS_CHR_PLUG_IN=%d\n", mt6333_get_rgs_chr_plug_in());
    pr_notice("[is_charger_exist] upmu_get_rgs_chrdet=%d\n", upmu_get_rgs_chrdet());
...
{
    pr_notice("[tc_3111] \n");
    pr_notice("[tc_3111] 1.1.1 Plug-in/out charger check charger plug-in status\n");

    while(is_charger_exist()==0)
    {
        pr_notice("please plug-in CHARGER!\n");
        mdelay(500);
    }

    pr_notice("[tc_3111] compare the following registers after charger plug-in\n");
    pr_notice("[tc_3111] RGS_CHR_PLUG_IN    need 1 => %d\n", mt6333_get_rgs_chr_plug_in());
    pr_notice("[tc_3111] RGS_CHR_MODE       need 1 => %d\n", mt6333_get_rgs_chr_mode());
    pr_notice("[tc_3111] RGS_POWER_ON_READY need 1 => %d\n", mt6333_get_rgs_power_on_ready());
    pr_notice("[tc_3111] \n");

    while(is_charger_exist()==1)
    {
        pr_notice("please plug-out CHARGER!\n");
        mdelay(500);
    }

    pr_notice("[tc_3111] compare the following register after charger plug-out\n");
    pr_notice("[tc_3111] RGS_CHR_PLUG_IN    need 0 => %d\n", mt6333_get_rgs_chr_plug_in());
    pr_notice("[tc_3111] RGS_CHR_MODE       need 0 => %d\n", mt6333_get_rgs_chr_mode());
    pr_notice("[tc_3111] RGS_POWER_ON_READY need 0 => %d\n", mt6333_get_rgs_power_on_ready());
    pr_notice("[tc_3111] \n");

    pr_notice("[tc_3111] Done\n");
}
...
void tc_3112(void)
{
    ...
    pr_notice("[tc_3112] \n");
    pr_notice("[tc_3112] 1.1.2 Check PWM, CP and M3 status after charger plug-in at CC mode (RG_CHR_EN=1 and 3.0V < VBAT < CV_Vth)\n");
    ...
    while(mt6333_get_rgs_power_on_ready() != 1)
    {
        ...
        {
            pr_notice("check rgs_power_on_ready fail\n");
            break;
        }
    }
    pr_notice("polling_time=%d of rgs_power_on_ready\n", polling_time);

    mt6333_set_rg_usbdl_mode_b(1);
    ret=mt6333_config_interface(0x04, 0x95, 0xFF, 0x0);

    pr_notice("[tc_3112] compare the following registers after charger plug-in\n");
    pr_notice("[tc_3112] mt6333_get_rgs_pwm_en                need 1 => %d\n", mt6333_get_rgs_pwm_en());
    pr_notice("[tc_3112] mt6333_get_rgs_cpcstsys_en           need 1 => %d\n", mt6333_get_rgs_cpcstsys_en());
    pr_notice("[tc_3112] mt6333_get_rgs_m3_en                 need 1 => %d\n", mt6333_get_rgs_m3_en());
    pr_notice("[tc_3112] mt6333_get_rgs_m3_s_en               need 1 => %d\n", mt6333_get_rgs_m3_s_en());
    pr_notice("[tc_3112] mt6333_get_rgs_m3_r_en               need 1 => %d\n", mt6333_get_rgs_m3_r_en());
    pr_notice("[tc_3112] mt6333_get_rgs_m3_boost_en           need 0 => %d\n", mt6333_get_rgs_m3_boost_en());
    pr_notice("[tc_3112] mt6333_get_rgs_pwm_voltage_config    need 0 => %d\n", mt6333_get_rgs_pwm_voltage_config());
    pr_notice("[tc_3112] mt6333_get_rgs_pwm_current_config    need 1 => %d\n", mt6333_get_rgs_pwm_current_config());
    pr_notice("[tc_3112] \n");
...
void tc_3113(void)
{
    ...
    pr_notice("[tc_3113] \n");
    pr_notice("[tc_3113] 1.1.3 Check PWM, CP and M3 status at chr_suspend mode (RG_CHR_EN=0 and CC_FLAG,3.4V < VBAT < CV_Vth) \n");
    ...
    pr_notice("[tc_3113] compare the following registers after charger plug-in\n");
    pr_notice("[tc_3113] mt6333_get_rgs_pwm_en                need 0 => %d\n", mt6333_get_rgs_pwm_en());
    pr_notice("[tc_3113] mt6333_get_rgs_cpcstsys_en           need 0 => %d\n", mt6333_get_rgs_cpcstsys_en());
    pr_notice("[tc_3113] mt6333_get_rgs_m3_en                 need 0 => %d\n", mt6333_get_rgs_m3_en());
    pr_notice("[tc_3113] mt6333_get_rgs_m3_s_en               need 0 => %d\n", mt6333_get_rgs_m3_s_en());
    pr_notice("[tc_3113] mt6333_get_rgs_m3_r_en               need 0 => %d\n", mt6333_get_rgs_m3_r_en());
    pr_notice("[tc_3113] mt6333_get_rgs_m3_boost_en           need 0 => %d\n", mt6333_get_rgs_m3_boost_en());
    pr_notice("[tc_3113] mt6333_get_rgs_pwm_voltage_config    need 1 => %d\n", mt6333_get_rgs_pwm_voltage_config());
    pr_notice("[tc_3113] mt6333_get_rgs_pwm_current_config    need 0 => %d\n", mt6333_get_rgs_pwm_current_config());
    pr_notice("[tc_3113] \n");
...
{
    pr_notice("[tc_3121] \n");
    pr_notice("[tc_3121] // 1.2.1 Set charger to auto-pwron state, and leave auto-pwron state by RG_USBDL_MODE_B\n");
    ...
    pr_notice("[tc_3121] after charger plug-in, polling\n");
    pr_notice("[tc_3121] RGS_AUTO_PWRON         need 1 => %d\n", mt6333_get_rgs_auto_pwron());
    pr_notice("[tc_3121] RGS_AUTO_PWRON_DONE    need 0 => %d\n", mt6333_get_rgs_auto_pwron_done());
    pr_notice("[tc_3121] RGS_POWER_ON_READY     need 1 => %d\n", mt6333_get_rgs_power_on_ready());
    pr_notice("[tc_3121] \n");

    pr_notice("[tc_3121] set RG_USBDL_MODE_B = 1\n");
    mt6333_set_rg_usbdl_mode_b(1);
    pr_notice("[tc_3121] \n");

    pr_notice("[tc_3121] compare the registers \n");
    pr_notice("[tc_3121] RGS_AUTO_PWRON         need 0 => %d\n", mt6333_get_rgs_auto_pwron());
    pr_notice("[tc_3121] RGS_AUTO_PWRON_DONE    need 1 => %d\n", mt6333_get_rgs_auto_pwron_done());
    pr_notice("[tc_3121] RGS_POWER_ON_READY     need 1 => %d\n", mt6333_get_rgs_power_on_ready());
    pr_notice("[tc_3121] \n");
    ...
    pr_notice("[tc_3121] Done\n");
}

void tc_3122(void)
{
    pr_notice("[tc_3122] 1.2.2 Set charger to auto-pwron state, and leave auto-pwron state by watchdog timeout\n");
    ...
    pr_notice("[tc_3122] after charger plug-in, polling\n");
    pr_notice("[tc_3122] RGS_AUTO_PWRON         need 1 => %d\n", mt6333_get_rgs_auto_pwron());
    pr_notice("[tc_3122] RGS_AUTO_PWRON_DONE    need 0 => %d\n", mt6333_get_rgs_auto_pwron_done());
    pr_notice("[tc_3122] RGS_POWER_ON_READY     need 1 => %d\n", mt6333_get_rgs_power_on_ready());
    pr_notice("[tc_3122] \n");
    ...
    {
        pr_notice("swchr wdt is not timeout!\n");
        mdelay(500);
    }
    pr_notice("Reg[0x%x]=0x%x\n", MT6333_CHRWDT_STATUS0, mt6333_get_reg_value(MT6333_CHRWDT_STATUS0));

    pr_notice("[tc_3122] compare the registers \n");
    pr_notice("[tc_3122] RGS_AUTO_PWRON         need 0 => %d\n", mt6333_get_rgs_auto_pwron());
    pr_notice("[tc_3122] RGS_AUTO_PWRON_DONE    need 1 => %d\n", mt6333_get_rgs_auto_pwron_done());
    pr_notice("[tc_3122] RGS_POWER_ON_READY     need 0 => %d\n", mt6333_get_rgs_power_on_ready());
    pr_notice("[tc_3122] \n");
...
void tc_3123(void)
{
    ...
    pr_notice("[tc_3123] Check if unable to enter auto-pwron state once leave auto-pwron state without charger plug-out \n");
    ...
    pr_notice("[tc_3123] after charger plug-in, polling\n");
    pr_notice("[tc_3123] RGS_AUTO_PWRON         need 1 => %d\n", mt6333_get_rgs_auto_pwron());
    pr_notice("[tc_3123] RGS_AUTO_PWRON_DONE    need 0 => %d\n", mt6333_get_rgs_auto_pwron_done());
    pr_notice("[tc_3123] RGS_POWER_ON_READY     need 1 => %d\n", mt6333_get_rgs_power_on_ready());
    pr_notice("[tc_3123] \n");

    pr_notice("[tc_3123] set RG_USBDL_MODE_B = 1\n");
    mt6333_set_rg_usbdl_mode_b(1);
    pr_notice("[tc_3123] RGS_AUTO_PWRON         need 0 => %d\n", mt6333_get_rgs_auto_pwron());
    pr_notice("[tc_3123] RGS_AUTO_PWRON_DONE    need 1 => %d\n", mt6333_get_rgs_auto_pwron_done());
    pr_notice("[tc_3123] RGS_POWER_ON_READY     need 1 => %d\n", mt6333_get_rgs_power_on_ready());

    pr_notice("[tc_3123] set RG_USBDL_MODE_B = 0\n");
    ...
    pr_notice("[tc_3123] Done\n");
}

void tc_3141(void)
{
    pr_notice("[tc_3141] \n");
    pr_notice("[tc_3141] reach the cut off current and compare the following register\n");
    pr_notice("[tc_3141] RGS_CHARGE_COMPLETE_HW = %d\n", mt6333_get_rgs_charge_complete_hw());

    mt6333_set_rg_int_en_chr_complete(1);

    pr_notice("[tc_3141] Can do test\n");
    //receive Interrupt

    pr_notice("[tc_3141] Done\n");
}
...
void tc_3211(void)
{
    ...
    pr_notice("[tc_3211] // -- 2.1.1 OTG mode on/off by setting RG_OTG_EN, check the power-on sequence d/a flags\n");
    pr_notice("[tc_3211] NO CHARGER!\n");

    pr_notice("[tc_3211] Disable HW CHRIN short auto protection\n");
    ret=mt6333_config_interface(0xF3,0x1,0x1,5);
    ret=mt6333_config_interface(0xF7,0x1,0x1,5);

    pr_notice("[tc_3211] set RG_OTG_EN = 1\n");
    ret=mt6333_config_interface(0x30,0x95,0xFF,0);
    mdelay(20);

    pr_notice("[tc_3211] RGS_OTG_MODE         need 1 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3211] RGS_CHR_MODE         need 0 => %d\n", mt6333_get_rgs_chr_mode());
    pr_notice("[tc_3211] RGS_OTG_DRV_EN       need 1 => %d\n", mt6333_get_rgs_otg_drv_en());
    pr_notice("[tc_3211] RSG_OTG_PRECC        need 0 => %d\n", mt6333_get_rgs_otg_precc());
    pr_notice("[tc_3211] RGS_OTG_EN           need 1 => %d\n", mt6333_get_rgs_otg_en());
    pr_notice("[tc_3211] RGS_CPCSTSYS_EN      need 1 => %d\n", mt6333_get_rgs_cpcstsys_en());
    pr_notice("[tc_3211] RGS_M3_EN            need 1 => %d\n", mt6333_get_rgs_m3_en());
    pr_notice("[tc_3211] RGS_M3_S_EN          need 1 => %d\n", mt6333_get_rgs_m3_s_en());
    pr_notice("[tc_3211] RGS_M3_R_EN          need 0 => %d\n", mt6333_get_rgs_m3_r_en());
    pr_notice("[tc_3211] RGS_M3_BOOST_EN      need 1 => %d\n", mt6333_get_rgs_m3_boost_en());
    pr_notice("[tc_3211] RGS_OTG_EN_STB       need 1 => %d\n", mt6333_get_rgs_otg_en_stb());
    pr_notice("[tc_3211] \n");

    pr_notice("[tc_3211] set RG_OTG_EN = 0\n");
    ret=mt6333_config_interface(0x30,0x94,0xFF,0);
    mdelay(20);

    pr_notice("[tc_3211] RGS_OTG_MODE         need 0 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3211] RGS_CHR_MODE         need 0 => %d\n", mt6333_get_rgs_chr_mode());
    pr_notice("[tc_3211] RGS_OTG_DRV_EN       need 0 => %d\n", mt6333_get_rgs_otg_drv_en());
    pr_notice("[tc_3211] RSG_OTG_PRECC        need 0 => %d\n", mt6333_get_rgs_otg_precc());
    pr_notice("[tc_3211] RGS_OTG_EN           need 0 => %d\n", mt6333_get_rgs_otg_en());
    pr_notice("[tc_3211] RGS_CPCSTSYS_EN      need 0 => %d\n", mt6333_get_rgs_cpcstsys_en());
    pr_notice("[tc_3211] RGS_M3_EN            need 0 => %d\n", mt6333_get_rgs_m3_en());
    pr_notice("[tc_3211] RGS_M3_S_EN          need 0 => %d\n", mt6333_get_rgs_m3_s_en());
    pr_notice("[tc_3211] RGS_M3_R_EN          need 0 => %d\n", mt6333_get_rgs_m3_r_en());
    pr_notice("[tc_3211] RGS_M3_BOOST_EN      need 0 => %d\n", mt6333_get_rgs_m3_boost_en());
    pr_notice("[tc_3211] RGS_OTG_EN_STB       need 0 => %d\n", mt6333_get_rgs_otg_en_stb());
    pr_notice("[tc_3211] \n");

    pr_notice("[tc_3211] Done\n");
}
...
void tc_3212(void)
{
    ...
    pr_notice("[tc_3212] // -- 2.1.2 Switching between charger mode and OTG mode\n");

    pr_notice("[tc_3212] Disable HW CHRIN short auto protection\n");
    ret=mt6333_config_interface(0xF3,0x1,0x1,5);
    ret=mt6333_config_interface(0xF7,0x1,0x1,5);
    ...
    //--------------------------------------------------------
    pr_notice("\n[tc_3212] a -------------------------\n");
    pr_notice("[tc_3212] set RG_OTG_EN = 1\n");
    ret=mt6333_config_interface(0x30,0x95,0xFF,0);
    mdelay(20);

    pr_notice("[tc_3212] RGS_OTG_MODE         need 1 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3212] RGS_CHR_MODE         need 0 => %d\n", mt6333_get_rgs_chr_mode());

    pr_notice("[tc_3212] set RG_OTG_EN = 0\n");
    ret=mt6333_config_interface(0x30,0x94,0xFF,0);
    mdelay(20);
    ...
    while(is_charger_exist()==0)
    {
        pr_notice("b please plug-in CHARGER!\n");
        mdelay(500);
    }
    pr_notice("[tc_3212] RGS_OTG_MODE         need 0 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3212] RGS_CHR_MODE         need 1 => %d\n", mt6333_get_rgs_chr_mode());
    while(is_charger_exist()==1)
    {
        pr_notice("b please plug-out CHARGER!\n");
        mdelay(500);
    }
    //--------------------------------------------------------
    pr_notice("\n[tc_3212] c -------------------------\n");
    ...
    //while(is_charger_exist()==0)
    {
        pr_notice("c please plug-in CHARGER!\n");
        mdelay(500);
    }
    pr_notice("[tc_3212] RGS_OTG_MODE         need 1 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3212] RGS_CHR_MODE         need 0 => %d\n", mt6333_get_rgs_chr_mode());
    ...
    //while(is_charger_exist()==1)
    {
        pr_notice("c please plug-out CHARGER!\n");
        mdelay(500);
    }
#endif
    //--------------------------------------------------------
    ...
    //while(is_charger_exist()==0)
    {
        pr_notice("d please plug-in CHARGER!\n");
        mdelay(500);
    }
    mdelay(1000);
    ...
    //while(is_charger_exist()==1)
    {
        pr_notice("d please plug-out CHARGER!\n");
        mdelay(500);
    }
...
void tc_3221(void)
{
    ...
    pr_notice("[tc_3221] // -- 2.2.1 watdog timeout at OTG mode, check OTG d/a flags\n");
    pr_notice("[tc_3221] NO CHARGER!\n");

    pr_notice("[tc_3221] Disable HW CHRIN short auto protection\n");
    ret=mt6333_config_interface(0xF3,0x1,0x1,5);
    ret=mt6333_config_interface(0xF7,0x1,0x1,5);

    pr_notice("[tc_3221] set RG_OTG_EN = 1\n");
    ret=mt6333_config_interface(0x30,0x95,0xFF,0);
    mdelay(20);
    ...
    pr_notice("[tc_3221] RGS_OTG_MODE         need 1 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3221] RGS_CHR_MODE         need 0 => %d\n", mt6333_get_rgs_chr_mode());
    pr_notice("[tc_3221] RGS_OTG_DRV_EN       need 1 => %d\n", mt6333_get_rgs_otg_drv_en());
    pr_notice("[tc_3221] RSG_OTG_PRECC        need 0 => %d\n", mt6333_get_rgs_otg_precc());
    pr_notice("[tc_3221] RGS_OTG_EN           need 1 => %d\n", mt6333_get_rgs_otg_en());
    pr_notice("[tc_3221] RGS_CPCSTSYS_EN      need 1 => %d\n", mt6333_get_rgs_cpcstsys_en());
    pr_notice("[tc_3221] RGS_M3_EN            need 1 => %d\n", mt6333_get_rgs_m3_en());
    pr_notice("[tc_3221] RGS_M3_S_EN          need 1 => %d\n", mt6333_get_rgs_m3_s_en());
    pr_notice("[tc_3221] RGS_M3_R_EN          need 0 => %d\n", mt6333_get_rgs_m3_r_en());
    pr_notice("[tc_3221] RGS_M3_BOOST_EN      need 1 => %d\n", mt6333_get_rgs_m3_boost_en());
    pr_notice("[tc_3221] RGS_OTG_EN_STB       need 1 => %d\n", mt6333_get_rgs_otg_en_stb());
    pr_notice("[tc_3221] \n");
    ...
    pr_notice("[tc_3221] RGS_OTG_MODE         need 0 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3221] RGS_CHR_MODE         need 0 => %d\n", mt6333_get_rgs_chr_mode());
    pr_notice("[tc_3221] RGS_OTG_DRV_EN       need 0 => %d\n", mt6333_get_rgs_otg_drv_en());
    pr_notice("[tc_3221] RSG_OTG_PRECC        need 0 => %d\n", mt6333_get_rgs_otg_precc());
    pr_notice("[tc_3221] RGS_OTG_EN           need 0 => %d\n", mt6333_get_rgs_otg_en());
    pr_notice("[tc_3221] RGS_CPCSTSYS_EN      need 0 => %d\n", mt6333_get_rgs_cpcstsys_en());
    pr_notice("[tc_3221] RGS_M3_EN            need 0 => %d\n", mt6333_get_rgs_m3_en());
    pr_notice("[tc_3221] RGS_M3_S_EN          need 0 => %d\n", mt6333_get_rgs_m3_s_en());
    pr_notice("[tc_3221] RGS_M3_R_EN          need 0 => %d\n", mt6333_get_rgs_m3_r_en());
    pr_notice("[tc_3221] RGS_M3_BOOST_EN      need 0 => %d\n", mt6333_get_rgs_m3_boost_en());
    pr_notice("[tc_3221] RGS_OTG_EN_STB       need 0 => %d\n", mt6333_get_rgs_otg_en_stb());
    pr_notice("[tc_3221] \n");

    pr_notice("[tc_3221] Done\n");
}
...
void tc_3311(void)
{
    ...
    pr_notice("[tc_3311] // -- 3.1.1 Flash light mode on/off by setting RG_FLASH_DRV_EN, check the power-on sequence d/a flags\n");
    pr_notice("[tc_3311] NO CHARGER!\n");

    pr_notice("[tc_3311] Disable HW CHRIN short auto protection\n");
    ret=mt6333_config_interface(0xF3,0x1,0x1,5);
    ret=mt6333_config_interface(0xF7,0x1,0x1,5);

    pr_notice("[tc_3311] set RG_FLASH_DRV_EN = 1\n");
    mt6333_set_rg_flash_drv_en(1);
    mdelay(20);

    pr_notice("[tc_3311] RGS_OTG_MODE         need 1 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3311] RGS_CHR_MODE         need 0 => %d\n", mt6333_get_rgs_chr_mode());
    pr_notice("[tc_3311] RGS_OTG_DRV_EN       need 1 => %d\n", mt6333_get_rgs_otg_drv_en());
    pr_notice("[tc_3311] RSG_OTG_PRECC        need 0 => %d\n", mt6333_get_rgs_otg_precc());
    pr_notice("[tc_3311] RGS_OTG_EN           need 1 => %d\n", mt6333_get_rgs_otg_en());
    pr_notice("[tc_3311] RGS_CPCSTSYS_EN      need 1 => %d\n", mt6333_get_rgs_cpcstsys_en());
    pr_notice("[tc_3311] RGS_M3_EN            need 1 => %d\n", mt6333_get_rgs_m3_en());
    pr_notice("[tc_3311] RGS_M3_S_EN          need 1 => %d\n", mt6333_get_rgs_m3_s_en());
    pr_notice("[tc_3311] RGS_M3_R_EN          need 0 => %d\n", mt6333_get_rgs_m3_r_en());
    pr_notice("[tc_3311] RGS_M3_BOOST_EN      need 1 => %d\n", mt6333_get_rgs_m3_boost_en());
    pr_notice("[tc_3311] RGS_OTG_EN_STB       need 1 => %d\n", mt6333_get_rgs_otg_en_stb());
    pr_notice("[tc_3311] \n");

    pr_notice("[tc_3311] set RG_FLASH_DRV_EN = 0\n");
    mt6333_set_rg_flash_drv_en(0);
    mdelay(20);

    pr_notice("[tc_3311] RGS_OTG_MODE         need 0 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3311] RGS_CHR_MODE         need 0 => %d\n", mt6333_get_rgs_chr_mode());
    pr_notice("[tc_3311] RGS_OTG_DRV_EN       need 0 => %d\n", mt6333_get_rgs_otg_drv_en());
    pr_notice("[tc_3311] RSG_OTG_PRECC        need 0 => %d\n", mt6333_get_rgs_otg_precc());
    pr_notice("[tc_3311] RGS_OTG_EN           need 0 => %d\n", mt6333_get_rgs_otg_en());
    pr_notice("[tc_3311] RGS_CPCSTSYS_EN      need 0 => %d\n", mt6333_get_rgs_cpcstsys_en());
    pr_notice("[tc_3311] RGS_M3_EN            need 0 => %d\n", mt6333_get_rgs_m3_en());
    pr_notice("[tc_3311] RGS_M3_S_EN          need 0 => %d\n", mt6333_get_rgs_m3_s_en());
    pr_notice("[tc_3311] RGS_M3_R_EN          need 0 => %d\n", mt6333_get_rgs_m3_r_en());
    pr_notice("[tc_3311] RGS_M3_BOOST_EN      need 0 => %d\n", mt6333_get_rgs_m3_boost_en());
    pr_notice("[tc_3311] RGS_OTG_EN_STB       need 0 => %d\n", mt6333_get_rgs_otg_en_stb());
    pr_notice("[tc_3311] \n");

    pr_notice("[tc_3311] Done\n");
}
...
void tc_3312(void)
{
    ...
    pr_notice("[tc_3312] // -- 3.1.2 Switching between charger mode and flash light mode \n");

    pr_notice("[tc_3312] Disable HW CHRIN short auto protection\n");
    ret=mt6333_config_interface(0xF3,0x1,0x1,5);
    ret=mt6333_config_interface(0xF7,0x1,0x1,5);
    ...
    pr_notice("[tc_3312] set mt6333_set_rg_flash_drv_en = 1\n");
    mt6333_set_rg_flash_drv_en(1);
    mdelay(20);

    pr_notice("[tc_3312] RGS_OTG_MODE         need 1 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3312] RGS_CHR_MODE         need 0 => %d\n", mt6333_get_rgs_chr_mode());

    pr_notice("[tc_3312] set mt6333_set_rg_flash_drv_en = 0\n");
    mt6333_set_rg_flash_drv_en(0);
    mdelay(20);
    ...
    pr_notice("[tc_3312] RGS_OTG_MODE         need 0 => %d\n", mt6333_get_rgs_otg_mode());
    pr_notice("[tc_3312] RGS_CHR_MODE         need 1 => %d\n", mt6333_get_rgs_chr_mode());
    while(is_charger_exist()==1)
    {
        pr_notice("please plug-out CHARGER!\n");
        mdelay(500);
...
{
    pr_notice("[show_mt6333_dvt] 0x%x\n", g_reg_value_mt6333);
...
static ssize_t store_mt6333_dvt(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_notice("[store_mt6333_dvt] \n");

    if(buf != NULL && size != 0)
    {
        pr_notice("[store_mt6333_dvt] buf is %s and size is %zu \n",buf,size);
        test_item = simple_strtoul(buf,&pvalue,10);
        pr_notice("[store_mt6333_dvt] test_item=%d\n", test_item);
...
{
    pr_notice("[show_mt6333_access] 0x%x\n", g_reg_value_mt6333);
    return sprintf(buf, "%u\n", g_reg_value_mt6333);
}
static ssize_t store_mt6333_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_notice("[store_mt6333_access] \n");

    if(buf != NULL && size != 0)
    {
        pr_notice("[store_mt6333_access] buf is %s and size is %zu \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 3)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
            pr_notice("[store_mt6333_access] write mt6333 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        else
        {
            ...
            pr_notice("[store_mt6333_access] read mt6333 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_mt6333);
            pr_notice("[store_mt6333_access] Please use \"cat mt6333_access\" to get value\r\n");
        }
    }
    return size;
}
...
void MT6333_PMIC_INIT_SETTING_V1_hpm(void)
{
    ...
    pr_notice( "[MT6333_PMIC_INIT_SETTING_V1_hpm] PMIC Chip = %x\n",g_mt6333_cid);
    pr_notice( "[MT6333_PMIC_INIT_SETTING_V1_hpm] 20130902\n");
    ...
    pr_notice( "after MT6333 init : Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
    ...
    // DCT
    pr_notice( "[MT6333_PMIC_INIT_SETTING_V1_hpm] pmu_drv_tool_customization_init\n");
    pr_notice( "MT6333 before: Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
        0x68, mt6333_get_reg_value(0x68),
        0x80, mt6333_get_reg_value(0x80),
        0x9F, mt6333_get_reg_value(0x9F)
        );
    pmu_drv_tool_customization_init(); // #ifdef CONFIG_MTK_MT6333_SUPPORT
    dump_ldo_status_read_debug();
    pr_notice( "MT6333 after: Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
        0x68, mt6333_get_reg_value(0x68),
        0x80, mt6333_get_reg_value(0x80),
        0x9F, mt6333_get_reg_value(0x9F)
        );
...
static int mt6333_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    pr_notice( "[mt6333_driver_probe] g_mt6333_cid=0x%x\n", g_mt6333_cid);
    ...
    pr_notice( "[MT6333_PMIC_INIT_SETTING_V1_hpm] Done\n");

    //MT6333 Interrupt Service
    MT6333_EINT_SETTING();
    pr_notice( "[MT6333_EINT_SETTING] Done\n");

    kthread_run(mt6333_thread_kthread, NULL, "mt6333_thread_kthread");
    pr_notice( "[mt6333_thread_kthread] Done\n");
...
static int mt6333_user_space_probe(struct platform_device *dev)
{
    ...
    pr_notice("******** mt6333_user_space_probe!! ********\n" );
...
static int __init mt6333_init(void)
{
    ...
    pr_notice("[mt6333_init] init start.\n");
    ...
    {
        pr_notice("[mt6333_init] failed to register mt6333 i2c driver.\n");
    }
    else
    {
        pr_notice("[mt6333_init] Success to register mt6333 i2c driver.\n");
    }

    // mt6333 user space access interface
    ret = platform_device_register(&mt6333_user_space_device);
    if (ret) {
        pr_notice("****[mt6333_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&mt6333_user_space_driver);
    if (ret) {
        pr_notice("****[mt6333_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
