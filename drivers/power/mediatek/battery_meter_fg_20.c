...
static int battery_meter_resume(struct platform_device *dev)
{
	...
	if(bat_is_charger_exist() == KAL_FALSE) {
		...
		oam_car_tmp=ap_suspend_car+(oam_i * time); 	/* 0.1mAh */
		...
		bm_print(BM_LOG_CRTI, "[battery_meter_resume]time:%d bat:%d ocv:%d r:%d i:%d ocar:%d ncar:%d lbat:%d %d\n",time,voltage,oam_v_ocv,oam_r,oam_i,ap_suspend_car/3600,oam_car_tmp/3600, 
